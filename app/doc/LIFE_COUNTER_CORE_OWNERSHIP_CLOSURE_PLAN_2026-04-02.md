...
- quando `player state` nasce de takeover do option-card do Lotus, o host agora tambem consegue fechar a superficie stale sem `reload` nos recortes seguros de dismiss sem mudanca, `partner commander`, `player counter` oculto, `commander damage` oculto e `appearance` com background solido, preservando `bundle_reload` nos subfluxos que ainda dependem de materializacao visual no runtime
...
- `player appearance` agora tambem pode fechar por `live_runtime` sem rebootar o bundle quando a mutacao fica limitada ao background solido de um unico jogador; nickname e imagens continuam em `reload_fallback`
- o takeover de `player appearance` vindo do `color card` agora tambem consegue fechar sem `reload` no dismiss sem mudanca, usando reset live do DOM para limpar a surface stale
- a surface direta de `player appearance` aberta pelo background do player agora tambem consegue fechar sem `reload` no dismiss sem mudanca, usando o mesmo reset live do DOM para limpar a `option-card`
- `player state` agora tambem herda esse mesmo recorte de `live_runtime` quando o efeito final do hub fica limitado a `player appearance` com background solido de um unico jogador
...
- os fluxos `open-native-*` agora tambem expõem `fallback_classification`, separando `ownership_bridge`, `support_utility` e `excluded_core_support`; isso fecha a leitura operacional do que ainda e ponte de ownership, do que e utilitario legitimo e do que ficou explicitamente fora do core desta fase
...
- a borda da shell agora tambem expõe `native_fallback_surface_rejected`, deixando explicito quando um `open-native-*` foi rejeitado por tipo desconhecido ou payload invalido em vez de morrer silenciosamente antes da abertura da sheet
- a borda da shell agora tambem rejeita explicitamente superfices nativas de jogador sem `targetPlayerIndex`, evitando que payload quebrado do Lotus caia em abertura com alvo implicito `0`
...
- a bateria Android de validacao final agora enumera explicitamente todos os arquivos `life_counter*_smoke_test.dart` presentes em `app/integration_test`, o que fecha a cobertura inventariada desta frente e desloca os proximos slices para decisao tecnica ou estabilizacao, nao mais para descoberta de testes soltos
