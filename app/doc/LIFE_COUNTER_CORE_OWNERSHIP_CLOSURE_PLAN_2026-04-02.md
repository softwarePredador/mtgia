...
- quando `player state` nasce de takeover do option-card do Lotus, o host agora tambem consegue fechar a superficie stale sem `reload` nos recortes seguros de dismiss sem mudanca e `partner commander`, preservando `bundle_reload` nos subfluxos que ainda dependem de materializacao visual no runtime
