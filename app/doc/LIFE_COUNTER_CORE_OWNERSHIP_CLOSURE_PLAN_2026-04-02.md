...
- `settings`, `player appearance` e `set life` agora tambem expõem `sync_blockers` nos applies puramente `reload-only`, deixando explicito quando o fallback veio de restricao arquitetural conhecida, e nao de falha de runtime ou de gating de elegibilidade
