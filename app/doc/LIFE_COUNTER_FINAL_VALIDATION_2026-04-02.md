...
- `flutter test integration_test/life_counter_native_player_counter_hidden_live_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
- `flutter test integration_test/life_counter_native_player_counter_visible_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
- `flutter test integration_test/life_counter_native_player_counter_hidden_autokill_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_player_counter_visible_cards_autokill_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_commander_damage_hidden_lifeloss_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_commander_damage_visible_cards_lifeloss_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_player_appearance_image_present_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_set_life_high_delta_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_player_state_hidden_counter_autokill_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_player_state_hidden_counter_visible_cards_autokill_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_player_state_hidden_commander_damage_lifeloss_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_native_player_state_hidden_commander_damage_visible_cards_lifeloss_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_player_state_partner_commander_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- `flutter test integration_test/life_counter_player_state_partner_commander_partner_image_fallback_smoke_test.dart -d emulator-5554 --reporter expanded --no-version-check`
...
- o atalho direto de `Set Life` tambem ficou validado no limite complementar de fallback: quando o delta passa do limite seguro de patch live, aplicar um novo total pela shell direta preserva a sessao canonica, mas o probe JS confirma que esse recorte continua em `reload_fallback` com `life_delta_exceeds_live_limit`.
...
- o hub de `Player State -> Manage Counters` tambem ficou validado no limite complementar do recorte oculto: com counters escondidos no player card, mas `autoKill=true`, aplicar `poison` pelo hub preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `poison_auto_kill_enabled`.
...
- o hub de `Player State -> Manage Counters` tambem ficou validado no limite complementar do recorte misto: com o player card ainda visivel, counters regulares escondidos e `autoKill=true`, aplicar `poison` pelo hub preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `poison_auto_kill_enabled`.
...
- o hub de `Player State -> Commander Damage` tambem ficou validado no limite complementar do recorte oculto: com commander damage escondido no player card, mas `lifeLossOnCommanderDamage=true`, aplicar dano pelo hub preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `life_loss_on_commander_damage_enabled`.
...
- o hub de `Player State -> Commander Damage` tambem ficou validado no limite complementar do recorte misto: com o player card ainda visivel, commander damage escondido e `lifeLossOnCommanderDamage=true`, aplicar dano pelo hub preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `life_loss_on_commander_damage_enabled`.
...
- o toggle de `Partner commander` pelo `Player State` tambem ficou validado no limite complementar do cenario padrao: com `showCountersOnPlayerCard=true` e counters ainda visiveis no player card, ativar o partner commander preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `show_counters_on_player_card_enabled`.
- o toggle de `Partner commander` pelo `Player State` tambem ficou validado no limite complementar com `backgroundImagePartner` presente: mesmo no recorte misto com player card visivel e counters regulares escondidos, ativar o partner commander preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `partner_background_image_present`.
...
- o apply direto de `player appearance` tambem ficou validado quando o jogador ja carrega `backgroundImage` e `backgroundImagePartner`: mesmo mudando apenas o `background` solido pela surface direta, a sessao canonica preserva as refs de imagem e o probe JS confirma que esse recorte segue em `reload_fallback` enquanto o payload ainda depende de assets visuais do Lotus.
...
- o fluxo direto de `Player Counter` tambem ficou validado no limite padrao visivel: com `showCountersOnPlayerCard=true`, aplicar `poison` pela shell direta preserva a sessao canonica, e o probe JS confirma que esse recorte continua em `reload_fallback` por `show_counters_on_player_card_enabled`.
...
- o fluxo direto de `Player Counter` tambem ficou validado no limite complementar do recorte misto: com `showCountersOnPlayerCard=true`, `showRegularCounters=false` e `autoKill=true`, aplicar `poison` pela shell direta preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `poison_auto_kill_enabled`.
...
- o fluxo direto de `Player Counter` tambem ficou validado no limite complementar do recorte oculto: com `showCountersOnPlayerCard=false`, mas `autoKill=true`, aplicar `poison` pela shell direta preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `poison_auto_kill_enabled`.
...
- a shell base de `Player State` tambem ficou validada no Android real nos dois gatilhos centrais do bundle: o takeover vindo de `player_option_card_presented` continua aplicando `Partner commander + Answer Left`, e o takeover vindo de `killed_overlay_pressed` continua aplicando `Decked Out`, ambos preservando a sessao canonica no fluxo esperado.
...
- o fluxo direto de `Commander Damage` tambem ficou validado no limite complementar do recorte oculto: com commander damage escondido no player card, mas `lifeLossOnCommanderDamage=true`, aplicar dano pela shell direta preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `life_loss_on_commander_damage_enabled`.
...
- o fluxo direto de `Commander Damage` tambem ficou validado no limite complementar do recorte misto: com `showCountersOnPlayerCard=true`, `showCommanderDamageCounters=false`, `autoKill=false` e `lifeLossOnCommanderDamage=true`, aplicar dano pela shell direta preserva a sessao canonica, e o probe JS confirma que esse caso continua em `reload_fallback` por `life_loss_on_commander_damage_enabled`.
...
- o hub de `Player State -> Set Life` tambem ficou validado no caminho vivo com `autoKill=true`: reduzir a vida abaixo de zero pelo ajuste rapido aplica o nocaute canonico no Android real, levando a sessao para `life=0` e registrando `Jogador 2 foi nocauteado` como `lastTableEvent`.
...
- o roundtrip de `table state` tambem ficou validado no snapshot vivo do Lotus: `stormCount`, `monarchPlayer`, `initiativePlayer`, `lastPlayerRolls`, `lastHighRolls` e `firstPlayerIndex` materializam `__manaloom_table_state` no payload persistido e voltam coerentes para a sessao canonica quando o app rehidrata somente a partir desse snapshot.
...
- o roundtrip de `commander damage` tambem ficou validado no snapshot vivo do Lotus: os detalhes de `commanderOneDamage/commanderTwoDamage` materializam o array `commanderDamage` do payload `players`, e depois voltam coerentes para `resolvedCommanderDamageDetails` quando o app rehidrata a partir do snapshot persistido.
...
- o roundtrip de `commander casts` tambem ficou validado no snapshot vivo do Lotus: `partnerCommander` e os detalhes de `commanderOneCasts/commanderTwoCasts` materializam `tax-1` e `tax-2` no payload `players`, e depois voltam coerentes para a sessao canonica quando o app rehidrata a partir do snapshot persistido.
...
- o roundtrip de `extra counters` tambem ficou validado no snapshot vivo do Lotus: counters customizados como `charge`, `rad` e `tickets` materializam o mapa `counters` no payload `players`, e depois voltam coerentes para `playerExtraCounters` quando o app rehidrata a partir do snapshot persistido.
