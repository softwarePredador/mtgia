...
- o atalho direto de `set life` agora tambem expõe `apply_strategy: live_runtime` e `reload_required: false` no recorte em que a mudanca fica limitada a um delta curto de vida no jogador alvo e o runtime do Lotus confirma os controles da mesa; fora desse recorte, o dominio continua em `reload_fallback`
