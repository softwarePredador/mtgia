...
- `dice` agora tambem expõe `apply_strategy: canonical_store_sync` e `reload_required: false` no recorte em que a shell so altera resultado canonico com `turn tracker` inativo; `commander damage`, `player appearance`, `player counter`, `player state` e `set life` continuam em `reload_fallback`
...
- `dice` agora tambem tem um recorte sem `reload` por `canonical_store_sync` quando a shell so altera `last rolls`, `first player` e `last event` com `turn tracker` inativo; `commander damage`, `player appearance`, `player counter`, `player state` e `set life` continuam em `reload`
