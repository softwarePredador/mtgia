...
| Análise funcional | `DeckAnalysisTab` aba `Análise` | `deck-analysis-functional-section-<deckId>`, `deck-analysis-functional-origin-<deckId>`, `deck-analysis-functional-bucket-<deckId>-<ramp|draw|removal|wipes|protection>`, `deck-analysis-functional-count-<deckId>-<bucket>`, `deck-analysis-functional-samples-<deckId>-<bucket>`, `deck-analysis-functional-sample-<deckId>-<bucket>-<index>` | Consome `/decks/:id/analysis` e mostra contagens + amostras de `functional_tags` com fallback para `stats.composition` legado. | Usar keys por deck/bucket; texto só como evidência da origem/cobertura. |
