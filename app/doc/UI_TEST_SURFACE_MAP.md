...
| Comandante na criação | `DeckListScreen` / `DeckCommanderSelector` | `deck-create-commander-section`, `deck-create-commander-select`, `deck-create-commander-selected`, `deck-create-commander-change`, `deck-create-commander-clear` | Commander/Brawl oferecem seleção opcional; outros formatos ocultam e limpam a escolha. O submit envia uma única carta com `quantity=1` e `is_commander=true` junto ao `POST /decks`. | Testar formato, seleção/troca/remoção, payload atômico, preservação após falha e ausência de payload oculto. |
| Picker de comandante | Dialog de busca | `commander-picker-dialog`, `deck-create-commander-search-field`, `deck-create-commander-loading`, `deck-create-commander-error`, `deck-create-commander-empty`, `deck-create-commander-results`, `deck-create-commander-result-<cardId>` | Busca a partir de 3 letras e exibe somente candidatos estruturalmente elegíveis para Commander/Brawl, agrupados por identidade jogável; arte, tipo e identidade de cor permanecem visíveis. O backend filtra antes da paginação e continua sendo a autoridade final de legalidade ao criar. | Cobrir foco inicial, debounce, erro/retry no modal, vazio, ausência de candidato inelegível, Brawl Planeswalker, Escape e viewport compacto. |
...
| Lista de resultados | `CardSearchScreen` | `card-search-results-list` | Renderiza uma linha por identidade jogável no Deckbuilder/Life Counter; o Binder preserva impressões físicas. | Screenshot + contagem mínima; provar Lorehold único fora do Binder. |
| Resultado de carta | `CardSearchScreen` | `card-search-result-<cardId>`, `card-search-printing-count-<cardId>` | Linha com nome, edição representativa, quantidade de impressões agrupadas e restrições Commander. Em seleção de comandante, candidatos inelegíveis não são renderizados. | `find.byKey` + texto como evidência; provar `2 impressões` e ausência de carta comum. |
...
| Leitura da carta sugerida | `OptimizationPreviewDialog` | `optimize-suggestion-<add\|remove>-<index>-preview-button`, `optimize-suggestion-<add\|remove>-<index>-hover-card-preview`, `optimize-suggestion-<add\|remove>-<index>-card-preview-panel` | Mouse e foco mostram a carta sem alterar a seleção; botão ou toque prolongado abrem o leitor responsivo na mesma rota/modal. A impressão é carregada por `card_id` somente sob demanda e respostas legadas usam fallback exato por nome. Escape/Back fecha primeiro o leitor e restaura o foco. | Mouse real com atraso/saída, foco por teclado, Escape, botão e long press em viewport compacto; validar cache, fallback, alvo de 48 px e checkbox inalterado. |
