                ...
                children: [
                  Material(
                    ...
                    child: CheckboxListTile(
                      key: const Key('deck-import-list-dialog-replace-switch'),
                      value: replaceAll,
                      onChanged: (value) {
                  ...
                  if (notFoundLines.isNotEmpty) ...[
                    const SizedBox(height: AppTheme.space12),
                    Container(
                      key: const Key('deck-import-list-dialog-not-found'),
                      padding: const EdgeInsets.all(AppTheme.space8),
                      decoration: BoxDecoration(
                  ...
                  if (importWarnings.isNotEmpty ||
                    ...
                    DeckDialogSectionCard(
                      key: const Key('deck-import-list-dialog-import-status'),
                      title: 'Status da importação',
                      accent: importWarnings.isNotEmpty || missingCommander
                          ? AppTheme.warning
                          : AppTheme.success,
                      icon: importWarnings.isNotEmpty || missingCommander
              ...
              onPressed: isImporting
                  ...
                  : () async {
                      ...
                      setDialogState(() {
                        ...
                        localizedMatchesCount = resultLocalizedMatchesCount;
                        missingCommander = resultMissingCommander;
                        commanderPreserved = resultCommanderPreserved;
                        importReviewVisible =
                            result['success'] == true && hasReviewDetails;
                      });

                      if (result['success'] == true) {
                        final imported = _intFrom(result['cards_imported']);
                        showSnackBar(
                        ...
                        setDialogState(() {
                          error =
                              result['error']?.toString() ?? 'Erro ao importar';
                        });
                      }
                    },
              icon: isImporting
                  ...
                  : Icon(importReviewVisible ? Icons.check : Icons.upload),
              label: Text(
                isImporting
                    ? 'Importando...'
                    : importReviewVisible
