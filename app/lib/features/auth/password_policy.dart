const registrationPasswordMinimumLength = 12;
const registrationPasswordMaximumLength = 256;
...
String? validateRegistrationPassword(
  ...
  if (length < registrationPasswordMinimumLength) {
    return 'Senha deve ter no mínimo 12 caracteres.';
  }
  if (length > registrationPasswordMaximumLength) {
    return 'Senha deve ter no máximo 256 caracteres.';
  }
  ...
  if (_isCommonPattern(canonicalPassword, password) ||
      containsAccountIdentity) {
    return 'Escolha uma senha menos previsível, sem sequências ou dados da conta.';
  }
  return null;
}

bool _isCommonPattern(String canonical, String original) {
  ...
  return original.runes.length >= 8 && original.runes.toSet().length <= 2;
}
...
String? _canonicalizeNullable(String? value) {
