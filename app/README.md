...
- `dice` agora tambem tem um recorte sem `reload` por `canonical_store_sync` quando a shell so altera resultado canonico de rolagem com `turn tracker` inativo; `commander damage`, `player appearance`, `player counter`, `player state` e `set life` continuam `reload-only`
...
- `dice` agora tambem anota `apply_strategy: canonical_store_sync`, `reload_required: false` e `live_patch_eligible: false` no recorte em que so muda resultado canonico com `turn tracker` inativo; `commander damage`, `player appearance`, `player counter`, `player state` e `set life` seguem em `reload_fallback`
