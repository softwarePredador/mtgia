...
void main() {
  group('buildLotusFallbackBootstrapValues', () {
    ...
    test('builds canonical bootstrap values when Lotus snapshot is missing', () async {
      ...
      final restoredSettings = LotusLifeCounterSettingsAdapter.tryBuildSettings(
        snapshot,
      );
      final restoredGameTimer = LotusLifeCounterGameTimerAdapter.tryBuildState(
        snapshot,
      );
      ...
      expect(restoredHistory.currentGameEntries.single.message, 'Player 4 lost 9 life');
      expect(
    ...
    test('builds history-only bootstrap values when only canonical history exists', () async {
      ...
      expect(restoredHistory.currentGameEntries.single.message, 'Player 1 gained 4 life');
