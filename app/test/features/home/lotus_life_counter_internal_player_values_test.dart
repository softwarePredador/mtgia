...
void main() {
  group('LotusLifeCounterScreen internal player values fallback', () {
    ...
    testWidgets(
      'keeps commander damage canonical sync without reload when player cards stay on but commander damage counters stay hidden',
      (tester) async {
        late _FakeLotusHost host;
        await _captureDebugLogs((logs) async {
          await tester.binding.setSurfaceSize(const Size(900, 1200));
    ...
    testWidgets(
      'keeps player counter canonical sync without reload when player cards stay on but regular counters stay hidden',
      (tester) async {
        late _FakeLotusHost host;
        await _captureDebugLogs((logs) async {
          await LifeCounterSettingsStore().save(
