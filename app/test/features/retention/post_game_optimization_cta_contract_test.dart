...
void main() {
  test('post-game screen links diagnostics to optimize and rebuild flows', () {
    ...
    final optimizeSectionsSource = File(
      'lib/features/decks/widgets/deck_optimize_sections.dart',
    ).readAsStringSync();

    expect(postGameSource, contains("path: '/decks/\${widget.deckId}'"));
    expect(postGameSource, contains("'postGameNoteId': note.id"));
    expect(
      postGameSource,
      contains("'optimize': rebuild ? 'rebuild' : 'post_game'"),
    );
    expect(
      postGameSource,
