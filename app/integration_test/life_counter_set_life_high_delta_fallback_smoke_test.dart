...
Future<Map<String, dynamic>> _readSetLifeFallbackState(
  WidgetTester tester,
  LotusStorageSnapshotStore snapshotStore,
  dynamic screenState,
) async {
  const storageKey = '__manaloom_test_set_life_high_delta_fallback';
  const nonceKey = '__manaloom_test_set_life_high_delta_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomSetLifeHighDeltaFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets('falls back to bundle reload on direct set life with high delta', (
    tester,
  ) async {
...
(() => {
  window.__manaloomSetLifeHighDeltaFallbackProbe = 'alive';
})()
''');
    ...
    expect(players[1]['life'], 50);

    final fallbackState = await _readSetLifeFallbackState(
      tester,
      snapshotStore,
      state,
    );
    expect(fallbackState['probe'], isNull);
  });
}
