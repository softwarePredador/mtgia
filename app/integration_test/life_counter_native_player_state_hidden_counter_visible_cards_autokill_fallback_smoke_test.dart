...
Future<Map<String, dynamic>> _readPlayerCounterHubState(
  ...
  const storageKey =
      '__manaloom_test_player_state_hidden_counter_visible_cards_autokill_fallback';
  const nonceKey =
      '__manaloom_test_player_state_hidden_counter_visible_cards_autokill_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerStateHiddenCounterVisibleCardsAutokillFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on mixed hidden player counter through player state when auto kill stays enabled',
    (tester) async {
...
(() => {
  window.__manaloomPlayerStateHiddenCounterVisibleCardsAutokillFallbackProbe = 'alive';
})()
''');
      ...
      final playerCounterHubState = await _readPlayerCounterHubState(
        tester,
        snapshotStore,
        state,
      );
      expect(playerCounterHubState['probe'], isNull);
