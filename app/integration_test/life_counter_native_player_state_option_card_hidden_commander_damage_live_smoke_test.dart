...
Future<Map<String, dynamic>> _readOptionCardCommanderDamageState(
  WidgetTester tester,
  LotusStorageSnapshotStore snapshotStore,
  dynamic screenState,
) async {
  const storageKey = '__manaloom_test_option_card_hidden_commander_damage_live';
  const nonceKey =
      '__manaloom_test_option_card_hidden_commander_damage_live_nonce';
...
(() => {
  try {
    ...
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomOptionCardCommanderDamageProbe ?? null,
      option_card_count:
...
void main() {
  ...
  testWidgets(
    'applies hidden commander damage from option-card takeover without bundle reload on the WebView path',
    (tester) async {
...
(() => {
  window.__manaloomOptionCardCommanderDamageProbe = 'alive';
})()
''');
      ...
      final optionCardState = await _readOptionCardCommanderDamageState(
        tester,
        snapshotStore,
        state,
      );
