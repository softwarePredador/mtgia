...
Future<Map<String, dynamic>> _readTableState(
  ...
  const storageKey = '__manaloom_test_table_state_clear_live';
  const nonceKey = '__manaloom_test_table_state_clear_live_nonce';
...
Future<void> _pumpUntilOwnershipCleared(
  WidgetTester tester,
  ...
  for (
    ...
    attempt < 20 &&
        (session == null ||
            session.monarchPlayer != null ||
            session.initiativePlayer != null);
    attempt += 1
  ) {
...
void main() {
  ...
  testWidgets('clears monarch and initiative live on the WebView path', (
    tester,
  ) async {
    ...
    await sessionStore.save(
      LifeCounterSession.initial(
        playerCount: 4,
      ).copyWith(monarchPlayer: 2, initiativePlayer: 1),
    );
    ...
    await tester.scrollUntilVisible(
      find.byKey(const Key('life-counter-native-table-state-initiative-clear')),
      250,
    ...
    await _pumpUntilOwnershipCleared(tester, sessionStore);
    ...
    expect(
      ...
      contains('"monarchPlayer":null'),
    );
    expect(
      rawSnapshot.values['__manaloom_table_state'],
      contains('"initiativePlayer":null'),
    );
  });
}
