...
Future<Map<String, dynamic>> _readAppearanceState(
  WidgetTester tester,
  LotusStorageSnapshotStore snapshotStore,
  dynamic screenState,
) async {
  const storageKey = '__manaloom_test_player_appearance_image_present';
  const nonceKey = '__manaloom_test_player_appearance_image_present_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerAppearanceImagePresentProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'keeps direct player appearance on reload fallback when image references are already present',
    (tester) async {
...
(() => {
  window.__manaloomPlayerAppearanceImagePresentProbe = 'alive';
})()
''');
      ...
      final appearanceState = await _readAppearanceState(
        tester,
        snapshotStore,
        state,
      );
