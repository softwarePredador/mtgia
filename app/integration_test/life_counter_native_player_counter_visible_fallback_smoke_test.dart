...
Future<Map<String, dynamic>> _readPlayerCounterState(
  ...
  const storageKey = '__manaloom_test_player_counter_visible_fallback';
  const nonceKey = '__manaloom_test_player_counter_visible_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerCounterVisibleFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on direct player counter when player cards stay enabled',
    (tester) async {
...
(() => {
  window.__manaloomPlayerCounterVisibleFallbackProbe = 'alive';
})()
''');
      ...
      expect(playerCounterState['probe'], isNull);
    },
  );
}
