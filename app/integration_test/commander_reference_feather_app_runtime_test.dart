...
class _FeatherCommanderCase {
  const _FeatherCommanderCase({
    required this.slug,
...
const _feather = _FeatherCommanderCase(
  slug: 'feather',
  commanderName: 'Feather, the Redeemed',
  prompt:
      'Commander Boros heroic spellslinger deck built around Feather with cheap '
      'protection, pump spells, Boros-legal card selection and cantrip effects, '
      'recursion/value, efficient removal and coherent combat win conditions '
      'while keeping the strategy interactive and on-theme.',
  allowedColors: {'R', 'W'},
...
void main() {
  ...
  testWidgets(
    'iPhone 15 runtime: Commander Reference Feather app proof',
    (tester) async {
      ...
      // ignore: avoid_print
      print('COMMANDER_REFERENCE_FEATHER_APP_BASE_URL ${ApiClient.baseUrl}');
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_FEATHER_APP_HEALTH status=${healthResponse.statusCode} '
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_FEATHER_APP_FINAL_SUMMARY ${jsonEncode(summary)}',
      );
    },
...
Future<_RuntimeCredentials> _registerByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_feather_01_login',
  );
  ...
  final username = 'runtime_feather_app_$unique';
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_feather_02_registered',
  );
...
Future<void> _loginByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_feather_03_logged_in',
  );
  _expectRuntimeUiHealthy(tester);
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_FEATHER_APP_AUTH_SUMMARY register_ui=true login_ui=true',
  );
}
...
Future<Map<String, dynamic>> _generateSaveOpenAndValidate(
  ...
  WidgetTester tester,
  ApiClient api,
  _FeatherCommanderCase runtimeCase,
) async {
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_FEATHER_APP_GENERATE_INITIAL_FEEDBACK '
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_feather_${runtimeCase.slug}_02_preview',
  );
  expect(find.text(runtimeCase.commanderName), findsWidgets);
  _expectRuntimeUiHealthy(tester);

  final unique = DateTime.now().millisecondsSinceEpoch.toRadixString(16);
  final deckName = 'QA Feather ${runtimeCase.slug} $unique';
  await tester.enterText(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_feather_${runtimeCase.slug}_03_details',
  );
  _expectRuntimeUiHealthy(tester);
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_FEATHER_APP_RUNTIME_SUMMARY ${jsonEncode(summary)}',
  );
...
Future<void> _openGenerateScreen(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_feather_${runtimeCase.slug}_01_generate_screen',
  );
}
...
Future<Map<String, dynamic>> _validateSavedDeck(
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_FEATHER_APP_VALIDATE_SUMMARY ${jsonEncode(summary)}',
  );
