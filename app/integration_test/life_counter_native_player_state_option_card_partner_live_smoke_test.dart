...
Future<Map<String, dynamic>> _readOptionCardResetState(
  WidgetTester tester,
  LotusStorageSnapshotStore snapshotStore,
  dynamic screenState,
) async {
  const storageKey = '__manaloom_test_option_card_partner_live';
  const nonceKey = '__manaloom_test_option_card_partner_live_nonce';
...
(() => {
  try {
    ...
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomOptionCardProbe ?? null,
      option_card_count:
...
void main() {
  ...
  testWidgets(
    'applies partner commander from option-card takeover without bundle reload on the WebView path',
    (tester) async {
...
(() => {
  window.__manaloomOptionCardProbe = 'alive';
})()
''');
      ...
      final optionCardState = await _readOptionCardResetState(
        tester,
        snapshotStore,
        state,
      );
