...
Future<LifeCounterSession?> _pumpUntilFirstPlayerApplied(
  WidgetTester tester,
  ...
  for (
    ...
    attempt < 20 &&
        (session == null ||
            session.firstPlayerIndex == null ||
            !(session.lastTableEvent?.startsWith('Primeiro jogador: ') ??
                false));
    attempt += 1
  ) {
...
Future<LifeCounterHistoryState?> _pumpUntilHistoryMirrorsLastEvent(
  ...
  for (
    ...
    attempt < 20 &&
        (history == null ||
            !(history.lastTableEvent?.startsWith('Primeiro jogador: ') ??
                false));
    attempt += 1
  ) {
    await tester.pump(const Duration(milliseconds: 500));
...
void main() {
  ...
  testWidgets(
    'applies first player roll with canonical sync on the live WebView path',
    (tester) async {
      ...
      final session = await _pumpUntilFirstPlayerApplied(tester, sessionStore);
      ...
      expect(session!.turnTrackerActive, isFalse);
      expect(session.firstPlayerIndex, isNotNull);
      expect(
        session.firstPlayerIndex,
        inInclusiveRange(0, session.playerCount - 1),
      );
      expect(session.lastTableEvent, startsWith('Primeiro jogador: '));
