...
Future<Map<String, dynamic>> _readAppearanceHubState(
  WidgetTester tester,
  LotusStorageSnapshotStore snapshotStore,
  dynamic screenState,
) async {
  const storageKey = '__manaloom_test_player_state_appearance_image_present';
  const nonceKey =
      '__manaloom_test_player_state_appearance_image_present_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerStateAppearanceImagePresentProbe ?? null
    }));
...
(() => {
  window.__manaloomPlayerStateAppearanceImagePresentProbe = 'alive';
})()
''');
      ...
      final appearanceHubState = await _readAppearanceHubState(
        tester,
        snapshotStore,
        state,
      );
      expect(appearanceHubState['probe'], isNull);
    },
  );
}
