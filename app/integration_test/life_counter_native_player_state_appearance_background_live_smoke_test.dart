...
Future<Map<String, dynamic>> _readPlayerAppearanceState(
  ...
  const storageKey = '__manaloom_test_player_state_appearance_live';
  const nonceKey = '__manaloom_test_player_state_appearance_live_nonce';
...
(() => {
  try {
    ...
    localStorage.setItem('$storageKey', JSON.stringify({
      ...
      probe: window.__manaloomPlayerStateAppearanceApplyProbe ?? null,
      option_card_count:
...
void main() {
  ...
  testWidgets(
    'applies solid player appearance live from the player state hub on the WebView path',
    (tester) async {
...
(() => {
  window.__manaloomPlayerStateAppearanceApplyProbe = 'alive';
})()
''');
