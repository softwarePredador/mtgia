...
Future<Map<String, dynamic>> _readCommanderDamageState(
  ...
  const storageKey =
      '__manaloom_test_commander_damage_visible_cards_lifeloss_fallback';
  const nonceKey =
      '__manaloom_test_commander_damage_visible_cards_lifeloss_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomCommanderDamageVisibleCardsLifeLossFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on direct commander damage while visible player cards keep life loss on commander damage enabled',
    (tester) async {
...
(() => {
  window.__manaloomCommanderDamageVisibleCardsLifeLossFallbackProbe = 'alive';
})()
''');
