...
Future<Map<String, dynamic>> _readPlayerCounterState(
  ...
  const storageKey =
      '__manaloom_test_player_counter_visible_cards_autokill_fallback';
  const nonceKey =
      '__manaloom_test_player_counter_visible_cards_autokill_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerCounterVisibleCardsAutokillFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on direct player counter while visible player cards keep poison auto kill enabled',
    (tester) async {
...
(() => {
  window.__manaloomPlayerCounterVisibleCardsAutokillFallbackProbe = 'alive';
})()
''');
