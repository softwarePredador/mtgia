...
Future<Map<String, dynamic>> _readPlayerCounterHubState(
  ...
  const storageKey =
      '__manaloom_test_player_state_hidden_counter_autokill_fallback';
  const nonceKey =
      '__manaloom_test_player_state_hidden_counter_autokill_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerStateHiddenCounterAutokillFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on hidden player counter through player state when auto kill stays enabled',
    (tester) async {
...
(() => {
  window.__manaloomPlayerStateHiddenCounterAutokillFallbackProbe = 'alive';
})()
''');
