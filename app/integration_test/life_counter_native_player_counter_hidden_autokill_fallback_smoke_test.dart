...
Future<Map<String, dynamic>> _readPlayerCounterState(
  ...
  const storageKey = '__manaloom_test_player_counter_hidden_autokill_fallback';
  const nonceKey =
      '__manaloom_test_player_counter_hidden_autokill_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomPlayerCounterHiddenAutokillFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on direct player counter while counters stay hidden but auto kill remains enabled',
    (tester) async {
...
(() => {
  window.__manaloomPlayerCounterHiddenAutokillFallbackProbe = 'alive';
})()
''');
