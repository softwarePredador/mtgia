...
Future<Map<String, dynamic>> _readOptionCardAppearanceState(
  WidgetTester tester,
  LotusStorageSnapshotStore snapshotStore,
  dynamic screenState,
) async {
  const storageKey = '__manaloom_test_option_card_player_state_appearance_live';
  const nonceKey =
      '__manaloom_test_option_card_player_state_appearance_live_nonce';
...
(() => {
  try {
    ...
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomOptionCardAppearanceProbe ?? null,
      option_card_count:
...
void main() {
  ...
  testWidgets(
    'applies solid player appearance from option-card takeover without bundle reload on the WebView path',
    (tester) async {
...
(() => {
  window.__manaloomOptionCardAppearanceProbe = 'alive';
})()
''');
      ...
      final appearanceState = await _readOptionCardAppearanceState(
        tester,
        snapshotStore,
        state,
      );
