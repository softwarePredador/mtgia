...
Future<Map<String, dynamic>> _readCommanderDamageState(
  ...
  const storageKey =
      '__manaloom_test_commander_damage_hidden_lifeloss_fallback';
  const nonceKey =
      '__manaloom_test_commander_damage_hidden_lifeloss_fallback_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomCommanderDamageHiddenLifeLossFallbackProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'falls back on hidden commander damage when life loss on commander damage stays enabled',
    (tester) async {
...
(() => {
  window.__manaloomCommanderDamageHiddenLifeLossFallbackProbe = 'alive';
})()
''');
