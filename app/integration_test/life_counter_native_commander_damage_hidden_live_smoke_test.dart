...
Future<Map<String, dynamic>> _readCommanderDamageState(
  ...
  const storageKey = '__manaloom_test_commander_damage_hidden_live';
  const nonceKey = '__manaloom_test_commander_damage_hidden_live_nonce';
...
(() => {
  try {
    localStorage.setItem('$storageKey', JSON.stringify({
      probe: window.__manaloomCommanderDamageHiddenLiveProbe ?? null
    }));
...
void main() {
  ...
  testWidgets(
    'applies hidden commander damage through the direct shell without reload',
    (tester) async {
...
(() => {
  window.__manaloomCommanderDamageHiddenLiveProbe = 'alive';
})()
''');
      ...
      expect(commanderDamageState['probe'], 'alive');
