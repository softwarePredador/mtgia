...
const _cases = [
  _CommanderRuntimeCase(
    ...
    prompt:
        'Commander Rakdos Prosper deck focused on exile value, treasure ramp, '
        'impulse draw, efficient interaction and resilient win conditions.',
    allowedColors: {'B', 'R'},
  ),
  _CommanderRuntimeCase(
    slug: 'edgar',
    ...
    prompt:
        'Commander Mardu Edgar Markov vampire typal deck with low curve '
        'vampires, anthem payoffs, token pressure, removal and finishers.',
    allowedColors: {'B', 'R', 'W'},
  ),
  _CommanderRuntimeCase(
    slug: 'aesi',
    ...
    prompt:
        'Commander Simic Aesi lands deck with ramp, extra land drops, card '
        'draw engines, landfall payoffs, interaction and big mana finishers.',
    allowedColors: {'G', 'U'},
  ),
];
...
void main() {
  ...
  testWidgets(
    'SM A135M runtime: Commander Reference app value generate/save/validate Prosper Edgar Aesi',
    (tester) async {
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_APP_VALUE_RUNTIME_BASE_URL ${ApiClient.baseUrl}',
      );
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_APP_VALUE_RUNTIME_HEALTH status=${healthResponse.statusCode} '
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_APP_VALUE_RUNTIME_ALL_SUMMARIES ${jsonEncode(summaries)}',
      );
    },
...
Future<_RuntimeCredentials> _registerByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_app_value_01_login',
  );
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_app_value_02_registered',
  );
...
Future<void> _loginByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_app_value_03_logged_in',
  );
  expectNoRawTechnicalErrorText(tester);
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_APP_VALUE_AUTH_SUMMARY registered=true login_ui=true',
  );
}
...
Future<Map<String, dynamic>> _generateSaveOpenAndValidate(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_app_value_${runtimeCase.slug}_01_prompt',
  );
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_APP_VALUE_GENERATE_INITIAL_FEEDBACK '
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_app_value_${runtimeCase.slug}_02_preview',
  );
  expect(find.text(runtimeCase.commanderName), findsWidgets);
  expectNoRawTechnicalErrorText(tester);

  final unique = DateTime.now().millisecondsSinceEpoch.toRadixString(16);
  final deckName = 'QA CmdRef ${runtimeCase.slug} $unique';
  await tester.enterText(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_app_value_${runtimeCase.slug}_03_details',
  );
  expectNoRawTechnicalErrorText(tester);

  final summary = await _validateSavedDeck(api, deckId!, runtimeCase);
  // ignore: avoid_print
  print('COMMANDER_REFERENCE_APP_VALUE_RUNTIME_SUMMARY ${jsonEncode(summary)}');
...
Future<Map<String, dynamic>> _validateSavedDeck(
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_APP_VALUE_VALIDATE_SUMMARY ${jsonEncode(summary)}',
  );
