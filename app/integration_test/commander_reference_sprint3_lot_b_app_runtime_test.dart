...
class _LotBCommanderCase {
  const _LotBCommanderCase({
    required this.slug,
...
const _cases = [
  _LotBCommanderCase(
    ...
    prompt:
        'Commander mono blue artifacts deck with construct pressure, artifact '
        'synergy, mana rocks, interaction, card selection and resilient finishers.',
    allowedColors: {'U'},
    archetype: 'artifacts_control',
  ),
  _LotBCommanderCase(
    slug: 'meren',
    commanderName: 'Meren of Clan Nel Toth',
    prompt:
        'Commander Golgari graveyard recursion deck with sacrifice outlets, '
        'toolbox creatures, self mill, removal, ramp and attrition finishers.',
    allowedColors: {'B', 'G'},
    archetype: 'graveyard_recursion',
  ),
];
...
void main() {
  ...
  testWidgets(
    'SM A135M runtime: Sprint 3 Lote B Commander Reference Urza and Meren',
    (tester) async {
      ...
      // ignore: avoid_print
      print('COMMANDER_REFERENCE_LOT_B_APP_BASE_URL ${ApiClient.baseUrl}');
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_LOT_B_APP_HEALTH status=${healthResponse.statusCode} '
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_LOT_B_APP_ALL_SUMMARIES ${jsonEncode(summaries)}',
      );
    },
...
Future<_RuntimeCredentials> _registerByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_b_01_login',
  );
  ...
  final username = 'qa_cmdref_lot_b_app_$unique';
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_b_02_registered',
  );
...
Future<void> _loginByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_b_03_logged_in',
  );
  expectNoRawTechnicalErrorText(tester);
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_LOT_B_APP_AUTH_SUMMARY register_ui=true login_ui=true',
  );
}
...
Future<Map<String, dynamic>> _generateSaveOpenAndValidate(
  ...
  WidgetTester tester,
  ApiClient api,
  _LotBCommanderCase runtimeCase,
) async {
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_LOT_B_APP_GENERATE_INITIAL_FEEDBACK '
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_b_${runtimeCase.slug}_02_preview',
  );
  expect(find.text(runtimeCase.commanderName), findsWidgets);
  expectNoRawTechnicalErrorText(tester);

  final unique = DateTime.now().millisecondsSinceEpoch.toRadixString(16);
  final deckName = 'QA LotB ${runtimeCase.slug} $unique';
  await tester.enterText(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_b_${runtimeCase.slug}_03_details',
  );
  expectNoRawTechnicalErrorText(tester);

  final summary = await _validateSavedDeck(api, deckId!, runtimeCase);
  // ignore: avoid_print
  print('COMMANDER_REFERENCE_LOT_B_APP_RUNTIME_SUMMARY ${jsonEncode(summary)}');
...
Future<void> _openGenerateScreen(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_b_${runtimeCase.slug}_01_generate_screen',
  );
}
...
Future<Map<String, dynamic>> _validateSavedDeck(
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_LOT_B_APP_VALIDATE_SUMMARY ${jsonEncode(summary)}',
  );
