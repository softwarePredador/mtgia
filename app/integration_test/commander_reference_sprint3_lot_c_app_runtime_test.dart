...
class _LotCCommanderCase {
  const _LotCCommanderCase({
    required this.slug,
    required this.commanderName,
    required this.prompt,
    required this.allowedColors,
    required this.archetype,
    required this.promotedOnBackend,
  });
...
const _cases = [
  _LotCCommanderCase(
    ...
    prompt:
        'Commander Azorius blink ETB value deck with efficient creatures, '
        'flicker engines, interaction, card advantage, ramp and resilient '
        'finishers without hard stax as the default plan.',
    allowedColors: {'W', 'U'},
    archetype: 'azorius_blink_etb',
    promotedOnBackend: true,
  ),
  _LotCCommanderCase(
    slug: 'purphoros',
    commanderName: 'Purphoros, God of the Forge',
    prompt:
        'Commander mono red token payoff burn deck with repeatable token makers, '
        'impact damage, sacrifice utility, haste, interaction, card flow and '
        'finishers without relying on Goblin typal as the main shell.',
    allowedColors: {'R'},
    archetype: 'mono_red_tokens_burn',
    promotedOnBackend: false,
  ),
];
...
void main() {
  ...
  testWidgets(
    'SM A135M runtime: Sprint 3 Lote C Commander Reference Brago plus Purphoros adjunct',
    (tester) async {
      ...
      // ignore: avoid_print
      print('COMMANDER_REFERENCE_LOT_C_APP_BASE_URL ${ApiClient.baseUrl}');
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_LOT_C_APP_HEALTH status=${healthResponse.statusCode} '
      ...
      // ignore: avoid_print
      print(
        'COMMANDER_REFERENCE_LOT_C_APP_ALL_SUMMARIES ${jsonEncode(summaries)}',
      );
    },
...
Future<_RuntimeCredentials> _registerByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_c_01_login',
  );
  ...
  final username = 'runtime_lot_c_app_$unique';
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_c_02_registered',
  );
...
Future<void> _loginByUi(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_c_03_logged_in',
  );
  expectNoRawTechnicalErrorText(tester);
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_LOT_C_APP_AUTH_SUMMARY register_ui=true login_ui=true',
  );
}
...
Future<Map<String, dynamic>> _generateSaveOpenAndValidate(
  ...
  WidgetTester tester,
  ApiClient api,
  _LotCCommanderCase runtimeCase,
) async {
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_LOT_C_APP_GENERATE_INITIAL_FEEDBACK '
    'commander="${runtimeCase.commanderName}" '
    'archetype=${runtimeCase.archetype} '
    'promoted_on_backend=${runtimeCase.promotedOnBackend} '
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_c_${runtimeCase.slug}_02_preview',
  );
  expect(find.text(runtimeCase.commanderName), findsWidgets);
  expectNoRawTechnicalErrorText(tester);

  final unique = DateTime.now().millisecondsSinceEpoch.toRadixString(16);
  final deckName = 'QA LotC ${runtimeCase.slug} $unique';
  await tester.enterText(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_c_${runtimeCase.slug}_03_details',
  );
  expectNoRawTechnicalErrorText(tester);

  final summary = await _validateSavedDeck(api, deckId!, runtimeCase);
  // ignore: avoid_print
  print('COMMANDER_REFERENCE_LOT_C_APP_RUNTIME_SUMMARY ${jsonEncode(summary)}');
...
Future<void> _openGenerateScreen(
  ...
  await captureVisualProof(
    binding,
    tester,
    'commander_reference_lot_c_${runtimeCase.slug}_01_generate_screen',
  );
}
...
Future<Map<String, dynamic>> _validateSavedDeck(
  ...
  // ignore: avoid_print
  print(
    'COMMANDER_REFERENCE_LOT_C_APP_VALIDATE_SUMMARY ${jsonEncode(summary)}',
  );
