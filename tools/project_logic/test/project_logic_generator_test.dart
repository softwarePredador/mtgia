...
void main() {
  ...
  test('indexes the executable product surfaces', () {
    ...
    expect(stats['database_tables'], 80);
    expect(stats['database_views'], 6);
    expect(stats['migrations'], 63);
  ...
  test('includes integration tests in inventory and source digest lineage', () {
    ...
    expect(
      digestInputs.where((path) => path == 'tools/project_logic/pubspec.lock'),
      hasLength(1),
    );
...
''',
  ...
  test('keeps PostgreSQL canonical and Hermes non-canonical', () {
    ...
    expect(database['latest_migration'], '065');
  });
  ...
  test('dependency inventory rejects missing resolution metadata', () async {
    final temp = Directory.systemTemp.createTempSync(
      'manaloom_logic_missing_metadata_',
    );
    addTearDown(() => temp.deleteSync(recursive: true));
    _writeResolvedWorkspaceFixture(temp, writeMetadata: false);

    await expectLater(
      ProjectLogicGenerator(temp).dependencyInventoryForTesting(),
      throwsA(
        isA<ProjectLogicException>().having(
          (error) => error.message,
          'message',
  ...
  test(
    'dependency inventory requires coherent metadata and ignores tool caches',
    () async {
      final temp = Directory.systemTemp.createTempSync(
        'manaloom_logic_dependencies_',
      );
      addTearDown(() => temp.deleteSync(recursive: true));
      _writeResolvedWorkspaceFixture(temp);
      _writePackageFixture(temp, 'tools/local_tool', 'local_tool_fixture');
  ...
  test('package metadata validation is fail-closed for malformed inputs', () {
    void expectMutationRejected(
      String label,
      void Function(Directory root) mutate,
    ) {
      final temp = Directory.systemTemp.createTempSync(
        'manaloom_logic_metadata_negative_',
      );
      addTearDown(() => temp.deleteSync(recursive: true));
      _writeResolvedWorkspaceFixture(temp);
      mutate(temp);
      expect(
        () => ProjectLogicGenerator(
          temp,
        ).validateWorkspacePackageMetadataForTesting(),
        throwsA(isA<ProjectLogicException>()),
        reason: label,
      );
    }

    expectMutationRejected('missing graph', (fixture) {
      File(
        p.join(fixture.path, 'app/.dart_tool/package_graph.json'),
      ).deleteSync();
    });
    if (!Platform.isWindows) {
      expectMutationRejected('linked package config', (fixture) {
        final config = File(
          p.join(fixture.path, 'app/.dart_tool/package_config.json'),
        );
        final target = File('${config.path}.target');
        config.renameSync(target.path);
        Link(config.path).createSync(p.basename(target.path));
      });
    }
    expectMutationRejected('malformed graph', (fixture) {
      File(
        p.join(fixture.path, 'server/.dart_tool/package_graph.json'),
      ).writeAsStringSync('{');
    });
    expectMutationRejected('unsupported graph config version', (fixture) {
      final graph = File(
        p.join(fixture.path, 'app/.dart_tool/package_graph.json'),
      );
      final payload = jsonDecode(graph.readAsStringSync());
      payload['configVersion'] = 2;
      graph.writeAsStringSync(jsonEncode(payload));
    });
    expectMutationRejected('divergent root URI', (fixture) {
      final config = File(
        p.join(fixture.path, 'app/.dart_tool/package_config.json'),
      );
      final payload = jsonDecode(config.readAsStringSync());
      payload['packages'][0]['rootUri'] = '../../server/';
      config.writeAsStringSync(jsonEncode(payload));
    });
    expectMutationRejected('different config and graph sets', (fixture) {
      final graph = File(
        p.join(fixture.path, 'server/.dart_tool/package_graph.json'),
      );
      final payload = jsonDecode(graph.readAsStringSync());
      payload['packages'].add({
        'name': 'unlocked',
        'version': '1.0.0',
        'dependencies': <String>[],
      });
    ...
    expectMutationRejected('unresolved graph dependency', (fixture) {
      final graph = File(p.join(fixture.path, '.dart_tool/package_graph.json'));
      final payload = jsonDecode(graph.readAsStringSync());
      payload['packages'][0]['dependencies'] = ['absent'];
      graph.writeAsStringSync(jsonEncode(payload));
    });
    expectMutationRejected('incomplete graph dependency inventory', (fixture) {
      final graph = File(
        p.join(fixture.path, 'app/.dart_tool/package_graph.json'),
      );
      final payload = jsonDecode(graph.readAsStringSync());
      payload['packages'][0].remove('dependencies');
      graph.writeAsStringSync(jsonEncode(payload));
    });
  });

  test(
    'canonicalizes logical and physical workspace file URIs identically',
    () {
      final logicalRoot = Directory(
        '/tmp',
      ).createTempSync('manaloom_logic_uri_alias_');
      addTearDown(() => logicalRoot.deleteSync(recursive: true));
      final source = File(p.join(logicalRoot.path, 'app/lib/example.dart'))
        ..createSync(recursive: true)
        ..writeAsStringSync('void example() {}\n');
      final physicalRoot = Directory(logicalRoot.resolveSymbolicLinksSync());
      final physicalSource = File(source.resolveSymbolicLinksSync());
      final generator = ProjectLogicGenerator(logicalRoot);

      expect(
        generator.canonicalLibraryUriForTesting(source.uri.toString()),
        'workspace:app/lib/example.dart',
      );
      expect(
        generator.canonicalLibraryUriForTesting(physicalSource.uri.toString()),
        'workspace:app/lib/example.dart',
      );
      expect(generator.root.path, physicalRoot.path);
    },
  );

  test('rejects ambiguous, non-local, and escaping semantic file URIs', () {
    final temp = Directory.systemTemp.createTempSync('manaloom_logic_uri_');
    final external = Directory.systemTemp.createTempSync(
      'manaloom_logic_uri_external_',
    );
    addTearDown(() {
      temp.deleteSync(recursive: true);
      external.deleteSync(recursive: true);
    });
    final source = File(p.join(temp.path, 'lib/example.dart'))
      ..createSync(recursive: true)
      ..writeAsStringSync('void example() {}\n');
    final outside = File(p.join(external.path, 'outside.dart'))
      ..writeAsStringSync('void outside() {}\n');
    final broken = Link(p.join(temp.path, 'lib/broken.dart'))
      ..createSync('missing.dart');
    final loop = Link(p.join(temp.path, 'lib/loop.dart'))
      ..createSync('loop.dart');
    final escape = Link(p.join(temp.path, 'lib/escape.dart'))
    ...
    final invalid = <String>[
      ...
      '${source.uri}#fragment',
      'file:relative.dart',
      'file://remotehost/tmp/example.dart',
      'file:///tmp/%ZZ.dart',
      'file:///tmp/a%2Fb.dart',
      'file:///tmp/a%5Cb.dart',
      'file:///tmp/a%252Fb.dart',
      broken.uri.toString(),
      loop.uri.toString(),
      escape.uri.toString(),
    ];
    for (final uri in invalid) {
      expect(
        () => generator.canonicalLibraryUriForTesting(uri),
        throwsA(isA<ProjectLogicException>()),
        reason: uri,
      );
    }
  ...
  test(
    'semantic ProjectLogicException remains terminal before diagnostics',
    () {
      final source = File(
        p.join(
          root.path,
      ...
      final semanticStart = source.indexOf(
        'Future<Map<String, Object?>> _semanticAnalysis',
      );
      final terminalCatch = source.indexOf(
        'on ProjectLogicException {',
        semanticStart,
      );
      final diagnosticCatch = source.indexOf(
        'on Object catch (error)',
        semanticStart,
      );

      expect(semanticStart, greaterThanOrEqualTo(0));
      expect(terminalCatch, greaterThan(semanticStart));
      expect(diagnosticCatch, greaterThan(terminalCatch));
      expect(
        source.substring(terminalCatch, diagnosticCatch),
        contains('rethrow;'),
      );
      expect(
        source.indexOf('_validateWorkspacePackageMetadata();'),
        lessThan(
          source.indexOf('final semanticAnalysis = await _semanticAnalysis'),
        ),
      );
    },
  );
...
void _writeResolvedWorkspaceFixture(
  Directory root, {
  bool writeMetadata = true,
}) {
  _writePackageFixture(
    root,
    '.',
    'workspace_fixture',
...
''');
  ...
  final taskCache = Platform.environment['PUB_CACHE'];
  if (taskCache == null || taskCache.isEmpty) {
    throw StateError('PUB_CACHE is required by the project logic tests.');
  }
  final metadata = Directory(p.join(directory.path, '.dart_tool'))
    ..createSync(recursive: true);
  File(p.join(metadata.path, 'package_config.json')).writeAsStringSync(
    jsonEncode({
      'configVersion': 2,
      'packages': [
        {
      ...
      'pubCache': Directory(taskCache).absolute.uri.toString(),
    }),
  );
  File(p.join(metadata.path, 'package_graph.json')).writeAsStringSync(
    jsonEncode({
      'configVersion': 1,
      'roots': [name],
      'packages': [
