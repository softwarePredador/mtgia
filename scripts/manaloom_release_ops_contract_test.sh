...
grep -Fq 'MANALOOM_PROXY_TOPOLOGY' \
  "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq 'len({proxy_ip, backend_ip, transport_peer_ip}) == 3' \
  "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
if grep -Fq 'MANALOOM_PRODUCTION_TRAEFIK_LOGICAL_IP' \
  "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"; then
  echo "deploy voltou a fixar o endereco efemero da tarefa Traefik" >&2
  exit 1
fi
...
grep -Fq -- '--flutter-engine-revision "$MANALOOM_RELEASE_FLUTTER_ENGINE_REVISION"' "$ROOT_DIR/scripts/manaloom_build_android_release.sh"
grep -Fq 'manaloom:gradle-aab-dependency-parity' "$ROOT_DIR/scripts/manaloom_publish_android_release.sh"
grep -Fq 'manaloom:flutter-native-release-component-count' "$ROOT_DIR/scripts/manaloom_publish_android_release.sh"
grep -Fq 'android-release-native-runtime' "$ROOT_DIR/scripts/manaloom_publish_android_release.sh"
...
grep -Fq "('052', 'version_battle_simulation_attempts')" "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq "('053', 'create_battle_replay_annotations')" "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq "('054', 'create_battle_jobs')" "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq "('055', 'create_battle_job_live_records')" "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq "('056', 'create_interactive_battle_sessions')" "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
...
grep -Fq 'migrations_041_057_ready' "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq '.checks.release_schema.status == "healthy"' "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq '.checks.release_schema.required_range == "038-057"' "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
grep -Fq '.checks.release_schema.latest_migration == "057"' "$ROOT_DIR/scripts/manaloom_deploy_backend_image.sh"
