#!/usr/bin/env python3
"""Tests for engine cut trace/replacement gate."""
...
import global_commander_engine_cut_trace_replacement_gate as gate
...
def create_db(path: Path) -> None:
    with sqlite3.connect(path) as conn:
        ...
        conn.execute("INSERT INTO deck_cards(deck_id, card_name) VALUES (619, 'Existing Engine')")
        ...
        "replacement_search_rows": [
            {
                ...
                "deck_id": "619",
                "commander": "Kaalia of the Vast",
                "required_replacement_roles": ["engine"],
            }
        ],
        "cut_followup_rows": [],
    }
...
class GlobalCommanderEngineCutTraceReplacementGateTests(unittest.TestCase):
    def test_replacement_candidates_filter_color_and_current_deck(self) -> None:
        ...
        self.assertNotIn("Rhystic Study", names)
        self.assertNotIn("Existing Engine", names)
    ...
    def test_build_report_keeps_candidate_copy_closed(self) -> None:
        ...
        self.assertEqual(report["summary"]["replacement_candidate_count"], 1)
        self.assertEqual(report["summary"]["next_gate"], "run_forced_access_trace_for_unexposed_engine_cut")
        self.assertIn("candidate_copy_closed_after_trace_replacement_gate", report["candidate_copy_blockers"])
