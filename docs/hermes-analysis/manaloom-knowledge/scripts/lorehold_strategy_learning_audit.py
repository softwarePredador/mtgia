...
DEFAULT_SQUEE_GATES = [
    REPORT_DIR / "lorehold_squee_hashseed0_isolated_cached_timeout_gate_20260627_v3.json",
]
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.append("- The strongest proven direction is not a generic big-spell upgrade; it is improving the 607 shell by cutting the expensive `Insurrection` slot for `Squee, Goblin Nabob`.")
    lines.append("- Decisive gate evidence now uses `PYTHONHASHSEED=0`, `deck_process_isolation=true`, per-game timeout, and the optimized battle-rule lookup cache; baseline/candidate-only reproductions match the comparative gate exactly.")
    lines.append("- New zone-trace evidence proves `Squee` can be cast, move to graveyard, and return during games, not only in a unit test. The clean gate has `squee_to_graveyard=7`, `squee_upkeep_return=5`, `squee_return_after_known_graveyard_entry=5`, and `squee_return_without_known_graveyard_entry=0`.")
    lines.append("- Proven Squee routes in this gate are battlefield-to-graveyard through combat/wipes plus one opponent mill (`Brain Freeze`).")
