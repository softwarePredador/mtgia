#!/usr/bin/env python3
"""Run trace/replacement evidence for engine cut follow-up gates.

This read-only gate consumes ``global_commander_engine_cut_followup_router``.
It runs natural current-scope replay traces for trace-required cuts and mines
local Commander staple/oracle evidence for same-lane engine replacements for
...
DEFAULT_ROUTER_REPORT = REPORT_DIR / "global_commander_engine_cut_followup_router_20260706_current.json"
...
DEFAULT_REPLAY_DIR = REPORT_DIR / "global_commander_engine_cut_trace_replays_20260706_current"
DEFAULT_OUT_PREFIX = REPORT_DIR / "global_commander_engine_cut_trace_replacement_gate_20260706_current"
...
def deck_scope(router_payload: Mapping[str, Any]) -> tuple[str, str]:
    for key in ("trace_plan_rows", "replacement_search_rows", "cut_followup_rows"):
...
def db_path_from_policy(engine_policy_payload: Mapping[str, Any]) -> Path:
...
def classify_trace(card: str, summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if usage > 0:
        status = "engine_cut_natural_trace_usage_observed_blocks_cut"
        next_gate = "find_different_engine_cut_or_same_lane_replacement_before_candidate_copy"
    elif exposure > 0 or decisions > 0:
        status = "engine_cut_natural_trace_seen_without_usage_needs_manual_negative_review"
        next_gate = "manual_negative_trace_review_for_engine_cut_before_candidate_copy"
    else:
        status = "engine_cut_natural_trace_no_target_exposure_needs_force_access"
        next_gate = "run_forced_access_trace_for_unexposed_engine_cut"
    return {
        "card_name": card,
        "status": status,
...
def run_natural_traces(
    *,
    ...
    for seed in range(seed_start, seed_start + max(1, seed_count)):
        ...
        status = "engine_cut_natural_replay_generated" if run["returncode"] == 0 else "engine_cut_natural_replay_failed"
        trace_summary = (
            summarize_trace_files(
...
def role_signals(type_line: str, oracle_text: str) -> list[str]:
    text = normalize_name(f"{type_line} {oracle_text}")
    signals = []
    if "whenever" in text and ("draw a card" in text or "draw cards" in text):
        signals.append("repeatable_draw_engine")
    if "whenever" in text and ("create a treasure" in text or "create that many treasure" in text):
        signals.append("treasure_engine")
    if "whenever" in text and ("create a token" in text or "create a food" in text):
        signals.append("token_engine")
    if "artifact" in text and ("create" in text or "draw" in text or "treasure" in text):
        signals.append("artifact_engine_overlap")
    if "additional combat" in text or "double strike" in text:
        signals.append("combat_engine")
    if "search your library" in text and ("artifact" in text or "enchantment" in text or "equipment" in text):
        signals.append("engine_access_tutor")
    if "at the beginning of your upkeep" in text and "draw a card" in text:
        signals.append("slow_draw_engine")
    return sorted(set(signals))


def replacement_candidate_status(signals: list[str]) -> str:
    if "artifact_engine_overlap" in signals and ("token_engine" in signals or "treasure_engine" in signals):
        return "same_lane_engine_candidate_needs_source_trace_review"
    if signals:
        return "adjacent_engine_candidate_needs_explicit_same_lane_proof"
    return "not_same_lane_engine_candidate"


def query_replacement_candidates(
    *,
    db_path: Path,
    deck_id: str,
    limit: int,
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for row in conn.execute(sql, (deck_id,)):
            ...
            if not key or key in seen:
                continue
            color_value = row["oracle_color_identity"] or row["staple_color_identity"]
            if not color_identity_allowed(color_value):
                continue
            signals = role_signals(str(row["type_line"] or ""), str(row["oracle_text"] or ""))
            status = replacement_candidate_status(signals)
            if status == "not_same_lane_engine_candidate":
                continue
            seen.add(key)
            candidates.append(
                {
...
def build_replacement_reviews(
    *,
    ...
    reviews = []
    strong = [row for row in candidates if row.get("status") == "same_lane_engine_candidate_needs_source_trace_review"]
    adjacent = [row for row in candidates if row.get("status") == "adjacent_engine_candidate_needs_explicit_same_lane_proof"]
    for row in replacement_rows:
        cut = str(row.get("card_name") or "")
        if strong:
            status = "engine_replacement_candidates_found_needs_source_trace_review"
            next_gate = "review_engine_replacement_candidates_before_candidate_copy"
        elif adjacent:
            status = "adjacent_engine_candidates_found_needs_explicit_same_lane_proof"
            next_gate = "prove_adjacent_engine_candidate_is_same_lane_before_candidate_copy"
        else:
            status = "no_local_engine_replacement_candidate_found"
...
def build_report(
    *,
    ...
    db_path = db_path_from_policy(engine_payload)
    ...
    generated_replays = sum(1 for row in seed_rows if row.get("status") == "engine_cut_natural_replay_generated")
    trace_no_exposure = [row["card_name"] for row in trace_reviews if row["status"] == "engine_cut_natural_trace_no_target_exposure_needs_force_access"]
    trace_usage = [row["card_name"] for row in trace_reviews if row["status"] == "engine_cut_natural_trace_usage_observed_blocks_cut"]
    trace_manual = [
        row["card_name"]
        for row in trace_reviews
        if row["status"] == "engine_cut_natural_trace_seen_without_usage_needs_manual_negative_review"
    ]
    replacement_ready_for_review = [
        row["cut_card"]
        for row in replacement_reviews
        if row["status"] == "engine_replacement_candidates_found_needs_source_trace_review"
    ]
    blockers = []
    ...
    blockers.append("candidate_copy_closed_after_trace_replacement_gate")
    if trace_no_exposure:
        status = "engine_cut_trace_replacement_gate_needs_forced_access"
        next_gate = "run_forced_access_trace_for_unexposed_engine_cut"
    elif trace_usage or trace_manual:
    ...
    elif replacement_ready_for_review:
        status = "engine_cut_trace_replacement_gate_found_replacement_candidates"
        next_gate = "review_engine_replacement_candidates_before_candidate_copy"
    else:
        status = "engine_cut_trace_replacement_gate_needs_more_external_source"
        next_gate = "expand_external_engine_replacement_source_lanes"
    return {
        "generated_at": utc_now(),
        "status": status,
        "artifact_type": "global_commander_engine_cut_trace_replacement_gate",
        "postgres_writes": False,
        "source_db_mutated": False,
        "battle_replay_performed": generated_replays > 0,
        ...
        "summary": {
            ...
            "replacement_cut_count": len(replacement_reviews),
            "replacement_candidate_count": len(candidates),
            "strong_replacement_candidate_count": sum(
                1 for row in candidates if row.get("status") == "same_lane_engine_candidate_needs_source_trace_review"
            ),
            "adjacent_replacement_candidate_count": sum(
                1 for row in candidates if row.get("status") == "adjacent_engine_candidate_needs_explicit_same_lane_proof"
            ),
        ...
        "replacement_review_rows": replacement_reviews,
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    ...
    for row in payload["replacement_review_rows"]:
        lines.append(
    ...
    if not payload["replacement_review_rows"]:
        lines.append("| none |  |  |  |  |")
    lines.extend(["", "## Replacement Candidate Sample", ""])
    if payload["replacement_candidate_rows"]:
