...
def load_profile_repair_land_cut_review_pairs(
    ...
    *,
    ...
    payload = read_json(review_report)
    if payload.get("artifact_type") != "global_commander_profile_repair_land_cut_reviewer":
        raise RuntimeError(f"{review_report} is not a profile repair land-cut reviewer report")
    if not payload.get("candidate_copy_allowed_now"):
        raise RuntimeError(f"profile repair land-cut candidate copy is not allowed by {review_report}")
    summary = payload.get("summary") or {}
    if deck_id and str(summary.get("deck_id")) != str(deck_id):
        raise RuntimeError(f"profile repair land-cut deck_id mismatch: expected {deck_id}, got {summary.get('deck_id')}")

    pairs: list[dict[str, Any]] = []
    for pair in payload.get("materialization_pairs") or []:
        if not isinstance(pair, Mapping):
            continue
        pair_add = str(pair.get("add") or "")
        pair_cut = str(pair.get("cut") or "")
        ...
        pairs.append(
            {
                ...
                "add": pair_add,
                "cut": pair_cut,
                "pair": dict(pair.get("pair") or pair),
                "candidate": dict(pair.get("candidate") or {}),
                "cut_candidate": dict(pair.get("cut_candidate") or {}),
                "source_pool_status": payload.get("status"),
            }
        )
    if not pairs:
        raise RuntimeError(f"no matching profile repair land-cut pairs found in {review_report}")
    ...
    return {
        ...
        "pairs": pairs,
        "source_pool_status": payload.get("status"),
        "source_report_db": expected_source_db(payload, review_report),
        "blocked_cut_candidates": [],
        "source_artifact_type": payload.get("artifact_type"),
        "next_gate": summary.get("next_gate"),
    }
