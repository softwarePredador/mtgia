#!/usr/bin/env python3
"""Route follow-up gates for engine cut usage/same-lane blockers.

This read-only gate consumes
``global_commander_engine_cut_usage_same_lane_proof_scout`` output and turns
its blockers into explicit next actions. It does not run battles, copy decks,
mutate databases, force traces, or promote packages.
"""
...
DEFAULT_SCOUT_REPORT = (
    REPORT_DIR / "global_commander_engine_cut_usage_same_lane_proof_scout_20260706_current.json"
)
DEFAULT_OUT_PREFIX = REPORT_DIR / "global_commander_engine_cut_followup_router_20260706_current"
...
def cut_followup_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if status == "engine_cut_usage_observed_blocks_candidate_copy":
        route_kind = "replacement_required"
        next_gate = "find_different_engine_cut_or_explicit_same_lane_replacement_before_candidate_copy"
        reason = "current_scope_usage_observed"
        required_evidence = "different_cut_source_or_explicit_same_lane_replacement_proof"
    elif status == "engine_cut_missing_current_scope_usage_trace":
    ...
    elif status == "engine_cut_seen_without_usage_needs_negative_review":
        route_kind = "negative_trace_review_required"
        next_gate = "manual_negative_trace_review_for_engine_cut_before_pair_review"
        reason = "seen_without_usage_needs_manual_review"
        required_evidence = "manual_negative_trace_review"
    elif status == "engine_cut_text_trace_candidate_needs_structured_review":
        route_kind = "structured_trace_review_required"
        next_gate = "review_text_trace_candidate_before_pair_review"
        ...
        next_gate = "manual_review_unclassified_engine_cut_status"
        ...
        rows.append(
            {
                ...
                "primary_route": "generate_or_import_current_scope_usage_trace_for_engine_cut",
        ...
        rows.append(
            {
                ...
                "allowed_recovery_routes": [
                    "find_different_engine_cut_with_current_scope_negative_evidence",
                    "prove_explicit_same_lane_replacement_before_candidate_copy",
                ],
                "blocked_recovery_routes": [
                    "cross_lane_removal_add_for_engine_cut_without_same_lane_route",
...
def build_report(*, scout_report: Path) -> dict[str, Any]:
    ...
    pair_ready_count = sum(
        ...
        if row.get("pair_status") == "engine_cut_pair_ready_for_manual_candidate_copy_review"
    )
    ...
    if pair_ready_count:
        status = "engine_cut_followup_router_ready_for_manual_pair_review"
        next_gate = "manual_review_engine_cut_pair_before_candidate_copy"
    elif trace_plan_rows and replacement_rows:
    ...
    elif trace_plan_rows:
        status = "engine_cut_followup_router_blocks_candidate_copy"
        next_gate = "run_current_scope_trace_plan_before_candidate_copy"
    elif replacement_rows:
        status = "engine_cut_followup_router_blocks_candidate_copy"
        next_gate = "find_different_engine_cut_or_same_lane_replacement_before_candidate_copy"
    else:
        status = "engine_cut_followup_router_needs_manual_review"
        next_gate = "manual_review_engine_cut_followup_router"
    blockers: list[str] = []
    blockers.extend(str(item) for item in scout_payload.get("candidate_copy_blockers") or [])
    if trace_plan_rows:
        blockers.append(
            "trace_required_for_engine_cuts:" + ",".join(str(row.get("card_name") or "") for row in trace_plan_rows)
        )
    if replacement_rows:
        blockers.append(
    ...
    return {
        ...
        "status": status,
        "artifact_type": "global_commander_engine_cut_followup_router",
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    ...
    if payload["trace_plan_rows"]:
        for row in payload["trace_plan_rows"]:
            lines.append(
                f"- `{row['card_name']}`: `{row['primary_route']}`; fallback `{row['fallback_route']}`"
            )
    else:
        lines.append("- none")
