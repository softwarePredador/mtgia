...
import global_commander_ramp_alternative_cut_forced_access_trace_generator as generator
...
def alternative_trace_payload(root: Path) -> dict[str, object]:
    db = root / "knowledge.db"
    battle = root / "battle.py"
...
class GlobalCommanderRampAlternativeCutForcedAccessTraceGeneratorTests(unittest.TestCase):
    def test_forced_usage_blocks_alternative_cut(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trace = write_json(root, "trace.json", alternative_trace_payload(root))
            ...
            report = generator.build_report(
                alternative_trace_report=trace,
        ...
        self.assertEqual(report["status"], "ramp_alternative_cut_forced_access_blocks_used_targets")
    ...
    def test_forced_available_without_usage_needs_manual_review(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trace = write_json(root, "trace.json", alternative_trace_payload(root))
        ...
        self.assertEqual(report["status"], "ramp_alternative_cut_forced_access_needs_manual_negative_review")
        ...
        self.assertIn("candidate_copy_closed_after_alternative_ramp_cut_forced_access", report["candidate_copy_blockers"])
