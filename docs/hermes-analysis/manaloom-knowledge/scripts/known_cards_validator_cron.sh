...
legalities_report="$ARTIFACT_DIR/legalities_sync_validator_$(date -u +%Y%m%d_%H%M%S).json"
python3 "$SCRIPT_DIR/sync_pg_legalities.py" \
  --sqlite-db "$MANALOOM_KNOWLEDGE_DB" \
  --report "$legalities_report"
cp "$legalities_report" "$ARTIFACT_DIR/latest_legalities_sync_validator.json"
...
contract_prefix="$ARTIFACT_DIR/pg_hermes_sqlite_contract_audit_validator_$(date -u +%Y%m%d_%H%M%S)"
python3 "$SCRIPT_DIR/pg_hermes_sqlite_contract_audit.py" \
  --sqlite-db "$MANALOOM_KNOWLEDGE_DB" \
  --out-prefix "$contract_prefix"
cp "$contract_prefix.json" "$ARTIFACT_DIR/latest_pg_hermes_sqlite_contract_audit_validator.json"
cp "$contract_prefix.md" "$ARTIFACT_DIR/latest_pg_hermes_sqlite_contract_audit_validator.md"
