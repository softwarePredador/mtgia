...
def test_target_player_life_gain_runner_executes_target_player_gain() -> None:
    effect = {
        ...
        "_rule_logical_key": "battle_rule_v1:heroes-reunion",
    }
    rule = {
        "card_name": "Heroes' Reunion",
        "logical_rule_key": "battle_rule_v1:heroes-reunion",
        "required_effect_fields": effect,
    }
    ...
    assert result["card_name"] == "Heroes' Reunion"
    assert result["target_player"] == "Spell Controller"
    assert result["life_gained"] == 7
    assert result["life_after"] == 27
    assert any(
        event == "life_total_changed"
        and data.get("card") == "Heroes' Reunion"
        and data.get("target_player") == "Spell Controller"
        and data.get("requested_delta") == 7
        and data.get("life_after") == 27
        for event, data in events
    )
...
def test_target_player_mill_draw_runner_mills_and_draws() -> None:
    effect = {
        "effect": "composite_resolution",
        ...
        "_rule_logical_key": "battle_rule_v1:thought-scour",
    }
    rule = {
        "card_name": "Thought Scour",
        "logical_rule_key": "battle_rule_v1:thought-scour",
        "required_effect_fields": effect,
    }
    scenario = package_builder.target_player_mill_draw_execution_scenario_from_expected_rule(rule)
    ...
    battle.get_card_effect = lambda card: dict(effect)
    try:
        result = validator.run_target_player_mill_draw_spell(battle, scenario, events)
    finally:
        battle.REPLAY_EVENT_HANDLER = previous_handler
        battle.get_card_effect = previous_get_card_effect

    assert result["card_name"] == "Thought Scour"
    assert result["target_player"] == "Opponent"
    assert result["cards_milled"] == 2
    assert result["cards_drawn"] == 1
    assert result["resolution_order"] == "mill_then_draw"
    assert [
        data.get("component_effect")
        for event, data in events
        if event == "composite_rule_component_resolved"
        and data.get("card") == "Thought Scour"
...
def test_fixed_draw_discard_spell_runner_executes_discard_hand_then_draw() -> None:
    battle = validator.load_battle(validator.DEFAULT_BATTLE)
    events = []
    previous_handler = battle.REPLAY_EVENT_HANDLER
    previous_get_card_effect = battle.get_card_effect
    ...
    try:
        result = validator.run_fixed_draw_discard_spell(
            battle,
            {
                "name": "Dangerous Wager discards hand then draws",
                "type": "fixed_draw_discard_spell",
                "card": {"name": "Dangerous Wager", "type_line": "Instant"},
                "controller_library": [
                    {"name": f"E2E Dangerous Wager Library Card {index + 1}", "type_line": "Sorcery", "effect": "draw_cards"}
                    for index in range(2)
                ],
                "controller_hand": [
                    {"name": f"E2E Dangerous Wager Hand Card {index + 1}", "type_line": "Instant", "effect": "draw_cards"}
                    for index in range(3)
                ],
                "expected_draw_count": 2,
                "expected_discard_count": 3,
                "expected_discard_count_source": "controller_hand_size",
                "expected_discard_random": False,
                "expected_draw_discard_order": "discard_then_draw",
            },
            events,
        )
    finally:
    ...
    assert result["card_name"] == "Dangerous Wager"
    assert result["cards_drawn"] == 2
    assert result["cards_discarded"] == 3
    assert result["discard_random"] is False
    assert result["order"] == "discard_then_draw"
...
def test_simple_mana_source_runner_validates_life_gained_dynamic_any_one_color_mana() -> None:
    battle = validator.load_battle(validator.DEFAULT_BATTLE)
    events = []
    previous_handler = battle.REPLAY_EVENT_HANDLER
    previous_get_card_effect = battle.get_card_effect
    ...
    battle.get_card_effect = lambda card: {
        ...
        "_rule_logical_key": "battle_rule_v1:accomplished-alchemist",
    }
    try:
        result = validator.run_simple_mana_source_refresh(
            battle,
            {
                "name": "Accomplished Alchemist refreshes life-gained dynamic any-color mana source",
                "type": "simple_mana_source_refresh",
                "card": {
                ...
                "controller_life_gained_this_turn": 3,
                "expected_available_mana_after_refresh": 3,
                "expected_conditional_mana": 3,
                "expected_conditional_colors": ["black", "blue", "green", "red", "white"],
                "expected_tapped": True,
    ...
    assert result["card_name"] == "Accomplished Alchemist"
    assert result["available_mana"] == 3
    assert result["conditional_mana"] == 3
