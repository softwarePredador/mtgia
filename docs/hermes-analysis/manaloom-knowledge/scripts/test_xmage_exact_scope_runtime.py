...
class XMageExactScopeRuntimeTest(unittest.TestCase):
    ...
    def test_damage_target_create_treasure_gives_treasure_after_damage(self) -> None:
        active = self.battle.Player("Active", None, [])
        opponent = self.battle.Player("Opponent", None, [])
        target = {
            "name": "Target Brigand",
        ...
        effect = {
            ...
            "_rule_logical_key": "battle_rule_v1:fixture_damage_treasure",
        }

        self.battle.apply_direct_damage(
            active,
            [opponent],
            {"name": "Improvised Weaponry", "type_line": "Sorcery"},
            effect,
            turn=4,
        ...
        self.assertTrue(
            any(
                event == "treasure_created"
                and data.get("card") == "Improvised Weaponry"
                and data.get("trigger") == "on_resolution_after_damage"
                and data.get("treasures_created") == 1
                and data.get("treasure_recipient") == "controller"
                and data.get("rule_logical_key") == "battle_rule_v1:fixture_damage_treasure"
                for event, data in self.events
            )
        )
    ...
    def test_composite_keyword_draw_spell_grants_keyword_and_draws(self) -> None:
        ...
        card = {
            "name": "Fixture Bladebrand",
            "type_line": "Instant",
            "oracle_text": "Target creature gains deathtouch until end of turn. Draw a card.",
        }
        ...
        self.assertTrue(target["deathtouch"])
        self.assertEqual(target["keywords"], ["deathtouch"])
        self.assertEqual([card["name"] for card in active.hand], ["Fresh Card"])
        self.assertEqual([card["name"] for card in active.graveyard], ["Fixture Bladebrand"])
        self.assertTrue(
            any(
                event == "stat_modifier_until_eot_resolved"
                and data.get("card") == "Fixture Bladebrand"
                and data.get("target") == "Active Bear"
                and data.get("granted_keywords_until_eot") == ["deathtouch"]
    ...
    def test_simple_activated_damage_pays_life_cost_and_damages_player(self) -> None:
        active = self.battle.Player("Active", None, [])
        opponent = self.battle.Player("Opponent", None, [])
        active.life = 10
        opponent.life = 5
        ...
        self.assertTrue(
            any(
                event == "activated_ability"
                and data.get("card") == "Reckless Assault"
                and data.get("activation_kind") == "simple_activated_damage"
                and data.get("life_paid") == 2
                and data.get("life_before") == 10
                and data.get("life_after") == 8
                and data.get("rule_logical_key") == "battle_rule_v1:reckless_assault"
                for event, data in self.events
            )
        )

    def test_simple_activated_damage_blocks_when_life_cost_is_too_risky(self) -> None:
        active = self.battle.Player("Active", None, [])
        opponent = self.battle.Player("Opponent", None, [])
        active.life = 3
        opponent.life = 5
        ...
        self.assertFalse(activated)
        self.assertEqual(active.available_mana(), 1)
        self.assertEqual(active.life, 3)
        self.assertEqual(opponent.life, 5)
        self.assertFalse(any(event == "activated_ability" for event, _ in self.events))
    ...
    def test_natural_ramp_cast_resolves_etb_draw_and_pays_mana_activation_cost(self) -> None:
        active = self.battle.Player(
            "Active",
            None,
            [{"name": "Fresh Card", "type_line": "Instant"}],
        ...
        self.battle.HANDCRAFTED_KNOWN_CARD_RULES["Fixture Prism"] = {
            "effect": "ramp_permanent",
            "battle_model_scope": "xmage_simple_mana_source_with_etb_draw_v1",
            "is_mana_source": True,
            "mana_produced": 1,
        ...
        self.battle.HANDCRAFTED_KNOWN_CARDS.add("Fixture Prism")

        cast = self.battle.cast_spells_v8(
            active,
            [opponent],
            [active, opponent],
            turn=4,
    ...
    def test_counter_target_spell_targeting_creature_constraint(self) -> None:
        ...
        counter_effect = {
            "effect": "counter",
            "battle_model_scope": "xmage_counter_target_and_draw_card_spell_v1",
            "target": "spell_targeting_creature",
            "target_constraints": {
                "zone": "stack",
                "stack_object": "spell",
        ...
        target_spell = {"name": "Target Trick", "type_line": "Instant", "cmc": 2}
        legal_stack = self.battle.StackItem(
            target_spell,
            active,
            {
                "effect": "target_spell",
        ...
        illegal_stack = self.battle.StackItem(
            target_spell,
            active,
            {
                "effect": "target_spell",
                "targets": [{"name": "Active", "target_type": "player"}],
            },
        )
    ...
    def test_counter_target_spell_targeting_controlled_permanent_constraint(self) -> None:
        active = self.battle.Player("Active", None, [])
        responder = self.battle.Player("Responder", None, [])
        counter_effect = {
            "effect": "counter",
        ...
        target_spell = {"name": "Target Removal", "type_line": "Instant", "cmc": 2}
        legal_stack = self.battle.StackItem(
            target_spell,
            active,
            {
    ...
    def test_counter_target_spell_cast_from_graveyard_constraint(self) -> None:
        active = self.battle.Player("Active", None, [])
        responder = self.battle.Player("Responder", None, [])
        counter_effect = {
            "effect": "counter",
            "battle_model_scope": "xmage_counter_target_and_draw_card_spell_v1",
            "target": "spell_cast_from_graveyard",
            "target_constraints": {
                "zone": "stack",
                "stack_object": "spell",
        ...
        target_spell = {"name": "Target Flashback", "type_line": "Sorcery", "cmc": 2}
        graveyard_stack = self.battle.StackItem(
            target_spell,
            active,
            {"effect": "draw_cards", "source_zone": "graveyard"},
        )
        hand_stack = self.battle.StackItem(
            target_spell,
            active,
            {"effect": "draw_cards", "source_zone": "hand"},
        )
    ...
    def test_counter_unless_pays_exiles_countered_spell_when_tax_unpaid(self) -> None:
        ...
        self.assertTrue(stack.items[-1].countered)
        self.assertTrue(target_spell.get("_exile_on_resolution"))
        self.assertTrue(
            any(
                event == "spell_countered"
                and data.get("counter") == "Fixture No More Lies"
                and data.get("result") == "countered"
                and data.get("countered_spell_to_exile") is True
                and data.get("counter_tax_paid") is False
                for event, data in self.events
            )
        )
        ...
        self.assertEqual([card["name"] for card in active.exile], ["Target Finisher"])
        self.assertEqual(active.graveyard, [])
        self.assertTrue(
            any(
                event == "countered_spell_moved_to_exile"
                and data.get("card") == "Target Finisher"
                and data.get("replacement_reason") == "counter_unless_pays_exile_replacement"
                for event, data in self.events
            )
        )

    def test_counter_unless_pays_exile_replacement_does_not_apply_when_tax_paid(self) -> None:
        active = self.battle.Player("Active", None, [])
        active.mana_pool.add_generic(3)
        responder = self.battle.Player("Responder", None, [])
        responder.mana_pool.add("white", 1)
        ...
        self.assertFalse(stack.items[-1].countered)
        self.assertFalse(target_spell.get("_exile_on_resolution", False))
        ...
        self.assertTrue(
            any(
                ...
                and data.get("counter") == "Fixture No More Lies"
                and data.get("result") == "not_countered_tax_paid"
                and data.get("countered_spell_to_exile") is False
                and data.get("counter_tax_paid") is True
                for event, data in self.events
            )
        )
    ...
    def test_multi_target_damage_each_target_deals_full_amount_to_each_legal_target(self) -> None:
        active = self.battle.Player("Active", None, [])
        opponent = self.battle.Player("Opponent", None, [])
        illegal_target = {
            "name": "Plain Land",
        ...
        self.assertEqual([card["name"] for card in opponent.battlefield], ["Plain Land", "First Creature", "Second Creature"])
        self.assertEqual(illegal_target.get("damage_marked_this_turn"), None)
        self.assertEqual(target_one.get("damage_marked_this_turn", 0), 2)
        self.assertEqual(target_two.get("damage_marked_this_turn", 0), 2)
        self.assertTrue(
            any(
                event == "multi_target_damage_resolved"
                and data.get("card") == "Swelter"
                and data.get("selected_target_count") == 2
                and data.get("total_damage") == 4
                and sorted(data.get("targets") or []) == ["First Creature", "Second Creature"]
                for event, data in self.events
            )
        )
    ...
    def test_simple_activated_recursion_self_sacrifice_returns_arcane_card(self) -> None:
        active = self.battle.Player("Active", None, [])
        opponent = self.battle.Player("Opponent", None, [])
        active.mana_pool.add_generic(1)
        active.mana_pool.add("green", 1)
        wrong_instant = {"name": "Wrong Instant", "type_line": "Instant", "cmc": 1}
        arcane_spell = {"name": "Target Ritual", "type_line": "Instant - Arcane", "cmc": 2}
        active.graveyard.extend([wrong_instant, arcane_spell])
        permanent = {
            "name": "Hana Kami",
            "type_line": "Creature - Spirit",
            "effect": "creature",
            ...
            "activated_effect": "recursion",
            "activated_battle_model_scope": "xmage_permanent_simple_activated_graveyard_to_hand_v1",
            "graveyard_to_hand_target": "arcane_card",
            "graveyard_to_hand_target_count": 1,
            "graveyard_to_hand_destination": "hand",
            ...
            "graveyard_to_hand_activation_requires_tap": False,
            "graveyard_to_hand_activation_requires_sacrifice": True,
            "_rule_logical_key": "battle_rule_v1:fixture_hana_kami",
        }
        active.battlefield.append(permanent)
        ...
        self.assertNotIn(permanent, active.battlefield)
        self.assertIn(permanent, active.graveyard)
        self.assertEqual([card["name"] for card in active.hand], ["Target Ritual"])
        self.assertEqual([card["name"] for card in active.graveyard], ["Wrong Instant", "Hana Kami"])
        self.assertTrue(
            any(
                event == "recursion_resolved"
                and data.get("card") == "Hana Kami"
                and data.get("activation_kind") == "simple_activated_graveyard_to_hand"
                and data.get("target_type") == "arcane_card"
                and data.get("recovered") == ["Target Ritual"]
                for event, data in self.events
            )
        )
