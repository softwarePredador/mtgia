...
def test_current_xmage_execution_contract_is_aligned():
    ...
    assert checks["deployment.current_ops_runtime"]["status"] == "pass"
    assert checks["sidecar.reproducible_image"]["status"] == "pass"
    ...
    assert checks["server.battle_persistence"]["status"] == "pass"
