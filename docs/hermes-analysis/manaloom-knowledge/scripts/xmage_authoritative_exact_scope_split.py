...
def recursion_target_constraints_for(
    ...
    *,
    ...
    elif target == "creature_or_vehicle":
        constraints["any_of"] = [
            {"card_types": ["creature"]},
            {"subtypes": ["vehicle"]},
        ]
...
def attack_damage_recursion_target_from_phrase(phrase: str) -> dict[str, Any] | None:
    normalized = re.sub(r"\s+", " ", str(phrase or "").strip().lower())
    normalized = normalized.replace("multicoloured", "multicolored")
    patterns: list[tuple[str, dict[str, Any]]] = [
        (r"^card$", {"target": "any_card"}),
        (r"^creature card$", {"target": "creature"}),
        (r"^permanent card$", {"target": "permanent"}),
        (r"^arcane card$", {"target": "arcane_card"}),
        (
            r"^artifact, creature, or enchantment card$",
            {"target": "artifact_or_creature_or_enchantment"},
        ),
        (
            r"^creature or vehicle card with mana value (?P<mv>\d+) or less$",
            {"target": "creature_or_vehicle", "mana_value_max_group": "mv"},
        ),
    ]
    for pattern, spec in patterns:
        ...
        if not match:
            continue
        result = {"target": spec["target"], "count": 1, "destination": "hand"}
        if spec.get("mana_value_max_group"):
            result["mana_value_max"] = int(match.group(str(spec["mana_value_max_group"])))
        return result
    return None


def triggered_graveyard_to_hand_from_oracle(metadata: dict[str, Any]) -> dict[str, Any] | str:
    text = oracle_text_after_leading_static_keywords(metadata)
    if "deals damage to an opponent" in text:
        return "triggered_recursion_damage_to_opponent_not_supported"
    trigger = None
    match = re.search(
        r"whenever (?:this creature|this vehicle|[^,]+?) attacks, (?P<body>.+)$",
        text,
    )
    if match:
        trigger = "attack"
        ...
        match = re.search(
            r"whenever (?:this creature|[^,]+?) deals combat damage to a player, (?P<body>.+)$",
            text,
        )
        if match:
            trigger = "combat_damage_to_player"
    if not match or trigger is None:
        return "triggered_recursion_oracle_trigger_not_supported"
    body = match.group("body").strip()
    activation_cost_mana = None
    cost_match = re.match(
        r"you may pay (?P<cost>(?:\{[0-9wubrgc]+\})+)\. if you do, (?P<body>.+)$",
        body,
    )
    if cost_match:
        activation_cost_mana = cost_match.group("cost").upper()
        body = cost_match.group("body").strip()
    body = body.removeprefix("you may ").strip()
    target_match = re.match(
        r"return target (?P<phrase>.+?) from your graveyard to your hand\.?$",
        body,
    )
    if not target_match:
        return "triggered_recursion_oracle_target_not_supported"
    target = attack_damage_recursion_target_from_phrase(target_match.group("phrase"))
    if target is None:
        return "triggered_recursion_oracle_target_not_supported"
    result = {"trigger": trigger, **target}
    if activation_cost_mana:
        parsed = parse_mana_cost_text(activation_cost_mana)
        if parsed is None:
            return "triggered_recursion_oracle_cost_not_supported"
        result["trigger_cost_mana"] = activation_cost_mana
        result["trigger_cost_generic"] = parsed[0]
        result["trigger_cost_colors"] = parsed[1]
    return result


def triggered_graveyard_to_hand_target_from_source(source_text: str) -> dict[str, Any] | str:
    text = str(source_text or "")
    lowered = text.lower()
    if "creature or Vehicle card with mana value 2 or less".lower() in lowered:
        target = "creature_or_vehicle"
        mana_value_max = 2
    elif "artifact, creature, or enchantment card" in lowered:
        target = "artifact_or_creature_or_enchantment"
        mana_value_max = None
    elif "FilterPermanentCard" in text or "permanent card from your graveyard" in lowered:
        target = "permanent"
        mana_value_max = None
    elif "SubType.ARCANE.getPredicate" in text or "arcane card from your graveyard" in lowered:
        target = "arcane_card"
        mana_value_max = None
    elif (
        "FILTER_CARD_CREATURE_YOUR_GRAVEYARD" in text
        or "FilterCreatureCard" in text
        or "creature card from your graveyard" in lowered
    ):
        target = "creature"
        mana_value_max = None
    elif "new TargetCardInYourGraveyard()" in text:
        target = "any_card"
        mana_value_max = None
    else:
        return "triggered_recursion_source_target_not_supported"
    result = {"target": target, "count": 1, "destination": "hand"}
    if mana_value_max is not None:
        result["mana_value_max"] = mana_value_max
    return result


def triggered_graveyard_to_hand_from_source(source_text: str, expected_trigger: str) -> dict[str, Any] | str:
    text = str(source_text or "")
    if len(re.findall(r"ReturnFromGraveyardToHandTargetEffect\s*\(", text)) != 1:
        return "triggered_recursion_source_effect_count_not_supported"
    if len(re.findall(r"TargetCardInYourGraveyard\s*\(", text)) != 1:
        return "triggered_recursion_source_target_count_not_supported"
    if expected_trigger == "attack":
        if "AttacksTriggeredAbility" not in text:
            return "triggered_recursion_source_trigger_mismatch"
    elif expected_trigger == "combat_damage_to_player":
        if "DealsCombatDamageToAPlayerTriggeredAbility" not in text:
            return "triggered_recursion_source_trigger_mismatch"
    else:
        return "triggered_recursion_source_trigger_mismatch"
    target = triggered_graveyard_to_hand_target_from_source(text)
    if isinstance(target, str):
        return target
    cost_match = re.search(
        r'DoIfCostPaid\s*\(.*?ReturnFromGraveyardToHandTargetEffect\s*\(\s*\).*?ManaCostsImpl<[^>]*>\s*\(\s*"([^"]+)"\s*\)',
        text,
        re.S,
    )
    if cost_match:
        ...
        if parsed is None:
            return "triggered_recursion_source_cost_not_supported"
        target["trigger_cost_mana"] = cost_text
        target["trigger_cost_generic"] = parsed[0]
        target["trigger_cost_colors"] = parsed[1]
    return target
...
def fixed_boost_keyword_draw_target_from_source(
    ...
    if not boost_match or not draw_match:
        return None
    if not (boost_match.start() < gain_match.start() < draw_match.start()):
        return None
    if len(re.findall(r"new\s+TargetCreaturePermanent\s*\(", text)) != 1:
        return None
    if not re.search(r"new\s+TargetCreaturePermanent\s*\(\s*\)", text):
        return None
    return boost[0], boost[1], "any", int(draw_matches[0])
...
def fixed_boost_keyword_draw_target_from_oracle(
    ...
    match = re.match(
        ...
        rf"and gains ({keyword_words}) until end of turn\. draw a card\.?$",
        text,
    )
    if not match:
        return None
    ...
    target_controller = "self" if match.group(1) else "any"
    keyword = TARGET_GRANT_KEYWORD_ORACLE_WORDS[match.group(4)]
    return power, toughness, keyword, target_controller, 1
...
def is_permanent_activated_recursion_to_hand_unit(row: dict[str, Any]) -> bool:
    if str(row.get("adapter_work_unit") or "") != RECURSION_UNIT:
        return False
    abilities = ability_classes(row)
    activated = {"SimpleActivatedAbility", "ActivateAsSorceryActivatedAbility"} & abilities
    remaining = abilities - activated
    return (
...
def is_creature_etb_graveyard_count_damage_unit(row: dict[str, Any]) -> bool:
    if str(row.get("adapter_work_unit") or "") != RECURSION_UNIT:
        return False
    return (
        effect_classes(row) == {"DamageTargetEffect"}
...
def is_permanent_attack_recursion_to_hand_unit(row: dict[str, Any]) -> bool:
    if str(row.get("adapter_work_unit") or "") != RECURSION_UNIT:
        return False
    abilities = ability_classes(row)
    remaining = abilities - {"AttacksTriggeredAbility", "EntersBattlefieldTappedAbility"}
    return (
        effect_classes(row) == {"ReturnFromGraveyardToHandTargetEffect"}
...
def is_creature_combat_damage_recursion_to_hand_unit(row: dict[str, Any]) -> bool:
    if str(row.get("adapter_work_unit") or "") != RECURSION_UNIT:
        return False
    abilities = ability_classes(row)
    combat_triggers = {
    ...
    present_triggers = abilities & combat_triggers
    remaining = abilities - present_triggers
    return (
        effect_classes(row) == {"ReturnFromGraveyardToHandTargetEffect"}
        and len(present_triggers) == 1
        and remaining.issubset(STATIC_SELF_KEYWORD_ABILITY_CLASSES)
        and set(row.get("xmage_signals") or []) == {"targeting", "triggered_ability"}
    )
...
def is_creature_attack_target_keyword_unit(row: dict[str, Any]) -> bool:
    abilities = ability_classes(row)
    keyword_abilities = abilities.intersection(TARGET_GRANT_KEYWORD_ABILITY_CLASSES)
    remaining = abilities - {"AttacksTriggeredAbility"} - keyword_abilities
    return (
        str(row.get("adapter_work_unit") or "") == BOOST_KEYWORD_UNIT
        and effect_classes(row) == {"GainAbilityTargetEffect"}
        and set(row.get("xmage_signals") or []) == {"targeting", "triggered_ability"}
...
def ordered_protection_subtypes(subtypes: set[str] | list[str]) -> list[str]:
    present = {
        str(subtype or "").strip().lower()
...
def protection_from_subtypes_from_oracle(metadata: dict[str, Any]) -> list[str] | None:
    raw = str(metadata.get("oracle_text") or "").strip()
    if not raw:
        return None
    text = re.sub(r"\([^)]*\)", "", raw)
    ...
    if not parts:
        return None
    if any(part not in PROTECTION_SUBTYPE_WORDS for part in parts):
        return None
...
def oracle_protection_clause(text: str) -> str | None:
    ...
    for index, part in enumerate(parts):
        if part.startswith("protection from "):
            ...
            capturing = True
            continue
        if capturing and (part.startswith("from ") or part.startswith("and from ")):
            clauses.append(part)
            continue
        if capturing:
            break
...
def protection_subtype_from_enum(value: str) -> str:
    return str(value or "").strip().lower().replace("_", " ")


def protection_from_subtypes_from_source(source: str) -> list[str] | str:
    text = source or ""
    if not re.search(r"\bProtectionAbility\b", text):
        return "static_protection_source_missing_protection_ability"
    ability_constructors = re.findall(
    ...
    if len(ability_constructors) != 1:
        return "static_protection_source_multiple_protection_abilities"
    subtypes = {
        protection_subtype_from_enum(match)
        for match in re.findall(r"\bSubType\.([A-Z0-9_]+)\.getPredicate\s*\(", text)
    }
    subtypes.update(
        protection_subtype_from_enum(match)
        for match in re.findall(r"\bnew\s+Filter(?:Permanent|Card|CreaturePermanent)?\s*\(\s*SubType\.([A-Z0-9_]+)", text)
    )
    if not subtypes:
...
def graveyard_count_damage_from_oracle(metadata: dict[str, Any]) -> dict[str, Any] | str | None:
    ...
    etb_x_match = re.match(
        r"^(?:[a-z0-9' -]+\s+[—-]\s+)?when .+ enters(?: the battlefield)?, "
        r"(?:it|this creature) deals x damage to (?P<target>target creature an opponent controls|"
        r"target creature or planeswalker an opponent controls), where x is the number of "
        r"(?P<filter>instant and sorcery|creature) cards in your graveyard\.?$",
        text,
    )
    if etb_x_match:
        target_phrase = etb_x_match.group("target")
        count_filter = etb_x_match.group("filter")
        target = "creature_or_planeswalker" if "planeswalker" in target_phrase else "creature"
        count_types = ["instant", "sorcery"] if count_filter == "instant and sorcery" else [count_filter]
        return {
            "target": target,
            "target_controller": "opponent",
            "graveyard_count_scope": "controller_graveyard",
            "graveyard_count_card_types": count_types,
            "damage_base_amount": 0,
            "damage_per_graveyard_count": 1,
        }
    etb_for_each_match = re.match(
        r"^(?:[a-z0-9' -]+\s+[—-]\s+)?when .+ enters(?: the battlefield)?, "
        r"(?:it|this creature) deals (?P<amount>\d+) damage to target opponent for each "
        r"(?P<filter>creature) card in your graveyard\.?$",
        text,
    )
    if etb_for_each_match:
        amount = int(etb_for_each_match.group("amount"))
        if amount <= 0:
            return "graveyard_count_damage_etb_per_count_not_positive"
        return {
            "target": "opponent",
            "graveyard_count_scope": "controller_graveyard",
            "graveyard_count_card_types": [etb_for_each_match.group("filter")],
            "damage_base_amount": 0,
            "damage_per_graveyard_count": amount,
        }
...
def graveyard_count_damage_source_target_matches(
    source: str,
    target: str,
    ...
    if target_controller == "opponent":
        if target == "creature":
            return "TargetOpponentsCreaturePermanent" in text
        if target == "creature_or_planeswalker":
            return "FilterCreatureOrPlaneswalkerPermanent" in text and "TargetController.OPPONENT" in text
    if target == "creature":
...
def graveyard_count_damage_from_source(source: str, metadata: dict[str, Any]) -> dict[str, Any] | str | None:
    ...
    if (
        "CardsInControllerGraveyardCount(StaticFilters.FILTER_CARD_INSTANT_OR_SORCERY)" in compact_text
        or "CardsInControllerGraveyardCount(newFilterInstantOrSorceryCard())" in compact_text
    ):
        return {
            "target": "creature",
            "target_controller": "opponent",
    ...
    if (
        "CardsInControllerGraveyardCount(StaticFilters.FILTER_CARD_CREATURE)" in compact_text
        or "CardsInControllerGraveyardCount(StaticFilters.FILTER_CARD_CREATURES)" in compact_text
    ):
        if "TargetOpponent" in text:
            return {
                "target": "opponent",
                "graveyard_count_scope": "controller_graveyard",
        ...
        if "FilterCreatureOrPlaneswalkerPermanent" in text and "TargetController.OPPONENT" in text:
            return {
...
def restricted_target_base(target: str) -> str:
    ...
    if target in {
        ...
        "attacking_or_blocking_creature_power_2_or_less",
        "attacking_or_blocking_creature_power_3_or_less",
        "tapped_creature",
        "untapped_creature",
        "flying_creature",
        "nonblack_creature",
...
def restricted_battlefield_target_from_oracle(metadata: dict[str, Any], action: str) -> str | None:
    ...
    patterns: list[tuple[str, str]] = [
        ...
        (r"target non-angel, non-demon, non-devil, non-dragon creature", "non_angel_demon_devil_dragon_creature"),
        (r"target non-elf creature", "non_elf_creature"),
...
def restricted_battlefield_target_from_source(source: str) -> str | None:
    ...
    if (
        ...
        return "attacking_or_blocking_creature_power_2_or_less"
    if (
        "FilterAttackingOrBlockingCreature" in text
        and "PowerPredicate(ComparisonType.FEWER_THAN, 4)" in text
    ):
        return "attacking_or_blocking_creature_power_3_or_less"
    if (
    ...
    if (
        'FilterCreaturePermanent("nongreen creature")' in text
    ...
    if (
        ...
        return "nonlegendary_creature"
    if 'FilterCreaturePermanent("legendary creature")' in text or "legendary creature" in text:
        return "legendary_creature"
    if (
        'FilterCreaturePermanent("nonsnow creature")' in text
        or "nonsnow creature" in text
    ):
        return "nonsnow_creature"
    subtype_targets = {
        "non-Angel, non-Demon, non-Devil, non-Dragon creature": "non_angel_demon_devil_dragon_creature",
        "non-Elf creature": "non_elf_creature",
...
def activated_ability_line_from_oracle(metadata: dict[str, Any]) -> str:
    raw_text = str(metadata.get("oracle_text") or "").strip()
    colon_lines = [
    ...
    if len(colon_lines) == 1:
        return colon_lines[0]
    stripped = oracle_text_after_leading_static_keywords(metadata)
    if stripped and stripped.count(":") == 1:
        return stripped
    return oracle_text(metadata)
...
TARGET_KEYWORD_IMPLICIT_CREATURE_SUBTYPES = {
    "dinosaur",
    "knight",
}
...
def activated_target_keyword_target_phrase_data(phrase: str) -> dict[str, Any] | str:
    ...
    for prefix in ("another target ", "another "):
    ...
    without_match = re.search(
        r"\s+without\s+(?P<keyword>deathtouch|double strike|first strike|flying|haste|hexproof|indestructible|lifelink|menace|reach|trample|vigilance)$",
        target_phrase,
    )
    if without_match:
        keyword = TARGET_GRANT_KEYWORD_ORACLE_WORDS.get(without_match.group("keyword"))
        if keyword is None:
            return "activated_target_keyword_source_target_not_supported"
        constraints["excluded_keywords"] = [keyword]
        target_phrase = target_phrase[: without_match.start()].strip()
    ...
    if not subtypes:
        return "activated_target_keyword_source_target_not_supported"
    if combat_state or all(subtype in TARGET_KEYWORD_IMPLICIT_CREATURE_SUBTYPES for subtype in subtypes):
        constraints["target_subtypes"] = subtypes
        return {
            "target": "creature",
            "target_controller": target_controller,
...
def activated_target_keyword_target_from_source(source: str) -> dict[str, Any] | str:
    ...
    controlled_subtype_match = re.search(
        r"new\s+FilterControlledPermanent\s*\(\s*SubType\.([A-Z0-9_]+)(?:\s*,\s*\"([^\"]+)\")?\s*\)",
        text,
        re.S,
    )
    if controlled_subtype_match and re.search(r"new\s+TargetPermanent\s*\(", text, re.S):
        if controlled_subtype_match.group(2):
            return activated_target_keyword_target_phrase_data(controlled_subtype_match.group(2))
        subtype = controlled_subtype_match.group(1).lower()
        return {
            "target": "creature",
            "target_controller": "self",
            "exclude_source": False,
            "target_constraints": {"card_types": ["creature"], "target_subtypes": [subtype]},
        }
...
def attack_target_keyword_from_oracle(metadata: dict[str, Any]) -> dict[str, Any] | str:
    text = strip_parenthetical_reminders(
        oracle_text_after_leading_static_keywords(metadata)
    )
    text = re.sub(r"\s+", " ", text).strip().lower()
    ...
    match = re.match(
        rf"^whenever (?:this creature|[^,.]+) attacks, "
        rf"(?P<optional>you may have )?(?P<another>another\s+)?target\s+"
        rf"(?P<target>.+?)\s+gains?\s+(?P<keyword>{keyword_words}) until end of turn\.?$",
        text,
    )
    if not match:
        return "attack_target_keyword_oracle_not_simple"
    target_phrase = match.group("target")
    target_data = activated_target_keyword_target_phrase_data(target_phrase)
    if isinstance(target_data, str):
        return target_data.replace("activated_target_keyword", "attack_target_keyword")
    if match.group("another"):
        target_data["exclude_source"] = True
        target_data.setdefault("target_constraints", {})["exclude_source"] = True
    return {
        "keyword": TARGET_GRANT_KEYWORD_ORACLE_WORDS[match.group("keyword")],
        "optional_trigger": bool(match.group("optional")),
        **target_data,
    }


def attack_target_keyword_from_source(source: str, keyword_ability_class: str) -> dict[str, Any] | str:
    text = source or ""
    if "AttacksTriggeredAbility" not in text:
        return "attack_target_keyword_source_not_attack_trigger"
    if "Zone.GRAVEYARD" in text:
        return "attack_target_keyword_source_not_battlefield"
    gain_matches = re.findall(r"new\s+GainAbilityTargetEffect\s*\(", text)
    if len(gain_matches) != 1:
        return "attack_target_keyword_source_not_single_effect"
    ability_expr = (
        rf"(?:{re.escape(keyword_ability_class)}\.getInstance\s*\(\s*\)"
        rf"|new\s+{re.escape(keyword_ability_class)}\s*\([^)]*\))"
    )
    if not re.search(
        ...
        return "attack_target_keyword_source_keyword_not_supported"
    target_data = activated_target_keyword_target_from_source(text)
    if isinstance(target_data, str):
        return target_data.replace("activated_target_keyword", "attack_target_keyword")
    optional_trigger = bool(
        re.search(r"new\s+AttacksTriggeredAbility\s*\([^;]*,\s*true\s*\)", text, re.S)
    )
    return {
        "keyword": TARGET_GRANT_KEYWORD_ABILITY_CLASSES[keyword_ability_class],
...
def activated_recursion_to_hand_from_source(source: str) -> dict[str, Any] | str:
    ...
    result = {
        ...
        "count": count,
        "up_to": up_to,
        "xmage_ability_class": ability_match.group(1),
        "activation_cost_mana": cost_text,
        "activation_cost_generic": activation_cost_generic,
        "activation_cost_colors": activation_cost_colors,
        "activation_requires_tap": requires_tap,
...
def dies_life_gain_amount_from_oracle(metadata: dict[str, Any]) -> int | None:
    ...
    match = re.fullmatch(
        r"(?:when|whenever) (?:this creature|[^,.]+) dies, you gain "
        r"(a|one|two|three|four|five|\d+) life\.?",
        text,
    )
    if not match:
        return None
    amount = number_word_to_int(match.group(1))
...
def sacrifice_mana_source_detail_from_line(text: str) -> dict[str, Any] | None:
    ...
    prefix_match = re.match(
        r"^(?P<costs>(?:\{(?:t|[0-9wubrgc]+)\},\s*)*)sacrifice [^:]+: add (?P<mana>.+?)\.?$",
        normalized,
    )
    if not prefix_match:
        return None
    costs = prefix_match.group("costs") or ""
    mana_phrase = prefix_match.group("mana").strip()
    requires_tap = "{t}" in costs
    activation_symbols = [
        symbol.upper()
    ...
    if mana_phrase == "one mana of any color":
        ...
        produced_symbols = _brace_mana_symbols(mana_phrase)
...
def sacrifice_mana_source_detail_from_oracle(metadata: dict[str, Any]) -> dict[str, Any] | None:
    text = strip_parenthetical_reminders(oracle_text(metadata))
    detail = sacrifice_mana_source_detail_from_line(text)
    if detail is not None:
        return detail

    candidates = []
    for line in normalized_oracle_lines(metadata):
        line_detail = sacrifice_mana_source_detail_from_line(line)
...
def java_simple_mana_symbols_from_source(source_text: str) -> list[str] | None:
    text = source_text or ""
    method_match = re.search(
        r"Mana\.(White|Blue|Black|Red|Green|Colorless)Mana\s*\(\s*(\d+)\s*\)",
        text,
    )
    if method_match:
    ...
    if constructor_match:
        ...
        symbols: list[str] = []
        for symbol, count in zip(["W", "U", "B", "R", "G", "C"], values[:6]):
            symbols.extend([symbol] * max(0, count))
...
def sacrifice_mana_source_source_blocker(source_text: str, detail: dict[str, Any]) -> str | None:
    ...
    if requires_tap and "TapSourceCost" not in text:
        return "mana_source_sacrifice_source_tap_cost_missing"
    if not requires_tap and "TapSourceCost" in text:
        return "mana_source_sacrifice_source_oracle_tap_mismatch"
    ...
    if activation_mana_cost:
        ...
        if generic_match:
            ...
            if (
                f"GenericManaCost({generic_cost})" not in text
                and f'ManaCostsImpl<>("{activation_mana_cost}")' not in text
                and f"ManaCostsImpl<>(\"{activation_mana_cost}\")" not in text
            ):
                return "mana_source_sacrifice_source_activation_cost_mismatch"
        elif f'ManaCostsImpl<>("{activation_mana_cost}")' not in text:
            return "mana_source_sacrifice_source_activation_cost_mismatch"
    elif "GenericManaCost" in text or "ManaCostsImpl<>" in text:
        return "mana_source_sacrifice_source_activation_cost_mismatch"
...
def parse_library_tutor_query(query: str) -> dict[str, Any] | None:
    ...
    if named_match:
        ...
        parsed["target_names"] = [
            name.strip()
            for name in re.split(r"\s+or\s+", named_match.group("name").strip())
            if name.strip()
        ]
        return parsed if count > 0 else None

    mana_value_match = re.fullmatch(
        r"(?P<card_type>artifact|creature|instant|sorcery|enchantment|planeswalker|permanent) cards? "
        r"with mana value (?P<first>\d+)(?: or (?P<second>\d+)| or less)?",
        phrase,
    )
    if mana_value_match:
        parsed["target"] = "any" if mana_value_match.group("card_type") == "permanent" else mana_value_match.group("card_type")
        parsed["target_card_types"] = (
            ["artifact", "creature", "enchantment", "planeswalker", "land", "battle"]
            if mana_value_match.group("card_type") == "permanent"
            else [mana_value_match.group("card_type")]
        )
    ...
    subtype_permanent_match = re.fullmatch(
        r"(?P<subtype>[a-z][a-z' -]+?) permanent cards?(?: with mana value (?P<mv>\d+) or less)?",
        phrase,
    )
    if subtype_permanent_match:
        parsed["target"] = "any"
        parsed["target_card_types"] = ["artifact", "creature", "enchantment", "planeswalker", "land", "battle"]
        parsed["target_subtypes"] = [subtype_permanent_match.group("subtype").strip()]
        if subtype_permanent_match.group("mv") is not None:
            parsed["target_mana_value_max"] = int(subtype_permanent_match.group("mv"))
        return parsed if count > 0 else None
...
def library_tutor_source_query(source: str) -> dict[str, Any] | None:
    ...
    if re.search(r"new\s+TargetCardInLibrary\s*\(\s*\)", text):
        return {"target": "any"}
    if re.search(r"new\s+FilterBasicCard\s*\(\s*SubType\.FOREST\s*\)", text):
        return {"target": "forest", "required_supertypes": ["basic"]}
    ...
    elif "new FilterPermanentCard" in text:
        base = {
            "target": "any",
            "target_card_types": ["artifact", "creature", "enchantment", "planeswalker", "land", "battle"],
        }
    ...
    if re.search(r"SuperType\.LEGENDARY\.getPredicate\s*\(\s*\)", text):
    ...
    filter_match = re.search(
        r'new\s+Filter(?:Card|LandCard|CreatureCard|ArtifactCard|EnchantmentCard|PlaneswalkerCard|PermanentCard)\s*\(\s*"([^"]+)"\s*\)',
        text,
    )
    if filter_match:
...
def activated_library_tutor_to_battlefield_from_oracle(metadata: dict[str, Any]) -> dict[str, Any] | str:
    text = activated_ability_line_from_oracle(metadata)
    text = re.sub(r"\s+", " ", text).strip().lower()
    if text.count(":") != 1:
        return "activated_library_tutor_oracle_not_simple"
    cost_text, effect_text = [part.strip() for part in text.split(":", 1)]
    source_name = re.sub(r"\s+", " ", str(metadata.get("name") or "").strip().lower())
    if source_name:
    ...
    activation = activation_cost_from_oracle_prefix(cost_text, allow_source_sacrifice=True)
    if isinstance(activation, str):
        return str(activation).replace("activated_self_boost", "activated_library_tutor")
    match = re.match(
        r"^search your library for (?P<query>.+?)(?:,| and) put "
        r"(?:it|that card|them|those cards) onto the battlefield(?P<tapped> tapped)?, "
        r"then shuffle(?: your library)?\.(?: .*)?$",
    ...
    parsed = parse_library_tutor_query(match.group("query"))
    if parsed is None:
        return "activated_library_tutor_oracle_target_not_supported"
    parsed["destination"] = "battlefield"
    parsed["enters_tapped"] = bool(match.group("tapped"))
    return {**parsed, **activation}


def activated_library_tutor_to_battlefield_from_source(source: str) -> dict[str, Any] | str:
    text = source or ""
    effect_matches = re.findall(r"SearchLibraryPutInPlayEffect\s*\(", text)
    if len(effect_matches) != 1:
        return "activated_library_tutor_source_effect_not_supported"
    effect_index = text.find("SearchLibraryPutInPlayEffect")
    window = text[max(0, effect_index - 500) : effect_index + 1800]
    if "SimpleActivatedAbility" not in window:
        return "activated_library_tutor_source_not_simple_activated"
    risky_cost_classes = {
        "CompositeCost",
        "DiscardCardCost",
        "DiscardTargetCost",
    ...
    present_risky = sorted(cost for cost in risky_cost_classes if cost in window)
    if present_risky:
        return "activated_library_tutor_source_cost_not_supported"
    parsed_target = library_tutor_source_query(text)
    if parsed_target is None:
        return "activated_library_tutor_source_target_not_supported"
    count, up_to = library_tutor_count_from_source(text)
    tapped_match = re.search(
        r"SearchLibraryPutInPlayEffect\s*\(\s*(?:new\s+TargetCardInLibrary\s*\([^)]*\)|\w+)\s*,\s*(true|false)",
        text,
    ...
    parsed_cost = parse_mana_cost_text(cost_text)
    if parsed_cost is None:
        return "activated_library_tutor_source_mana_cost_not_supported"
    activation_cost_generic, activation_cost_colors = parsed_cost
    return {
        **parsed_target,
        "count": count,
...
def target_constraints_for(target: str) -> dict[str, Any]:
    ...
    if target == "attacking_or_blocking_creature_power_2_or_less":
        return {"card_types": ["creature"], "combat_state": "attacking_or_blocking", "power_max": 2}
    if target == "attacking_or_blocking_creature_power_3_or_less":
        return {"card_types": ["creature"], "combat_state": "attacking_or_blocking", "power_max": 3}
    ...
    if target == "nonlegendary_creature":
        return {"card_types": ["creature"], "exclude_supertypes": ["legendary"]}
    if target == "nonsnow_creature":
        return {"card_types": ["creature"], "exclude_supertypes": ["snow"]}
    if target == "non_angel_demon_devil_dragon_creature":
...
def split_row(
    ...
    *,
    ...
    attack_recursion_permanent_unit = is_permanent_attack_recursion_to_hand_unit(row)
    combat_damage_recursion_creature_unit = is_creature_combat_damage_recursion_to_hand_unit(row)
    ...
    boost_keyword_draw_spell_unit = (
        unit == DRAW_UNIT
        and effect_classes(row)
        == {"BoostTargetEffect", "DrawCardSourceControllerEffect", "GainAbilityTargetEffect"}
        and set(row.get("xmage_signals") or []) == {"targeting", "draw"}
        and len(ability_classes(row)) == 1
        and next(iter(ability_classes(row))) in TARGET_GRANT_KEYWORD_ABILITY_CLASSES
    )
    ...
    if (
        ...
        and not attack_recursion_permanent_unit
        and not combat_damage_recursion_creature_unit
    ...
    if permanent_activated_tutor_battlefield_unit:
        if not is_permanent_metadata(metadata) or is_spell(metadata):
            return None, "activated_library_tutor_not_permanent"
        oracle_tutor = activated_library_tutor_to_battlefield_from_oracle(metadata)
        if isinstance(oracle_tutor, str):
            return None, oracle_tutor
        source_tutor = activated_library_tutor_to_battlefield_from_source(source_text)
        if isinstance(source_tutor, str):
            return None, source_tutor
        mismatch = library_tutor_specs_match(source_tutor, oracle_tutor)
        if mismatch:
            return None, f"activated_library_tutor_source_oracle_{mismatch}_mismatch"
        for key in (
            "activation_cost_mana",
            "activation_cost_generic",
            "activation_cost_colors",
            ...
            if source_tutor.get(key) != oracle_tutor.get(key):
                return None, f"activated_library_tutor_source_oracle_{key}_mismatch"
        type_line = str(metadata.get("type_line") or "").lower()
        permanent_effect = (
            "creature"
            if "creature" in type_line
        ...
        activated_effect = {
            "effect": "tutor",
            "battle_model_scope": PERMANENT_ACTIVATED_TUTOR_BATTLEFIELD_SCOPE,
            "ability_kind": "activated",
            "activated_effect": "tutor",
            "activated_battle_model_scope": PERMANENT_ACTIVATED_TUTOR_BATTLEFIELD_SCOPE,
            "target": target,
            "tutor_target": target,
        ...
        if bool(source_tutor.get("activation_requires_sacrifice")):
            activated_effect["activated_self_sacrifice_tutor_to_battlefield"] = True
            effect_json["activated_self_sacrifice_tutor_to_battlefield"] = True
    ...
    if attack_target_keyword_unit:
        if not is_creature_metadata(metadata):
            return None, "attack_target_keyword_not_creature"
        keyword_abilities = ability_classes(row).intersection(TARGET_GRANT_KEYWORD_ABILITY_CLASSES)
        if len(keyword_abilities) != 1:
            return None, "attack_target_keyword_ability_not_single"
        keyword_ability_class = next(iter(keyword_abilities))
        oracle_spec = attack_target_keyword_from_oracle(metadata)
        if isinstance(oracle_spec, str):
            return None, oracle_spec
        source_spec = attack_target_keyword_from_source(source_text, keyword_ability_class)
        if isinstance(source_spec, str):
            return None, source_spec
        for key in (
            "keyword",
            ...
            if source_spec.get(key) != oracle_spec.get(key):
                return None, "attack_target_keyword_source_oracle_mismatch"
        oracle_keywords = static_keywords_from_oracle(metadata)
        if oracle_keywords is None:
            return None, "attack_target_keyword_oracle_static_keyword_not_exact"
        ability_keywords = keywords_from_ability_classes(row)
        if not oracle_keywords.issubset(ability_keywords):
            return None, "attack_target_keyword_oracle_static_keyword_mismatch"
        static_keywords = oracle_keywords
        effect_json = {
            "effect": "creature",
            "battle_model_scope": ATTACK_TRIGGER_TARGET_KEYWORD_SCOPE,
            "ability_kind": "triggered",
            "trigger": "attack",
            "trigger_effect": "target_keyword_until_eot",
            "target": oracle_spec["target"],
            "target_controller": oracle_spec["target_controller"],
            "target_constraints": dict(oracle_spec.get("target_constraints") or {"card_types": ["creature"]}),
            "power_delta": 0,
            "toughness_delta": 0,
            "duration": "until_end_of_turn",
            "granted_keywords_until_eot": [oracle_spec["keyword"]],
            "attack_trigger_target_keyword": True,
            "attack_trigger_optional": bool(oracle_spec.get("optional_trigger")),
            "xmage_effect_class": "GainAbilityTargetEffect",
        ...
        return build_proposal(
            ...
            metadata,
            effect_json,
            family_id="xmage_creature_attack_target_keyword_until_eot",
        ), "selected_exact_scope"
    ...
    if permanent_activated_recursion_to_hand_unit:
        ...
        activated_effect = {
            ...
            "xmage_ability_class": parsed_activation.get("xmage_ability_class", "SimpleActivatedAbility"),
            **{
                key: parsed_activation[key]
                for key in (
                    "activation_cost_mana",
        ...
        if parsed_activation.get("activation_timing"):
            ...
            effect_json["graveyard_to_hand_activation_timing"] = parsed_activation["activation_timing"]
    ...
    if static_protection_from_colors_creature_unit:
        ...
        if not oracle_keywords.issubset(keywords):
    ...
    if dies_life_gain_creature_unit:
        if not is_creature_metadata(metadata):
            return None, "dies_life_gain_not_creature"
        amount = dies_life_gain_amount_from_oracle(metadata)
        constructor_amount = java_constructor_int(source_text, "GainLifeEffect")
        if amount is None or amount <= 0:
            return None, "dies_life_gain_amount_not_fixed"
        if constructor_amount is None:
            return None, "dies_life_gain_amount_source_not_fixed"
        if constructor_amount != amount:
            return None, "dies_life_gain_amount_source_oracle_mismatch"
        keyword_list = ordered_keywords(keywords_from_ability_classes(row))
        effect_json = {
            "effect": "creature",
            "battle_model_scope": DIES_LIFE_GAIN_CREATURE_SCOPE,
            "ability_kind": "triggered",
            "trigger": "dies",
            "gain_life_when_this_dies": amount,
            "xmage_effect_class": "GainLifeEffect",
            "xmage_ability_class": "DiesSourceTriggeredAbility",
        }
        if keyword_list:
    ...
    if attack_recursion_permanent_unit or combat_damage_recursion_creature_unit:
        if not is_permanent_metadata(metadata) or is_spell(metadata):
            return None, "triggered_recursion_not_permanent"
        if combat_damage_recursion_creature_unit and not is_creature_metadata(metadata):
            return None, "combat_damage_recursion_not_creature"
        oracle_trigger = triggered_graveyard_to_hand_from_oracle(metadata)
        if isinstance(oracle_trigger, str):
            return None, oracle_trigger
        expected_trigger = (
            "combat_damage_to_player"
            if combat_damage_recursion_creature_unit
            else "attack"
        )
        if str(oracle_trigger.get("trigger")) != expected_trigger:
            return None, "triggered_recursion_oracle_trigger_mismatch"
        source_trigger = triggered_graveyard_to_hand_from_source(source_text, expected_trigger)
        ...
        for key in (
            ...
            "mana_value_max",
            "trigger_cost_mana",
            "trigger_cost_generic",
            "trigger_cost_colors",
        ):
            if source_trigger.get(key) != oracle_trigger.get(key):
                return None, f"triggered_recursion_source_oracle_{key}_mismatch"
        target_type = str(oracle_trigger["target"])
        count = int(oracle_trigger["count"])
        mana_value_max = oracle_trigger.get("mana_value_max")
        type_line = str(metadata.get("type_line") or "").lower()
        permanent_effect = (
            "creature"
            if "creature" in type_line
        ...
        keyword_list = ordered_keywords(keywords_from_ability_classes(row))
        if expected_trigger == "combat_damage_to_player":
            effect_json = {
                "effect": "creature",
                "battle_model_scope": DAMAGE_RECURSION_CREATURE_SCOPE,
                "ability_kind": "triggered",
                "trigger": "combat_damage_to_player",
                "combat_damage_player_graveyard_recursion": True,
                "combat_damage_recursion_target": target_type,
                "combat_damage_recursion_count": count,
            ...
            if mana_value_max is not None:
                effect_json["combat_damage_recursion_mana_value_max"] = mana_value_max
        else:
            effect_json = {
                "effect": permanent_effect,
                "battle_model_scope": ATTACK_RECURSION_PERMANENT_SCOPE,
                "ability_kind": "triggered",
                "trigger": "attack",
                "attack_trigger_graveyard_recursion": True,
                "attack_recursion_target": target_type,
                "attack_recursion_count": count,
            ...
            if mana_value_max is not None:
                effect_json["attack_recursion_mana_value_max"] = mana_value_max
            if oracle_trigger.get("trigger_cost_mana"):
                effect_json["attack_recursion_trigger_cost_mana"] = oracle_trigger["trigger_cost_mana"]
                effect_json["attack_recursion_trigger_cost_generic"] = oracle_trigger["trigger_cost_generic"]
                effect_json["attack_recursion_trigger_cost_colors"] = list(oracle_trigger["trigger_cost_colors"])
        if "EntersBattlefieldTappedAbility" in ability_classes(row):
            effect_json["enters_tapped"] = True
        if keyword_list:
            effect_json["keywords"] = keyword_list
            effect_json["_keywords_are_self"] = True
        ...
        return build_proposal(
            ...
            family_id=(
                ...
                if expected_trigger == "combat_damage_to_player"
                else "xmage_permanent_attack_graveyard_to_hand"
            ),
        ), "selected_exact_scope"
    ...
    if unit == DRAW_UNIT:
        ...
        if classes == {"BoostTargetEffect", "DrawCardSourceControllerEffect", "GainAbilityTargetEffect"}:
            if not boost_keyword_draw_spell_unit:
                return None, "boost_keyword_draw_ability_class_not_simple"
            if not is_spell(metadata):
                return None, "not_instant_or_sorcery_spell"
            abilities = ability_classes(row)
            ...
            if source_boost is None:
                return None, "boost_keyword_draw_source_not_exact_fixed"
            oracle_boost = fixed_boost_keyword_draw_target_from_oracle(metadata)
            if oracle_boost is None:
                return None, "boost_keyword_draw_oracle_not_exact_fixed"
            source_power, source_toughness, source_target_controller, source_draw_count = source_boost
            (
                oracle_power,
                oracle_toughness,
                oracle_keyword,
            ...
            if (source_power, source_toughness) != (oracle_power, oracle_toughness):
                return None, "boost_keyword_draw_source_oracle_boost_mismatch"
            if keyword != oracle_keyword:
                return None, "boost_keyword_draw_source_oracle_keyword_mismatch"
            if source_target_controller != oracle_target_controller:
                return None, "boost_keyword_draw_source_oracle_target_mismatch"
            if source_draw_count != oracle_draw_count:
                return None, "boost_keyword_draw_source_oracle_draw_count_mismatch"
            boost_keyword_component = {
                "effect": "stat_modifier_until_eot",
            ...
            effect_json = {
                "effect": "composite_resolution",
                "battle_model_scope": BOOST_KEYWORD_DRAW_SCOPE,
                "target": "creature",
                "target_constraints": {"card_types": ["creature"]},
                "target_controller": oracle_target_controller,
                "power_delta": oracle_power,
                ...
                "count": oracle_draw_count,
                "_composite_rule_components": [boost_keyword_component, draw_component],
    ...
    if unit == RECURSION_UNIT:
        ...
        if etb_graveyard_count_damage_creature_unit:
            if not is_creature_metadata(metadata):
                return None, "etb_graveyard_count_damage_not_creature"
            if ability_kind(row) != "triggered":
                return None, "etb_graveyard_count_damage_not_triggered"
            oracle_spec = graveyard_count_damage_from_oracle(metadata)
            if isinstance(oracle_spec, str):
                return None, oracle_spec
            if oracle_spec is None:
                return None, "etb_graveyard_count_damage_oracle_not_supported"
            source_spec = graveyard_count_damage_from_source(source_text, metadata)
            if isinstance(source_spec, str):
                return None, source_spec
            if source_spec is None:
                return None, "etb_graveyard_count_damage_source_not_supported"
            for key in (
                "target",
                "target_controller",
                "graveyard_count_scope",
                ...
                if oracle_spec.get(key) != source_spec.get(key):
                    return None, f"etb_graveyard_count_damage_{key}_mismatch"
            target = str(oracle_spec["target"])
            target_controller = oracle_spec.get("target_controller")
            if not graveyard_count_damage_source_target_matches(
                source_text,
                target,
                str(target_controller) if target_controller else None,
            ):
                return None, "etb_graveyard_count_damage_target_source_mismatch"
            target_constraints = target_constraints_for(target)
            if target_controller:
                target_constraints["controller"] = str(target_controller)
            effect_json = {
                "effect": "creature",
                "battle_model_scope": ETB_GRAVEYARD_COUNT_DAMAGE_CREATURE_SCOPE,
                "ability_kind": "triggered",
                "trigger": "enters_battlefield",
                "etb_dynamic_damage": True,
                "etb_damage_target": target,
            ...
            return build_proposal(
                ...
                metadata,
                effect_json,
                family_id="xmage_creature_etb_dynamic_graveyard_count_damage",
            ), "selected_exact_scope"
    ...
    if unit in RAMP_UNITS:
        ...
        if self_sacrifice_mana_source:
            mana_source_detail = sacrifice_mana_source_detail_from_oracle(metadata)
            if mana_source_detail is None:
                return None, "mana_source_sacrifice_oracle_not_simple"
            source_blocker = sacrifice_mana_source_source_blocker(source_text, mana_source_detail)
            if source_blocker:
                return None, source_blocker
            type_line = str(metadata.get("type_line") or "").lower()
            permanent_type = (
            ...
            effect_json = {
                "effect": "ramp_permanent",
                "battle_model_scope": SELF_SACRIFICE_MANA_SOURCE_SCOPE,
                "is_mana_source": True,
