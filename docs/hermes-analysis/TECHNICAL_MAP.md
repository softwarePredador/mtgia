...
- **P1 — Incoerencia de ownership em rotas app-facing de IA/deck**: revalidacao local de 2026-05-28 confirmou que `POST /ai/optimize` e `POST /ai/archetypes` recebem `deck_id` do app, mas carregam `decks`/`deck_cards` sem escopo `user_id` em parte do caminho `server/routes` -> `server/lib`. Jobs async de optimize ainda aceitam leitura quando `user_id = NULL`; rotas experimentais `simulate`, `recommendations`, `simulate-matchup` e `weakness-analysis` tambem devem ser escopadas antes de virarem superficie app-facing.
- **P1/P2 — Helpers duplicados com risco de drift**: heurísticas semânticas (`_looksLikeComboPiece`, `_looksLikeEngine`, `_looksLikePayoff`, `_looksLikeEnabler`, `_looksLikeWincon`) existem tanto em `functional_card_tags.dart` quanto em `optimization_functional_roles.dart`; `_isBasicLandName` tem variantes em optimize, generated deck validation, meta reference e commander-reference; utilitários de request/payload repetem-se em múltiplas rotas de trades/conversations.
- **P1/P2 — Pipeline semantico de cartas parcialmente saneado**: revalidacao em `origin/master@00437690` confirmou prioridade `functional_tags_then_semantic_v2_then_heuristic`, preservacao multi-role no optimize, testes focados e docs operacionais. Permanece pendente centralizar as politicas por nome restantes (`premiumLandNames`, sets premium/high-power em candidate quality) em policy/config/tabela versionada; `/decks/:id/recommendations` e `/ai/weakness-analysis` continuam experimentais/not-proven ate reutilizarem a camada semantica compartilhada ou terem contrato interno explicito.
