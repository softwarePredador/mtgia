...
- **P1 — Pipeline semantico de cartas ainda nao converge em uma fonte unica**: deck analysis carrega `semantic_tags_v2`, mas `summarizeFunctionalTagsForDeck` conta primeiro `card_function_tags` e depois heuristica v1; optimize usa `semantic_tags_v2` antes de oracle/type, mas colapsa multi-tags para um unico role e o veto parcial so bloqueia perdas de `draw`, `removal`, `ramp` e `wipe`. Complete/optimize ainda possui fallbacks por listas hardcoded de nomes (`loadUniversalCommanderFallbacks`, `loadArchetypeCommanderFoundationFillers`, `_premiumCommanderFillerNames`).
