...
| `server/routes/ai/optimize/index.dart` | 2581 | ⚠️ Split iniciado | Response/cache, envelope async, request parsing, payload final, warnings finais, diagnostics finais, fallback vazio, payloads de rejeição, validação pós-processamento, retry orchestration, filtro inicial de sugestões, filtro de identidade de cor, filtro de bracket, top-up deterministic/complete e proteção de remoção de lands foram movidos; manter rota como orquestração fina e mover reequilíbrio final para support service |
