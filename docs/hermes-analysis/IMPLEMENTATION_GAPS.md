...
| `server/routes/ai/optimize/index.dart` | 2731 | ⚠️ Split iniciado | Response/cache, envelope async, request parsing, payload final, warnings finais, diagnostics finais, fallback vazio e payloads de rejeição foram movidos; manter rota como orquestração fina e mover retry orchestration/validação pós-processamento para support services |
