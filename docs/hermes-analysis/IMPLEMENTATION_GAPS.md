...
| P2 | Ownership de arquivos no container Hermes pode bloquear crons legítimas | Hermes report-only do commit `29916949` confirmou arquivos alterados como `root:root` dentro do container; isso pode causar `PermissionError` em crons que precisam editar/gerar artefatos | Resolver como tarefa operacional controlada (`chown`/usuário de execução) somente depois de decidir quais crons podem escrever; não mascarar como bug de código do battle engine |
