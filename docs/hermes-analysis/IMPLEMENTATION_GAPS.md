...
| P1 | Formalizar a politica de mulligan Commander no auditor/trace como `curve + color + plan + sequencing + interaction` | A parte legal ja esta fechada; em 2026-06-17 o London Mulligan passou a escolher bottom por politica auditavel, e em 2026-06-18 o keep passou a rejeitar `reactive_only` land-heavy e `expensive_cluster_without_setup`. O gap restante agora e enriquecer o trace comparativo com alternativas rejeitadas e calibrar em corpus maior por comandante/arquetipo | Abertura de maos mais reproduzivel e melhor rastreabilidade do porquê keep/mull/bottom |
