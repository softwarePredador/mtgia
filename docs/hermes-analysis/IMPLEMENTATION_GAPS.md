...
| P1 | Reduzir dependencia do fallback literal Lorehold no builder deterministico | Em 2026-06-16 o builder passou a consumir `usage_hot_cards` antes do fallback literal, reduzindo `fallback_only` de `25` para `16`; em 2026-06-17 o limite de candidatos aprendidos do generator subiu para `50`, o recheck local `live4` zerou `built_in_fallback_only_count`, e após normalizar a row persistida do Lorehold o recheck `live5` manteve `built_in_fallback_only_count=0`, `profile.usable_runtime_origin=null` e `source_usage_counts.deterministic_fallback=42` | O problema deixou de ser “fallback puro residual” e virou “fallback auxiliar ainda alto em 42 slots”. A ação correta agora é atacar esse bucket por papel/coverage e manter explainability por carta, não remover o fallback inteiro no escuro |
