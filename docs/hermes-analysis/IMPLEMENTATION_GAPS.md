...
| `server/routes/ai/optimize/index.dart` | 2515 | ⚠️ Split iniciado | Response/cache, envelope async, request parsing, payload final, warnings finais, diagnostics finais, fallback vazio, payloads de rejeição, validação pós-processamento, retry orchestration, filtro inicial de sugestões, filtro de identidade de cor, filtro de bracket, top-up deterministic/complete, proteção de remoção de lands, reequilíbrio pós-filtros, coleta EDHREC, query de dados completos das adições/quality gate e análise virtual pós-swap foram movidos; manter rota como orquestração fina e extrair execução do `OptimizationValidator`/rejeição final para support service |
