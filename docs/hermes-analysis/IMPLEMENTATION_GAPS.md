...
| P1 | Sair do bucket hardcoded de arquétipo no quality gate e passar a usar `role_targets`/assinatura do profile | Parcialmente fechado em 2026-06-19: `filterUnsafeOptimizeSwapsByCardData()` e o gate final agora aceitam `profileRoleTargets`, mapeiam chaves especializadas como `spell_payoffs_copy_engines`, `draw_rummage_opponent_turn_draw`, `board_wipes_resets`, `spot_interaction` e `lands`, e o endpoint `/ai/optimize` passa `role_targets` do commander reference profile quando disponível. Isso protege engines/plano do comandante e usa range de terrenos do profile antes dos buckets genéricos. Pendente: rodar corpus maior de optimize para calibrar falsos positivos por comandante e expandir diagnostics públicos de why-blocked sem expor metadados brutos Hermes | Optimize mais aderente ao profile real do comandante, inclusive Lorehold, sem quebrar fallback archetype-only quando não houver profile |
| P1 | Congelar a precedência do builder determinístico em código e diagnostics | Fechado no slice 2026-06-19: `deterministicReferenceDeckSourcePrecedence` declara `active_learned_deck -> reference_card_stats -> reference_corpus_packages -> profile_expected_packages -> usage_hot_cards -> deterministic_fallback` e `toDiagnosticsJson()` expõe essa ordem para auditores | Qualquer relatório/provenance deve ler o diagnostics; docs antigas com outra ordem são históricas e não fonte ativa |
...
- Slice adicional de remoção por exílio:
  - `Exile target creature.` e `Exile target nonland permanent.` continuam
    entrando na fila `needs_rule_review` como `targeted_interaction`, mas agora
    têm focused evidence próprio antes de qualquer promoção manual;
  ...
  - `manaloom_battle_rule_focused_evidence.py` cria regra SQLite temporária com
    `destination=exile`, executa replay focado, exige alvo em `exile`, alvo
...
- Expandir `targeted_interaction` e `mass_removal_or_modal_wipe` somente para
  ...
  target permanent/artifact/enchantment, destroy/exile com rider, modal wipe,
