...
| `server/routes/ai/optimize/index.dart` | 2670 | ⚠️ Split iniciado | Response/cache, envelope async, request parsing, payload final, warnings finais, diagnostics finais, fallback vazio, payloads de rejeição e validação pós-processamento foram movidos; manter rota como orquestração fina e mover retry orchestration para support service |
