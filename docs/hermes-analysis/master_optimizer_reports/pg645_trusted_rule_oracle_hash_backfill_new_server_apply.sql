...
CREATE TABLE IF NOT EXISTS manaloom_deploy_audit.pg645_trusted_rule_oracle_hash_backfill_20260707 AS
SELECT r.*
...
BEGIN
  ...
  IF v_bad_count > 0 THEN
    RAISE EXCEPTION 'PG645 abort: found % trusted active rules with missing hash but no safe card oracle_text source', v_bad_count;
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(E'\n', nullif(r.notes, ''), 'PG645: oracle_hash backfilled from cards.oracle_text for trusted active rule.'),
    updated_at = now(),
    last_seen_at = now()
  FROM public.cards c
