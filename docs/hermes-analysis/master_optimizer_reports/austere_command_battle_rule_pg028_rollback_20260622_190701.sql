...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg028_austere_command_battle_rule_20260622_190701') IS NULL THEN
    RAISE EXCEPTION 'PG028 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'austere command'
  AND (
    logical_rule_key = 'battle_rule_v1:5f19a608b87445bcc5c7ebb7ad96eb64'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg028_austere_command_battle_rule_20260622_190701
...
SELECT
  'pg028_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
