\echo 'PG825B trusted rule oracle_hash backfill postcheck'

WITH backup AS (
  SELECT *
  FROM manaloom_deploy_audit.pg825b_trusted_rule_oracle_hash_backfill_new_server_20260712
),
remaining AS (
...
SELECT
  (SELECT rows FROM remaining) AS verified_auto_rules_missing_oracle_hash,
