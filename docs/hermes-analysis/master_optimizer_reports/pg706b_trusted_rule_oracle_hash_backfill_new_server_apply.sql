...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg706b_trusted_rule_oracle_hash_backfill_20260710_151900;

CREATE TABLE manaloom_deploy_audit.pg706b_trusted_rule_oracle_hash_backfill_20260710_151900 AS
SELECT r.*
...
BEGIN
  ...
  IF v_bad_count > 0 THEN
    RAISE EXCEPTION 'PG706B abort: found % trusted executable rules with missing hash but no safe cards.oracle_text source', v_bad_count;
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(E'\n', nullif(r.notes, ''), 'PG706B: oracle_hash backfilled from cards.oracle_text for trusted executable rule.'),
    updated_at = now(),
    last_seen_at = now()
  FROM public.cards c
