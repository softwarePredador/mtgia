...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg077_seething_song_metadata_restore_20260623_062422') IS NOT NULL THEN
    RAISE EXCEPTION 'PG077 Seething Song metadata restore backup table already exists';
  END IF;
END $$;

CREATE TABLE manaloom_deploy_audit.pg077_seething_song_metadata_restore_20260623_062422 AS
...
BEGIN
  ...
  IF v_card <> 1 THEN
    RAISE EXCEPTION 'PG077 Seething Song metadata restore precondition failed: expected 1 target card with current oracle hash, got %', v_card;
  END IF;
  IF v_rule <> 1 THEN
    RAISE EXCEPTION 'PG077 Seething Song metadata restore precondition failed: expected 1 target runtime row with current oracle hash, got %', v_rule;
  END IF;
  IF v_missing_metadata <> 1 THEN
    RAISE EXCEPTION 'PG077 Seething Song metadata restore precondition failed: expected 1 metadata-deficient row, got %', v_missing_metadata;
  END IF;
END $$;
...
SET
  ...
  notes = concat_ws(
    E'\n',
    nullif(notes, ''),
    'PG077 addendum: restored PG058/PG073/PG075 Seething Song red ritual metadata after PG077 hash restore exposed a mana_color_status regression; no executor semantic change.'
  )
WHERE normalized_name = 'seething song'
  AND logical_rule_key = 'battle_rule_v1:3eb15dc581c6b913158f9b63c023f3d7'
  AND execution_status = 'auto';
