...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg772b_trusted_rule_oracle_hash_backfill_new_server_20260711;

CREATE TABLE manaloom_deploy_audit.pg772b_trusted_rule_oracle_hash_backfill_new_server_20260711 AS
SELECT
  r.*,
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(
      E'\n',
      nullif(r.notes, ''),
      'PG772B contract backfill: oracle_hash restored from cards.oracle_text for trusted executable rules after PG772 fixed draw/gain-life.'
    )
  FROM public.cards c
