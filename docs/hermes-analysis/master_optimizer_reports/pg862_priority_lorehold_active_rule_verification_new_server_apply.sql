...
CREATE TABLE manaloom_deploy_audit.pg862_priority_lorehold_active_rule_verification_new_server_20260713 AS
SELECT r.*
...
BEGIN
  ...
  IF v_bad IS NOT NULL THEN
    RAISE EXCEPTION 'PG862 abort: target rows are not exact active/auto matches: %', v_bad;
  END IF;
END $$;
...
updated AS (
  ...
  SET
    ...
    reviewed_by = 'codex-pg862-active-rule-verification',
    reviewed_at = now(),
    updated_at = now(),
    notes = concat_ws(E'\n', nullif(r.notes, ''), 'PG862: promoted from active/auto to verified/auto after focused priority runtime proof, current Oracle hash check, and PG861 contract audits.')
  FROM target t
