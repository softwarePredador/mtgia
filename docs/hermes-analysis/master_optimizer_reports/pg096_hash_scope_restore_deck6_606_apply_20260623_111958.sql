...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg096_hash_scope_restore_deck6_606_20260623_111958') IS NOT NULL THEN
    RAISE EXCEPTION 'PG096 backup table already exists';
  END IF;
END $$;

CREATE TABLE manaloom_deploy_audit.pg096_hash_scope_restore_deck6_606_20260623_111958 AS
SELECT *
...
SET effect_json = tr.expected_effect_json,
    ...
    notes = concat_ws(
      E'\n',
      NULLIF(cbr.notes, ''),
      'PG096 hash/scope restore from versioned PG094 canonical package after fresh PG->SQLite audit exposed PostgreSQL metadata drift in deck 6/606 shared rules.'
    ),
    reviewed_by = 'codex-pg096',
    reviewed_at = now(),
    updated_at = now(),
    last_seen_at = now()
FROM target_rows tr
