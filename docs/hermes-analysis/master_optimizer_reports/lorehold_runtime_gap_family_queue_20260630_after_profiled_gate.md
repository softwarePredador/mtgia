...
- Generated at: `2026-06-30T05:18:07Z`
...
- Promotion lanes: `{"batch_metadata_candidate_requires_pg_precheck": 8, "mapper_metadata_or_test_scenario_required": 30, "split_family_scope_review_required": 23}`
...
| ---: | --- | ---: | --- | --- | --- | --- | --- |
| 1 | `manual_model` | 30 | `manual_model_required` | `not_batch_safe` | `{"contextual": 19, "early_mana": 3, "finisher_or_big_spell": 4, "hand_filter": 4}` | `{"mapper_metadata_or_test_scenario_required": 30}` | manual Oracle/reference review |
...
| 3 | `recursion` | 5 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"contextual": 4, "early_mana": 1}` | `{"split_family_scope_review_required": 5}` | graveyard target selection, zone movement to hand or battlefield, and replacement-cost annotations |
| 4 | `draw_engine` | 3 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"contextual": 1, "early_mana": 1, "hand_filter": 1}` | `{"split_family_scope_review_required": 3}` | static and activated draw-engine bookkeeping with delayed card movement |
| 5 | `free_cast` | 3 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"contextual": 3}` | `{"batch_metadata_candidate_requires_pg_precheck": 1, "split_family_scope_review_required": 2}` | cast-without-paying resolvers with source-zone, timing-bypass, and replacement-destination bookkeeping |
| 6 | `board_wipe_choice` | 3 | `runtime_family_required` | `implement_family_before_metadata_batch` | `{"contextual": 1, "hand_filter": 1, "pressure_absorber_or_protection": 1}` | `{"split_family_scope_review_required": 3}` | multi-player choice/wipe/sacrifice resolution |
| 7 | `targeted_protection` | 2 | `runtime_supported_family` | `metadata_batch_after_pg_precheck` | `{"contextual": 1, "protection_window": 1}` | `{"split_family_scope_review_required": 2}` | targeted own-creature protection grant and cleanup-aware target legality |
| 8 | `topdeck_play` | 2 | `runtime_supported_family` | `metadata_batch_after_pg_precheck` | `{"hand_filter": 2}` | `{"batch_metadata_candidate_requires_pg_precheck": 1, "split_family_scope_review_required": 1}` | top-of-library visibility and permission to play cards from library under board-state conditions |
| 9 | `life_total_change` | 1 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"hand_filter": 1}` | `{"split_family_scope_review_required": 1}` | life total gain, doubling, and set-to-derived-value effects |
| 10 | `passive` | 1 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"early_mana": 1}` | `{"split_family_scope_review_required": 1}` | static battlefield annotation and passive support execution |
| 11 | `ramp_permanent` | 1 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"early_mana": 1}` | `{"split_family_scope_review_required": 1}` | battlefield mana artifacts and triggered resource permanents |
| 12 | `tutor` | 1 | `runtime_family_partially_supported_review_required` | `split_by_scope_before_metadata_batch` | `{"contextual": 1}` | `{"split_family_scope_review_required": 1}` | library search, constrained target selection, and zone movement to hand, top, graveyard, or battlefield |
| 13 | `static_damage_modifier` | 1 | `runtime_supported_family` | `metadata_batch_after_pg_precheck` | `{"finisher_or_big_spell": 1}` | `{"batch_metadata_candidate_requires_pg_precheck": 1}` | continuous replacement effects that modify damage amounts by source controller and target ownership |
...
- `static_ability`: 5 cards; lanes `{"contextual": 4, "hand_filter": 1}`; samples `Blood Moon, Rem Karolus, Stalwart Slayer, Sawhorn Nemesis, Serra Ascendant, The Walls of Ba Sing Se`
- `triggered_ability`: 4 cards; lanes `{"contextual": 2, "early_mana": 1, "finisher_or_big_spell": 1}`; samples `Ancient Copper Dragon, Ancient Gold Dragon, Possibility Storm, Slickshot Show-Off`
...
- `targeting`: 2 cards; lanes `{"finisher_or_big_spell": 1, "hand_filter": 1}`; samples `Beacon of Immortality, Chandra's Ignition`
...
- `targeting;mill;activated_ability`: 3 cards; lanes `{"contextual": 2, "early_mana": 1}`; samples `Codex Shredder, Perpetual Timepiece, Wand of Vertebrae`
...
- `targeting`: 1 cards; lanes `{"hand_filter": 1}`; samples `Single Combat`
...
- `static_ability`: 1 cards; lanes `{"early_mana": 1}`; samples `Vedalken Orrery`
