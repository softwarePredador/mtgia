...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg767b_trusted_rule_oracle_hash_backfill_20260711;

CREATE TABLE manaloom_deploy_audit.pg767b_trusted_rule_oracle_hash_backfill_20260711 AS
SELECT
  r.*,
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(
      E'\n',
      nullif(r.notes, ''),
      'PG767B contract backfill: oracle_hash restored from cards.oracle_text for trusted executable rule after PG767 contract audit.'
    )
  FROM public.cards c
