\echo 'PG845B trusted rule oracle_hash backfill postcheck'
...
backup AS (
  SELECT count(*) AS rows
  FROM manaloom_deploy_audit.pg845b_trusted_rule_oracle_hash_backfill_new_server_20260712
),
updated AS (
  SELECT count(*) AS rows
  FROM card_battle_rules br
  JOIN manaloom_deploy_audit.pg845b_trusted_rule_oracle_hash_backfill_new_server_20260712 b
