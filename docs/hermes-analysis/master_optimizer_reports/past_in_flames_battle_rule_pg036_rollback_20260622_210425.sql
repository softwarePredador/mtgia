...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg036_past_in_flames_battle_rule_20260622_210425') IS NULL THEN
    RAISE EXCEPTION 'PG036 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'past in flames'
  AND (
    logical_rule_key = 'battle_rule_v1:ccdb2d362690ed2c1ef32711b42e51be'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg036_past_in_flames_battle_rule_20260622_210425
...
SELECT
  'pg036_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
