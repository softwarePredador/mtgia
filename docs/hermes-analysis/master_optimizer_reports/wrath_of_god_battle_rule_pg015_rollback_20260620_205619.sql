...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg015_wrath_of_god_20260620_205619') IS NULL THEN
    RAISE EXCEPTION 'PG015 rollback backup table is missing';
  END IF;
END $$;
...
WHERE lower(card_name) = 'wrath of god'
  AND logical_rule_key IN (
    ...
    FROM manaloom_deploy_audit.pg015_wrath_of_god_20260620_205619
...
SELECT
  'pg015_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
