...
-- Expected after apply: resolved_cache_rows = 7.
SELECT COUNT(*) AS resolved_cache_rows
FROM card_oracle_cache
WHERE normalized_name IN ('anointed procession', 'brain in a jar', 'entreat the angels', 'haze of rage', 'late to dinner', 'miraculous recovery', 'strata scythe')
...
WHERE coc.normalized_name IN ('anointed procession', 'brain in a jar', 'entreat the angels', 'haze of rage', 'late to dinner', 'miraculous recovery', 'strata scythe')
