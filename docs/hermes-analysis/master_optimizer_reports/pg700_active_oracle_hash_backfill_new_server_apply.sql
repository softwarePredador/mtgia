...
CREATE TABLE manaloom_deploy_audit.pg700_active_oracle_hash_backfill_new_server_20260709 AS
SELECT
  br.normalized_name,
  br.card_id,
  br.card_name,
  ...
  UPDATE card_battle_rules br
     SET oracle_hash = b.new_oracle_hash,
         notes = concat_ws(
           ' | ',
           nullif(br.notes, ''),
           'pg700 active oracle_hash backfill from cards.oracle_text md5'
         ),
         updated_at = now()
