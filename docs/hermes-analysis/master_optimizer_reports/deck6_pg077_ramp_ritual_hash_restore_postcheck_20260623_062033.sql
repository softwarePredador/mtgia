...
SELECT
  ...
  count(*) FILTER (WHERE cbr.oracle_hash = t.expected_oracle_hash) AS expected_hash_rows,
  count(*) FILTER (WHERE cbr.oracle_hash IS NULL OR cbr.oracle_hash = '') AS target_missing_hash_rows,
  (
    SELECT count(*)
    FROM manaloom_deploy_audit.pg077_ramp_ritual_hash_restore_20260623_062033
  ) AS backup_rows
