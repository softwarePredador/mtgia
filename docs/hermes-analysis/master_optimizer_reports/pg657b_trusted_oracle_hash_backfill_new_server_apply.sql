...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg657b_trusted_oracle_hash_backfill_20260708;

CREATE TABLE manaloom_deploy_audit.pg657b_trusted_oracle_hash_backfill_20260708 AS
SELECT r.*
...
BEGIN
  ...
  IF v_bad_count > 0 THEN
    RAISE EXCEPTION 'PG657b abort: found % trusted executable rules with missing hash but no safe card oracle_text source', v_bad_count;
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(E'\n', nullif(r.notes, ''), 'PG657b: oracle_hash repaired from cards.oracle_text for trusted executable rule after PG657 contract audit surfaced missing hashes.'),
    updated_at = now(),
    last_seen_at = now()
  FROM public.cards c
