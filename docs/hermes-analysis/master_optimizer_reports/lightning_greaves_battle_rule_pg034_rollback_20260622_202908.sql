...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg034_lightning_greaves_battle_rule_20260622_202908') IS NULL THEN
    RAISE EXCEPTION 'PG034 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'lightning greaves'
  AND (
    logical_rule_key = 'battle_rule_v1:5ea7f2a8349a93ea46e05b60ee8cdaac'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg034_lightning_greaves_battle_rule_20260622_202908
...
SELECT
  'pg034_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
