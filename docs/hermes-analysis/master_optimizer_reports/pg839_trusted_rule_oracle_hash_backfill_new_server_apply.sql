\echo 'PG839 trusted verified/auto oracle_hash backfill apply'

BEGIN;

CREATE SCHEMA IF NOT EXISTS manaloom_deploy_audit;

DROP TABLE IF EXISTS manaloom_deploy_audit.pg839_trusted_rule_oracle_hash_backfill_new_server_20260712;

CREATE TABLE manaloom_deploy_audit.pg839_trusted_rule_oracle_hash_backfill_new_server_20260712 AS
SELECT
  br.*,
  md5(c.oracle_text) AS pg839_new_oracle_hash,
  NOW() AS pg839_backed_up_at
...
updated AS (
  ...
  SET
    ...
    notes = CONCAT_WS(
      E'\n',
      NULLIF(br.notes, ''),
      'PG839: backfilled oracle_hash for trusted verified/auto rule from current cards.oracle_text md5 on 2026-07-12.'
    )
  FROM target
...
SELECT
  (SELECT COUNT(*) FROM manaloom_deploy_audit.pg839_trusted_rule_oracle_hash_backfill_new_server_20260712) AS backup_rows,
