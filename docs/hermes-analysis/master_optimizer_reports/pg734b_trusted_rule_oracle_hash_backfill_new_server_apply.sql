...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg734b_trusted_rule_oracle_hash_backfill_new_server_20260711;

CREATE TABLE manaloom_deploy_audit.pg734b_trusted_rule_oracle_hash_backfill_new_server_20260711 AS
SELECT
  br.card_id,
  br.normalized_name,
  br.card_name,
...
updated AS (
  ...
  SET
    ...
    updated_at = NOW(),
    notes = CONCAT_WS(' | ', NULLIF(br.notes, ''), 'pg734b trusted_rule_oracle_hash_backfill_new_server')
  FROM target
