...
SELECT
  ...
  count(*) FILTER (WHERE card_id IS NOT NULL) AS target_card_rows,
  count(*) FILTER (WHERE target_oracle_hash = expected_oracle_hash) AS expected_oracle_hash_rows,
  count(*) FILTER (WHERE review_status IN ('verified', 'active') AND execution_status IN ('auto', 'executable')) AS trusted_runtime_rows,
  count(*) FILTER (WHERE nullif(oracle_hash, '') IS NULL) AS rows_missing_oracle_hash,
  count(*) FILTER (WHERE oracle_hash IS NOT NULL AND oracle_hash IS DISTINCT FROM expected_oracle_hash) AS hash_mismatch_rows,
