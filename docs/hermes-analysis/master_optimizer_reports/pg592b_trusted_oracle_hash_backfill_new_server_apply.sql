...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg592b_trusted_oracle_hash_backfill_new_server_backup;

CREATE TABLE manaloom_deploy_audit.pg592b_trusted_oracle_hash_backfill_new_server_backup AS
SELECT
  r.card_id,
  r.logical_rule_key,
  r.card_name,
...
SET
  ...
  notes = concat_ws(
    E'\n',
    nullif(r.notes, ''),
    'PG592B: backfilled oracle_hash from cards.oracle_text md5 for trusted executable rule contract.'
  ),
  updated_at = now()
