...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg040_swords_to_plowshares_battle_rule_20260622_221254') IS NULL THEN
    RAISE EXCEPTION 'PG040 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'swords to plowshares'
  AND (
    logical_rule_key = 'battle_rule_v1:379008f3f03f94258292123453e3041c'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg040_swords_to_plowshares_battle_rule_20260622_221254
...
SELECT
  'pg040_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
