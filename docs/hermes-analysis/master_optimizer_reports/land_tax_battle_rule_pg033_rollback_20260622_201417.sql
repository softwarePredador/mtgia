...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg033_land_tax_battle_rule_20260622_201417') IS NULL THEN
    RAISE EXCEPTION 'PG033 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'land tax'
  AND (
    logical_rule_key = 'battle_rule_v1:e3f5f35c6a9ee4fd8c7b9972c4152bef'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg033_land_tax_battle_rule_20260622_201417
...
SELECT
  'pg033_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
