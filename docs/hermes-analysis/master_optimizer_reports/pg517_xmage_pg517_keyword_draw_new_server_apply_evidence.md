# PG517 Keyword Draw Apply Evidence

Generated on 2026-07-05 for deploy id
`xmage_pg517_keyword_draw_new_server`.

## Scope

PG517 promotes only local-XMage-backed instant/sorcery spells with the exact
fixed pattern:

- target creature gains one supported keyword until end of turn;
...
Those neighbors remain blocked because their XMage source does not match the
single fixed `GainAbilityTargetEffect` plus fixed draw-one source pattern.
...
- Package:
  `docs/hermes-analysis/master_optimizer_reports/pg517_xmage_pg517_keyword_draw_new_server_package.md`
- Manifest:
  `docs/hermes-analysis/master_optimizer_reports/pg517_xmage_pg517_keyword_draw_new_server_manifest.json`
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg517_xmage_pg517_keyword_draw_new_server_precheck.sql`
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg517_xmage_pg517_keyword_draw_new_server_apply.sql`
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg517_xmage_pg517_keyword_draw_new_server_postcheck.sql`
- Rollback SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg517_xmage_pg517_keyword_draw_new_server_rollback.sql`
...
- Postcheck: all five promoted cards have `promoted_rule_rows=1`,
...
- `canonical_snapshot_rows_exported=6031`.
...
- Focused unit suite:
  ...
  reports `837` tests passing.
- Focused runtime smoke without override resolved `Bladebrand` through
  `get_card_effect`, granted `deathtouch` to `Active Bear`, drew `Fresh Card`,
  moved `Bladebrand` to graveyard, and emitted `composite_rule_resolved`.
...
- Candidate exact split: `proposal_count=5`,
...
- `snapshot_has_any_rule=6034`.
- `snapshot_has_verified_rule=4789`.

## Decision

PG517 is applied, synced, and validated. Do not rebuild it from the pre-PG517
