...
- generated_at: `2026-07-04T21:09:33Z`
- deck_id: `607`
...
- total cards: `100`
- reports loaded: `6/6`
- unique candidates considered: `137`
- gate-ready candidates: `0`
- hypotheses needing named cut/gate: `95`
- blocked/rejected rows: `38`
- role mapping watch items: `5`
...
| --- | --- | --- | --- |
| `card_value` | `card_value_priority_keep_607_with_role_watch_items` | `keep_607` | `docs/hermes-analysis/master_optimizer_reports/lorehold_card_value_priority_synthesis_20260704_learning.json` |
...
| `mana` | `mana_sequence_no_direct_auto_upgrade_current_607` | `keep_607` | `docs/hermes-analysis/master_optimizer_reports/lorehold_mana_sequence_policy_synthesis_20260704_learning.json` |
...
## Role/Tag Watch Before New Cuts

- `Deflecting Swat`: tag `draw`, lane `protection_resilience`, watch `draw_tag_masks_interaction_or_protection_function`.
- `Emeria's Call // Emeria, Shattered Skyclave`: tag `unknown`, lane `protection_resilience`, watch `unmapped_primary_tag:unknown`.
- `Promise of Loyalty`: tag `draw`, lane `protection_resilience`, watch `draw_tag_masks_interaction_or_protection_function`.
- `Redirect Lightning`: tag `draw`, lane `interaction_removal`, watch `draw_tag_masks_interaction_or_protection_function`.
- `Tragic Arrogance`: tag `unknown`, lane `unknown`, watch `unmapped_primary_tag:unknown`.

## Promotion Gate Checklist

- `baseline_deck_shape`: passed `true`; 100 cards, commanders=['Lorehold, the Historian']
- `all_axis_reports_loaded`: passed `true`; card_value, interaction, mana, payoff, selection, staple
- `axis_decisions_do_not_promote_candidate`: passed `true`; gate_ready_candidate_count=0
- `role_tag_watch_before_new_cuts`: passed `false` required-before-promotion; role_mapping_watch_items=5
