...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg012_flame_wave_battle_rule_20260620_195700') IS NULL THEN
    RAISE EXCEPTION 'PG012 rollback backup table is missing';
  END IF;
END $$;
...
WHERE lower(card_name) = 'flame wave'
  AND (
    logical_rule_key = 'battle_rule_v1:7a932c36df79f966f5f144a3b3a87d84'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg012_flame_wave_battle_rule_20260620_195700
...
SELECT
  'pg012_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
