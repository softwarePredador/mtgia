...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg037_path_to_exile_battle_rule_20260622_212057') IS NULL THEN
    RAISE EXCEPTION 'PG037 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'path to exile'
  AND (
    ...
    OR logical_rule_key IN (
      ...
      FROM manaloom_deploy_audit.pg037_path_to_exile_battle_rule_20260622_212057
...
SELECT
  'pg037_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
