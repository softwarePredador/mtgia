WITH target AS (
  SELECT
    ...
    'Dawn''s Truce'::text AS card_name,
    ...
    '{"category":"protection","effect":"gift_hexproof_indestructible","timing":"instant"}'::jsonb AS expected_deck_role_json
),
card_match AS (
  SELECT
    c.id,
...
SELECT
  ...
  (SELECT count(*) FROM public.card_battle_rules r JOIN target t ON r.normalized_name = t.normalized_name AND r.effect_json @> '{"effect":"indestructible"}'::jsonb AND r.execution_status <> 'disabled') AS rows_still_claiming_plain_indestructible,
  ...
  to_regclass('manaloom_deploy_audit.pg103_dawns_truce_gift_hexproof_indestructible_20260623_133226') IS NOT NULL AS backup_table_already_exists;
