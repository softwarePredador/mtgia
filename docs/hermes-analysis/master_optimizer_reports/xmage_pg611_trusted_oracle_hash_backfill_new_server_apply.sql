...
BEGIN
  ...
  FROM pg611_trusted_oracle_hash_backfill_target;
  IF target_count <> 44 THEN
    RAISE EXCEPTION 'expected 44 hash backfill targets, got %', target_count;
  END IF;

  SELECT count(*) INTO candidate_count
  FROM card_battle_rules r
  JOIN pg611_trusted_oracle_hash_backfill_target t USING (normalized_name, logical_rule_key)
  ...
  IF candidate_count <> 44 THEN
    RAISE EXCEPTION 'expected 44 eligible hash backfill rows, got %', candidate_count;
  END IF;
END $$;
...
BEGIN
  ...
  IF remaining_count <> 0 THEN
    RAISE EXCEPTION 'expected no remaining empty oracle_hash rows, got %', remaining_count;
  END IF;
END $$;
