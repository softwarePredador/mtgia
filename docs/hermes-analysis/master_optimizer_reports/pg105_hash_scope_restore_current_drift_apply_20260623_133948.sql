...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg105_hash_scope_restore_current_drift_20260623_133948') IS NOT NULL THEN
    RAISE EXCEPTION 'PG105 backup table already exists';
  END IF;
END $$;

CREATE TABLE manaloom_deploy_audit.pg105_hash_scope_restore_current_drift_20260623_133948 AS
SELECT *
...
updated AS (
  ...
  SET
    ...
    notes = concat_ws(
      E'\n',
      NULLIF(cbr.notes, ''),
      'PG105: restored hash/scope metadata for 12 canonical runtime rules after current PG drift was detected while validating PG099; no deck swap.'
    ),
    reviewed_by = 'codex-pg105',
    reviewed_at = now(),
    updated_at = now(),
    last_seen_at = now()
  FROM target_rows tr
