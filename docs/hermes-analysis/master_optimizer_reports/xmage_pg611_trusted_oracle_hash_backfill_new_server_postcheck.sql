...
checked AS (
  SELECT r.card_name, r.normalized_name, r.logical_rule_key, r.oracle_hash,
...
SELECT count(*) AS target_rows,
       count(*) FILTER (WHERE oracle_hash = expected_oracle_hash) AS matching_oracle_hash_rows,
       count(*) FILTER (WHERE coalesce(oracle_hash, '') = '') AS empty_oracle_hash_rows
FROM checked;
