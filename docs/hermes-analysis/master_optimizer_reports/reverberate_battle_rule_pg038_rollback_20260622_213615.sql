...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg038_reverberate_battle_rule_20260622_213615') IS NULL THEN
    RAISE EXCEPTION 'PG038 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'reverberate'
  AND (
    logical_rule_key = 'battle_rule_v1:0269136edf067f696c8576740b720e14'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg038_reverberate_battle_rule_20260622_213615
...
SELECT
  'pg038_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
