...
SELECT
  ...
  count(*) FILTER (WHERE effect_json ? 'battle_model_scope') AS target_scoped_before_rows,
  count(*) FILTER (WHERE effect_json = expected_effect_json) AS target_effect_already_expected_rows,
  count(*) FILTER (WHERE deck_role_json = expected_deck_role_json) AS target_deck_role_already_expected_rows,
  (SELECT count(*) FROM scoped_missing_hash) AS scoped_trusted_auto_missing_hash_rows,
  to_regclass('manaloom_deploy_audit.pg079_deck606_runtime_semantics_20260623_044955') IS NOT NULL AS backup_table_already_exists
FROM target_rows;
