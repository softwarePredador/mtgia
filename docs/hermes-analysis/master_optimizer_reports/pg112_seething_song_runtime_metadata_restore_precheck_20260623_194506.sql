...
SELECT
  ...
  (SELECT count(*) FROM row_match WHERE nullif(oracle_hash, '') IS NULL
      OR effect_json->>'mana_color_status' IS DISTINCT FROM 'abstracted_to_generic_pool_runtime'
      OR effect_json->>'oracle_runtime_scope' IS DISTINCT FROM 'single_shot_red_ritual_runtime_generic_pool_color_annotation'
      OR effect_json->>'pg058_l3b_simple_red_ritual_family' IS DISTINCT FROM 'deck6_simple_red_rituals') AS metadata_deficient_rows,
  (SELECT to_regclass('manaloom_deploy_audit.pg112_seething_song_runtime_metadata_restore_20260623_194506') IS NOT NULL) AS backup_table_exists;
