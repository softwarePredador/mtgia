...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg788b_trusted_rule_oracle_hash_backfill_new_server_20260711;

CREATE TABLE manaloom_deploy_audit.pg788b_trusted_rule_oracle_hash_backfill_new_server_20260711 AS
SELECT
  r.*,
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(
      E'\n',
      nullif(r.notes, ''),
      'PG788B contract backfill: oracle_hash restored from cards.oracle_text for trusted executable rules after PG788 existing safe scopes.'
    )
  FROM public.cards c
