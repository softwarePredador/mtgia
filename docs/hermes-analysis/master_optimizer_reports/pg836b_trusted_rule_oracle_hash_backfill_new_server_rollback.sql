\echo 'PG836B trusted rule oracle_hash backfill rollback'

BEGIN;

WITH backup AS (
  SELECT *
  FROM manaloom_deploy_audit.pg836b_trusted_rule_oracle_hash_backfill_new_server_20260712_1822
),
restored AS (
  ...
  SET
    ...
    notes = CONCAT_WS(
      E'\n',
      NULLIF(br.notes, ''),
      'PG836B rollback: restored oracle_hash from deploy audit backup.'
    )
  FROM backup
