...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg101_hash_scope_restore_current_drift_20260623_094218') IS NOT NULL THEN
    RAISE EXCEPTION 'PG101 backup table already exists';
  END IF;
END $$;

CREATE TABLE manaloom_deploy_audit.pg101_hash_scope_restore_current_drift_20260623_094218 AS
SELECT *
...
target_rows AS (
  SELECT e.*, cbr.card_id, md5(coalesce(c.oracle_text, '')) AS current_card_hash
  FROM expected e
...
precheck AS (
  SELECT
    ...
    count(*) FILTER (WHERE current_card_hash = expected_oracle_hash) AS hash_rows
  FROM target_rows
),
updated AS (
  UPDATE card_battle_rules cbr
  SET
    effect_json = tr.expected_effect_json,
    ...
    notes = concat_ws(
      E'\n',
      NULLIF(cbr.notes, ''),
      'PG101: restored hash/scope metadata for 12 canonical runtime rules after current PG drift was detected while validating PG099; no deck swap.'
    ),
    reviewed_by = 'codex-pg101',
    reviewed_at = now(),
    updated_at = now(),
    last_seen_at = now()
  FROM target_rows tr
