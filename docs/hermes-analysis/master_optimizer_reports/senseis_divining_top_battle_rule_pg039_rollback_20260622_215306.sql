...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg039_senseis_top_battle_rule_20260622_215306') IS NULL THEN
    RAISE EXCEPTION 'PG039 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name = 'sensei''s divining top'
  AND (
    logical_rule_key = 'battle_rule_v1:70c8478871f352b46cee1af296117951'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg039_senseis_top_battle_rule_20260622_215306
...
SELECT
  'pg039_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
