...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg077_ramp_ritual_hash_restore_20260623_062033') IS NOT NULL THEN
    RAISE EXCEPTION 'PG077 ramp/ritual hash restore backup table already exists';
  END IF;
END $$;
...
CREATE TABLE manaloom_deploy_audit.pg077_ramp_ritual_hash_restore_20260623_062033 AS
SELECT now() AS backed_up_at, to_jsonb(cbr) AS payload
FROM card_battle_rules cbr
JOIN pg077_hash_targets t
...
BEGIN
  ...
  IF v_cards <> 5 THEN
    RAISE EXCEPTION 'PG077 ramp/ritual hash restore precondition failed: expected 5 target cards with current oracle hashes, got %', v_cards;
  END IF;
  IF v_rules <> 5 THEN
    RAISE EXCEPTION 'PG077 ramp/ritual hash restore precondition failed: expected 5 target rules, got %', v_rules;
  END IF;
END $$;
...
SET
  ...
  confidence = greatest(cbr.confidence, 0.970),
  ...
  notes = concat_ws(
    E'\n',
    nullif(cbr.notes, ''),
    'PG077 addendum: restored oracle_hash provenance exposed missing by PG077 PostgreSQL-to-SQLite sync; no semantic runtime change.'
  )
FROM pg077_hash_targets t
