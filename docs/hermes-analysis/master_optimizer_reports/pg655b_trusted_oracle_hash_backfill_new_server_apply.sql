...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg655b_trusted_oracle_hash_backfill_20260708;

CREATE TABLE manaloom_deploy_audit.pg655b_trusted_oracle_hash_backfill_20260708 AS
SELECT r.*
...
BEGIN
  ...
  IF v_bad_count > 0 THEN
    RAISE EXCEPTION 'PG655b abort: found % trusted executable rules with missing hash but no safe card oracle_text source', v_bad_count;
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(E'\n', nullif(r.notes, ''), 'PG655b: oracle_hash repaired from cards.oracle_text for trusted executable rule after PG655 readiness hash lane surfaced.'),
    updated_at = now(),
    last_seen_at = now()
  FROM public.cards c
