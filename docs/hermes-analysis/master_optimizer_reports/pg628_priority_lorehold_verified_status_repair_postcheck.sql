-- PG628 postcheck: expected after apply:
-- target_verified_auto_rows = 4
-- requested_cards_snapshot_verified_after = 4/4
...
SELECT
  'requested_cards_snapshot_verified_after',
  COUNT(*)::text || '/' || (SELECT COUNT(*) FROM requested_cards)::text
FROM card_pick p
JOIN card_intelligence_snapshot cis ON cis.card_id = p.card_id
