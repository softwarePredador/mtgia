...
metrics AS (
  SELECT
    ...
    (SELECT count(*) FROM global_expected WHERE lane = 'deterministic_semantic_v2') AS semantic_global_expected_count,
    (SELECT encode(digest(string_agg(card_id::text, E'\n' ORDER BY card_id::text), 'sha256'), 'hex') FROM global_expected WHERE lane = 'deterministic_semantic_v2') AS semantic_global_expected_sha256,
    (SELECT count(*) FROM semantic_snapshot_owners) AS semantic_snapshot_owner_count,
    ...
    (SELECT count(*) FROM deferred_missing_semantic) AS deferred_semantic_backfill_count,
    (SELECT encode(digest(string_agg(card_id::text, E'\n' ORDER BY card_id::text), 'sha256'), 'hex') FROM deferred_missing_semantic) AS deferred_semantic_backfill_sha256,
    (SELECT count(*) FROM deferred_missing_semantic m JOIN public.card_semantic_tags_v2 s ON s.card_id=m.card_id AND s.source='deterministic_semantic_v2') AS semantic_source_conflicts,
...
SELECT
  ...
  CASE
    WHEN heuristic_cards_scanned = 33841
     ...
     AND semantic_global_expected_count = 736
     AND semantic_global_expected_sha256 = '512cb67eca26b4c86d4555fcf14cae19e6e9f0a9bd24239db0d4cc6caa49418c'
     AND semantic_snapshot_owner_count = 24181
     AND semantic_expected_count = 684
     AND semantic_expected_sha256 = '573c94fad8b4ba9d9789daadd506e4ce2b0143dfa0d049eb8687c4c8cd90e8bd'
     ...
     AND semantic_target_count = 1557
     AND semantic_snapshot_count = 1557
     AND deferred_semantic_backfill_count = 52
     AND deferred_semantic_backfill_sha256 = '4f29cbcbbdaa9a10bf285ff808c40ab8f3026367a2b3bc873fd51424cad5b199'
     ...
     AND to_regclass('manaloom_deploy_audit.pg873_sac_outlet_deferred_semantic_20260715') IS NULL
