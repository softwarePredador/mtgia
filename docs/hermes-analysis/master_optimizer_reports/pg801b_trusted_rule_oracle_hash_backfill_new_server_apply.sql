\echo 'PG801B trusted rule oracle_hash backfill apply'

BEGIN;

CREATE SCHEMA IF NOT EXISTS manaloom_deploy_audit;

DROP TABLE IF EXISTS manaloom_deploy_audit.pg801b_trusted_rule_oracle_hash_backfill_new_server_20260712;

CREATE TABLE manaloom_deploy_audit.pg801b_trusted_rule_oracle_hash_backfill_new_server_20260712 AS
SELECT
  br.*,
  md5(c.oracle_text) AS pg801b_new_oracle_hash,
  NOW() AS pg801b_backed_up_at
...
updated AS (
  ...
  SET
    ...
    notes = CONCAT_WS(
      E'\n',
      NULLIF(br.notes, ''),
      'PG801B trusted rule oracle_hash backfill from cards.oracle_text on 2026-07-12.'
    )
  FROM target
...
SELECT
  (SELECT COUNT(*) FROM manaloom_deploy_audit.pg801b_trusted_rule_oracle_hash_backfill_new_server_20260712) AS backup_rows,
