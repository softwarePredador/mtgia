...
SELECT
  ...
  ) AS rejected_generated_shadow_rows,
  count(*) FILTER (
    WHERE review_status = 'needs_review'
       OR execution_status = 'review_only'
  ) AS remaining_review_shadow_rows,
  (SELECT count(*)
   FROM manaloom_deploy_audit.pg248_deck606_land_shadow_cleanup_20260628) AS backup_rows
FROM target_rows;
...
SELECT
  ...
  r.execution_status,
  r.effect_json,
  right(r.notes, 240) AS notes_tail
FROM target_cards t
JOIN public.cards c
  ON lower(c.name) = lower(t.card_name)
JOIN public.card_battle_rules r
...
ORDER BY c.name, r.source, r.logical_rule_key;
