...
SELECT
  ...
  count(*) FILTER (WHERE coalesce(r.effect_json, '{}'::jsonb) @> t.effect_patch) AS target_effect_patch_match_rows,
  count(*) FILTER (WHERE r.confidence = t.expected_confidence) AS target_confidence_match_rows,
...
SELECT count(*) AS backup_rows FROM manaloom_deploy_audit.pg090_rule_hash_scope_restore_20260623_062000;
