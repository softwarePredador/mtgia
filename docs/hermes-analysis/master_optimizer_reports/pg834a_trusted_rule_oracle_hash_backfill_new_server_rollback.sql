\echo 'PG834A trusted rule oracle_hash backfill rollback'

BEGIN;

WITH backup AS (
  SELECT *
  FROM manaloom_deploy_audit.pg834a_trusted_rule_oracle_hash_backfill_new_server_20260712
),
restored AS (
  ...
  SET
    ...
    notes = CONCAT_WS(
      E'\n',
      NULLIF(br.notes, ''),
      'PG834A rollback: restored oracle_hash from deploy audit backup.'
    )
  FROM backup
