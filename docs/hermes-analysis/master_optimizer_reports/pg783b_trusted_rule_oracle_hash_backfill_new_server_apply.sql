-- PG783B trusted rule oracle_hash backfill apply.
...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg783b_trusted_rule_oracle_hash_backfill_new_server_20260711;

CREATE TABLE manaloom_deploy_audit.pg783b_trusted_rule_oracle_hash_backfill_new_server_20260711 AS
SELECT br.*
...
WITH updated AS (
  ...
  SET oracle_hash = md5(c.oracle_text),
      notes = concat_ws(
        E'\n',
        NULLIF(br.notes, ''),
        'PG783B trusted rule oracle_hash backfill from cards.oracle_text on 2026-07-11.'
      ),
      updated_at = NOW()
  FROM public.cards c
