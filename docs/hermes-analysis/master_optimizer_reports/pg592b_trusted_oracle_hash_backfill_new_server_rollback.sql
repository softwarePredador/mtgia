...
SET
  ...
  notes = concat_ws(
    E'\n',
    nullif(r.notes, ''),
    'PG592B rollback: restored prior oracle_hash after trusted executable hash backfill.'
  ),
  updated_at = now()
FROM manaloom_deploy_audit.pg592b_trusted_oracle_hash_backfill_new_server_backup b
WHERE r.card_id = b.card_id
  AND r.logical_rule_key = b.logical_rule_key;

SELECT count(*) AS restored_rows
FROM card_battle_rules r
JOIN manaloom_deploy_audit.pg592b_trusted_oracle_hash_backfill_new_server_backup b
