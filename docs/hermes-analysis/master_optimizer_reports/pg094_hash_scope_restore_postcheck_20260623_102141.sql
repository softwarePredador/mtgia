...
target_rows AS (
  SELECT tr.*, cbr.oracle_hash, cbr.effect_json, cbr.confidence, cbr.review_status,
...
SELECT
  ...
  (SELECT count(*) FROM target_rows WHERE oracle_hash = expected_oracle_hash) AS hash_restored_rows,
  (SELECT count(*) FROM target_rows WHERE effect_json = expected_effect_json) AS effect_json_restored_rows,
  (SELECT count(*) FROM target_rows WHERE abs(confidence - expected_confidence) < 0.0001 AND review_status = expected_review_status AND execution_status = expected_execution_status AND rule_version = expected_rule_version) AS status_restored_rows,
  (SELECT count(*) FROM manaloom_deploy_audit.pg094_hash_scope_restore_20260623_102141) AS backup_rows;
