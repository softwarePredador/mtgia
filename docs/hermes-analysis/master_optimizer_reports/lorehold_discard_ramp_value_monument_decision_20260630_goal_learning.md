...
Status: `rejected_current_replacements_keep_monument`.
...
- tested additions: `Cool but Rude` and `Currency Converter`;
...
- `Currency Converter` is still a coherent Lorehold card, but this exact cut is
  not coherent because the total record only ties while the critical Winota
  matchup regresses.
...
- Do not keep testing replacements over `Monument to Endurance` unless the
  candidate is same-lane and can preserve Winota.
