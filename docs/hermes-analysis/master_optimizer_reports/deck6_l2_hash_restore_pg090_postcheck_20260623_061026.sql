...
SELECT
  ...
  count(*) FILTER (WHERE r.execution_status = 'auto' AND r.review_status IN ('active', 'verified')) AS trusted_auto_rows,
  count(*) FILTER (WHERE r.rule_version >= 2) AS rule_version_at_least_2_rows,
  count(*) FILTER (WHERE r.effect_json ? 'battle_model_scope') AS scoped_rows
