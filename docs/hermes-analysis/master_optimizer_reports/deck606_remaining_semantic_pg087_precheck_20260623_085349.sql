...
SELECT
  ...
  'creature'::text AS expected_effect,
  jsonb_build_object(
    'cmc', 2.0,
    'effect', 'creature',
    'power', 2,
...
CREATE TEMP TABLE pg087_deck606_remaining_semantic_cards_resolved AS
SELECT
  c.id,
  c.name,
  c.type_line,
  ...
  t.expected_logical_rule_key,
  t.expected_scope,
  t.expected_effect
FROM pg087_deck606_remaining_semantic_target t
JOIN cards c ON lower(c.name) = t.normalized_name;

CREATE TEMP TABLE pg087_deck606_remaining_semantic_active_rules AS
SELECT c.name, cbr.*
FROM pg087_deck606_remaining_semantic_cards_resolved c
JOIN card_battle_rules cbr ON cbr.card_id = c.id
WHERE cbr.execution_status IS DISTINCT FROM 'disabled';

WITH
trusted_rows AS (
  SELECT ar.*
  FROM pg087_deck606_remaining_semantic_active_rules ar
  WHERE ar.source = 'curated'
    AND ar.review_status = 'verified'
    AND ar.execution_status = 'auto'
),
shadow_rows AS (
  SELECT ar.*
  FROM pg087_deck606_remaining_semantic_active_rules ar
  WHERE ar.source = 'generated'
     OR ar.review_status IN ('needs_review', 'review_only')
     OR ar.execution_status = 'review_only'
),
new_key_conflicts AS (
  SELECT cbr.*
  FROM card_battle_rules cbr
  JOIN pg087_deck606_remaining_semantic_target t
    ON cbr.logical_rule_key = t.expected_logical_rule_key
  WHERE lower(coalesce(cbr.card_name, cbr.normalized_name)) <> t.normalized_name
)
SELECT
  (SELECT count(*) FROM pg087_deck606_remaining_semantic_target) AS expected_target_rules,
  (SELECT count(*) FROM pg087_deck606_remaining_semantic_cards_resolved) AS cards_resolved_rows,
  (SELECT count(*) FROM trusted_rows) AS current_trusted_auto_rows,
  (SELECT count(*) FROM trusted_rows WHERE nullif(oracle_hash, '') IS NULL) AS current_trusted_missing_hash_rows,
  (SELECT count(*) FROM pg087_deck606_remaining_semantic_cards_resolved WHERE computed_oracle_hash = expected_oracle_hash) AS oracle_hash_match_rows,
  (SELECT count(*) FROM shadow_rows) AS active_shadow_rows,
  (SELECT count(*) FROM new_key_conflicts) AS new_key_conflict_rows,
  to_regclass('manaloom_deploy_audit.pg087_deck606_remaining_semantic_20260623_085349') IS NOT NULL AS backup_table_already_exists;
...
SELECT
  ...
  c.mana_cost,
  c.cmc,
  c.computed_oracle_hash,
  c.expected_oracle_hash,
  c.expected_logical_rule_key,
  c.expected_scope,
  c.expected_effect,
  c.oracle_text
FROM pg087_deck606_remaining_semantic_cards_resolved c
...
SELECT
  ...
  ar.execution_status,
  ar.oracle_hash,
  ar.effect_json->>'effect' AS effect,
  ar.effect_json->>'battle_model_scope' AS battle_model_scope,
  ar.effect_json AS effect_json
FROM pg087_deck606_remaining_semantic_active_rules ar
ORDER BY ar.name, ar.source, ar.logical_rule_key;
