...
CREATE TABLE IF NOT EXISTS manaloom_deploy_audit.pg758b_trusted_rule_oracle_hash_backfill_20260711 AS
SELECT
  r.*,
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(
      E'\n',
      nullif(r.notes, ''),
      'PG758B contract backfill: oracle_hash restored from cards.oracle_text for trusted executable rule after PG758 audit.'
    )
  FROM public.cards c
