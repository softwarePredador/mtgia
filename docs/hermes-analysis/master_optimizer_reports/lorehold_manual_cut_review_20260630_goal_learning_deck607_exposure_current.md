...
- Generated at: `2026-06-30T20:47:52Z`
...
- Exposure profiles: `docs/hermes-analysis/master_optimizer_reports/lorehold_card_exposure_profile_20260630_goal_learning_deck607_current.json`
- Safe-cut replanner: `docs/hermes-analysis/master_optimizer_reports/lorehold_safe_cut_replanner_20260630_goal_learning_current.json`
...
- Cut evidence status counts: `{"blocked_by_cut_safety": 8, "blocked_by_prior_rejection": 24, "measured_cut_exposure_needs_same_lane_benchmark": 2, "measured_high_cut_exposure": 5, "never_cut": 29, "same_lane_only": 2, "structural_dependency": 24}`
- Cut evidence action counts: `{"blocked": 61, "commander": 1, "mana_base": 28, "manual_same_lane_only": 2, "requires_same_lane_gate": 2}`
...
| --- | --- | --- | ---: | --- | --- |
| Creative Technique | `same_lane_only` | `requires_same_lane_gate` | 38 | runtime_ready_unexposed | Cut-safety permits only a same-lane win after prior strong-seed regression. |
| Bender's Waterskin | `same_lane_only` | `requires_same_lane_gate` | 266 | ramp_engine | Cut-safety permits only a same-lane win after prior strong-seed regression. |
| Generous Gift | `measured_cut_exposure_needs_same_lane_benchmark` | `manual_same_lane_only` | 42 | spot_removal | Replay profile measured 42 deduplicated exposures, so this is not a blind low-use cut.; Measured role is spot_removal.; Exposure signals: miracle_hit, paid_cast_exposure, spot_removal. |
| Monument to Endurance | `measured_cut_exposure_needs_same_lane_benchmark` | `manual_same_lane_only` | 56 | discard_ramp_value | Replay profile measured 56 deduplicated exposures, so this is not a blind low-use cut.; Measured role is discard_ramp_value.; Exposure signals: discard_payoff, paid_cast_exposure, ramp_engine. |

## Protected By Measured Exposure

| Card | Exposure | Role | Reasons |
| --- | ---: | --- | --- |
| Path to Exile | 145 | spot_removal | Replay profile measured 145 deduplicated exposures for this cut slot.; Measured role is spot_removal.; Exposure signals: miracle_hit, paid_cast_exposure, spot_removal. |
| Swords to Plowshares | 234 | spot_removal | Replay profile measured 234 deduplicated exposures for this cut slot.; Measured role is spot_removal.; Exposure signals: miracle_hit, paid_cast_exposure, spot_removal. |
| Esper Sentinel | 456 | draw_filter_value | Replay profile measured 456 deduplicated exposures for this cut slot.; Measured role is draw_filter_value.; Exposure signals: draw_filter_value, paid_cast_exposure, tutor_target. |
| Smothering Tithe | 854 | ramp_engine | Replay profile measured 854 deduplicated exposures for this cut slot.; Measured role is ramp_engine.; Exposure signals: discard_or_rummage_context, paid_cast_exposure, ramp_engine. |
| Sensei's Divining Top | 2972 | runtime_ready_unexposed | Replay profile measured 2972 deduplicated exposures for this cut slot.; Measured role is runtime_ready_unexposed.; Exposure signals: paid_cast_exposure. |
