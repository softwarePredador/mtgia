...
WITH expected(normalized_name, card_name, logical_rule_key, expected_oracle_hash, expected_confidence, expected_review_status, expected_execution_status, expected_rule_version, expected_effect_json) AS (
  VALUES
...
target_rows AS (
  SELECT e.*, cbr.oracle_hash, cbr.effect_json, cbr.confidence, cbr.review_status,
         cbr.execution_status, cbr.rule_version, md5(coalesce(c.oracle_text, '')) AS current_card_hash
  FROM expected e
...
SELECT
  ...
  count(*) FILTER (WHERE current_card_hash = expected_oracle_hash) AS current_card_hash_match_rows,
  ...
  to_regclass('manaloom_deploy_audit.pg105_hash_scope_restore_current_drift_20260623_133948') IS NOT NULL AS backup_table_already_exists
FROM target_rows;
