...
DROP TABLE IF EXISTS manaloom_deploy_audit.pg652b_trusted_oracle_hash_backfill_20260708;

CREATE TABLE manaloom_deploy_audit.pg652b_trusted_oracle_hash_backfill_20260708 AS
SELECT r.*
...
BEGIN
  ...
  IF v_bad_count > 0 THEN
    RAISE EXCEPTION 'PG652b abort: found % trusted executable rules with missing hash but no safe card oracle_text source', v_bad_count;
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    ...
    notes = concat_ws(E'\n', nullif(r.notes, ''), 'PG652b: oracle_hash backfilled from cards.oracle_text for trusted executable rule.'),
    updated_at = now(),
    last_seen_at = now()
  FROM public.cards c
