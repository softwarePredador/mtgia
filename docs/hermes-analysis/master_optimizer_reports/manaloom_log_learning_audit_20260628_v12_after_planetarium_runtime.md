...
- generated_at: `2026-06-28T19:33:42Z`
...
- log_quality_gap: `2`
- postgres_sync_gap: `2`
- runtime_or_test_failure: `271`
- runtime_rule_gap: `571`
- xmage_mapping_gap: `1324`

## Action Queue

1. `critical` `runtime_rule_gap` `coherence_critical_high_findings` count=567 next=`prioritize_cards_with_no_active_or_no_trusted_rules_in_current_deck_scope`
   - coherence audit has critical=1, high=392 [docs/hermes-analysis/master_optimizer_reports/deck_card_battle_rule_coherence_audit_current_explicit_20260628_123905.json]
     ...
     top_lorehold_runtime_missing: `Invincible Hymn, Heroes Remembered, Boros Reckoner, Beacon of Immortality, Ancient Copper Dragon`
   ...
   - coherence audit has critical=1, high=340 [docs/hermes-analysis/master_optimizer_reports/xmage_current_replay_batch_pipeline_20260626_pg240_bolt_bend_postsync_v1_combined_coherence.json]
     ...
     top_lorehold_runtime_missing: `Penance, Magmakin Artillerist, Invincible Hymn, Heroes Remembered, Boros Reckoner`
...
2. `critical` `runtime_or_test_failure` `text_runtime_traceback` count=9 next=`inspect_text_log_and_convert_recurring_pattern_to_structured_gate_field`
   - text log contains runtime_traceback [docs/hermes-analysis/master_optimizer_reports/pgc031_call_forth_tempest_cascade_runtime_package.md]
...
6. `critical` `runtime_or_test_failure` `text_test_failure` count=1 next=`inspect_text_log_and_convert_recurring_pattern_to_structured_gate_field`
   - text log contains test_failure [docs/hermes-analysis/master_optimizer_reports/pgc031_call_forth_tempest_cascade_runtime_package.md]
...
8. `high` `blocked_workflow` `text_blocked` count=87 next=`inspect_text_log_and_convert_recurring_pattern_to_structured_gate_field`
...
11. `high` `runtime_rule_gap` `squee_graveyard_anomaly` count=2 next=`fix_squee_zone_accounting_before_using_recursion_results`
...
13. `high` `evidence_gap` `candidate_unobserved` count=1 next=`do_not_score_swap_until_forced_or_natural_access_sample_exists`
...
15. `high` `evidence_gap` `top_level_status` count=1 next=`rerun_with_candidate_observation_or_min_used_sample`
...
20. `medium` `battle_rhythm_gap` `prior_rhythm_flags` count=3 next=`calibrate_or_quarantine_gate_metrics_before_treating_winrate_as_strategy_signal`
...
21. `medium` `evidence_gap` `focus_card_library_only` count=2 next=`rerun_targeted_gate_with_forced_focus_access_or_larger_sample`
   ...
   - focus card traced but not accessed: The Mind Stone [docs/hermes-analysis/master_optimizer_reports/lorehold_birgi_min_sample_forced_probe_20260628_v1_20260628_113410_birgi_spellchain_cut_jeskas_will.json]
22. `medium` `runtime_or_test_failure` `gate_row_status` count=2 next=`inspect_gate_row_error_and_add_or_fix_runtime_test`
...
24. `medium` `battle_rhythm_gap` `top_level_status` count=1 next=`inspect_rhythm_flags_before_using_gate_for_deckbuilder`
...
26. `medium` `battle_runtime_quality` `stalled_games` count=1 next=`rerun_with_checkpoint_trace_and_fix_loop_or_decision_dead_end`
...
30. `medium` `evidence_gap` `focus_card_library_only` count=1 next=`rerun_targeted_gate_with_forced_focus_access_or_larger_sample`
...
34. `low` `log_quality_gap` `json_extra_data` count=2 next=`normalize_artifact_writer_to_emit_single_json_document`
