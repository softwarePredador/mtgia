...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg025_one_ring_orims_battle_rules_20260622_152115') IS NULL THEN
    RAISE EXCEPTION 'PG025 rollback backup table is missing';
  END IF;
END $$;
...
WHERE normalized_name IN ('the one ring', 'orim''s chant')
  AND (
    ...
    OR logical_rule_key IN (
      ...
      FROM manaloom_deploy_audit.pg025_one_ring_orims_battle_rules_20260622_152115
...
SELECT
  'pg025_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
