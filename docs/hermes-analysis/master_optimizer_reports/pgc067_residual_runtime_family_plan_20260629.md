# PGC067 Residual Runtime Family Plan

- Generated UTC: `2026-06-29T11:34:37Z`
- Baseline input: `39` cards from `annotation_runtime_batch_probe_20260629_pgc060`.
- After PGC067: `13` cards fully clean: `City of Brass`; `Clifftop Retreat`; `Dualcaster Mage`; `Elves of Deep Shadow`; `Erode`; `Inspiring Vantage`; `Mana Confluence`; `Reiterate`; `Reverberate`; `Sundering Eruption // Volcanic Fissure`; `Sundown Pass`; `Tarnished Citadel`; `Untimely Malfunction`.
- Remaining residual: `26` cards still have at least one `annotation_only` field.
- Runtime executor value paths: `42` in the current snapshot.
...
| `PGC063` | pain/five-color mana source life and damage costs | `City of Brass`; `Elves of Deep Shadow`; `Mana Confluence`; `Tarnished Citadel` | Promoted to `runtime_executor_v1`, modeled costs through `conditional_mana_modes`, synced PG -> SQLite/snapshot, and validated by spend scenarios. |
| `PGC064` | copy spell choose-new-targets runtime | `Dualcaster Mage`; `Reverberate`; `Reiterate` | Added copied-spell target-selection executor, promoted `choose_new_targets_status` to `runtime_executor_v1`, synced PG -> SQLite/snapshot, and validated response plus ETB copy scenarios. |
| `PGC065` | modal single-target stack target change runtime | `Return the Favor`; `Untimely Malfunction` | Added generic modal target-change executor, promoted `change_target_mode_status` and `redirect_target_mode_status`, synced PG -> SQLite/snapshot, and validated real response scenarios. |
...
| `PGC067` | mana buyback optional additional cost and hand replacement | `Reiterate` | Added buyback cost choice and resolved-spell hand replacement, promoted `buyback_status`, synced PG -> SQLite/snapshot, validated no-buyback and paid-buyback paths, and removed `Reiterate` from the residual set. |
...
| `Reiterate` | Stack copy, new-target selection, optional buyback cost, and return-to-hand replacement now execute in battle runtime. |
...
| ---: | --- | --- | --- |
| 1 | Spree cost selection and stack-object copy breadth | `Return the Favor` | Model spree mode-cost selection and copying activated/triggered abilities. Validate copy-spell mode, change-target mode, and legal stack-object filters separately. |
