...
shadow_rows AS (
  SELECT
    ...
    count(*) FILTER (
      WHERE cbr.execution_status = 'review_only'
         OR cbr.review_status = 'needs_review'
    ) AS review_only_shadow_rows
...
SELECT
  (SELECT count(*) FROM pg078_deck606_l2_target_rules) AS expected_target_rules,
  count(*) FILTER (WHERE logical_rule_key IS NOT NULL) AS target_rule_rows,
  count(DISTINCT name) FILTER (WHERE oracle_text IS NOT NULL) AS distinct_target_cards_with_oracle_text,
  ...
  (SELECT review_only_shadow_rows FROM shadow_rows) AS review_only_shadow_rows,
  to_regclass('manaloom_deploy_audit.pg078_deck606_l2_hash_scope_restore_20260623_063535') IS NOT NULL AS backup_table_already_exists
FROM target_rows;
...
SELECT
  ...
  c.cmc,
  md5(coalesce(c.oracle_text, '')) AS expected_oracle_hash,
  regexp_replace(coalesce(c.oracle_text, ''), '\s+', ' ', 'g') AS oracle_text,
  cbr.normalized_name,
