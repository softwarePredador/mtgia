...
CREATE TABLE IF NOT EXISTS manaloom_deploy_audit.pg584b_trusted_rule_oracle_hash_backfill_new_server_backup AS
SELECT
  r.normalized_name,
  r.logical_rule_key,
  r.card_name,
...
INSERT INTO manaloom_deploy_audit.pg584b_trusted_rule_oracle_hash_backfill_new_server_backup (
  normalized_name,
  logical_rule_key,
  card_name,
...
WHERE r.source IN ('manual', 'curated')
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM manaloom_deploy_audit.pg584b_trusted_rule_oracle_hash_backfill_new_server_backup b
