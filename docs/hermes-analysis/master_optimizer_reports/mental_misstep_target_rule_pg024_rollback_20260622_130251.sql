...
DO $$
BEGIN
  IF to_regclass('manaloom_deploy_audit.pg024_mental_misstep_target_rule_20260622_130251') IS NULL THEN
    RAISE EXCEPTION 'PG024 rollback backup table is missing';
  END IF;
END $$;
...
WHERE lower(card_name) = 'mental misstep'
  AND (
    logical_rule_key = 'battle_rule_v1:da6a568dbdfeda5d4009574d953db55e'
    OR logical_rule_key IN (
      SELECT split_part(key, '|', 2)
      FROM manaloom_deploy_audit.pg024_mental_misstep_target_rule_20260622_130251
...
SELECT
  'pg024_rollback_result' AS check_name,
  card_name,
  logical_rule_key,
  effect_json,
  source,
