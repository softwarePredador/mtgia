-- PG-001 rollback: Partner/background identity metadata backfill.
...
BEGIN
    ...
    FROM pg001_partner_identity_expected_rollback;

    IF expected_count <> 10 THEN
        RAISE EXCEPTION 'PG-001 rollback expected 10 input rows, got %', expected_count;
    END IF;

    SELECT count(*) INTO matched_count
    FROM pg001_partner_identity_expected_rollback e
    JOIN commander_learned_decks d
      ON d.id = e.id
     AND d.source_ref = e.source_ref;

    IF matched_count <> 10 THEN
        RAISE EXCEPTION 'PG-001 rollback expected 10 live id/source_ref matches, got %', matched_count;
    END IF;

    SELECT count(*) INTO rollback_ready_count
    FROM pg001_partner_identity_expected_rollback e
    JOIN commander_learned_decks d
      ON d.id = e.id
     AND d.source_ref = e.source_ref
    WHERE d.metadata = e.applied_metadata;

    IF rollback_ready_count <> 10 THEN
        RAISE EXCEPTION 'PG-001 rollback expected 10 rows matching applied metadata, got %', rollback_ready_count;
    END IF;
END $$;

CREATE TEMP TABLE pg001_partner_identity_rolled_back ON COMMIT DROP AS
WITH rolled_back AS (
    UPDATE commander_learned_decks d
       SET metadata = e.rollback_metadata
      FROM pg001_partner_identity_expected_rollback e
     WHERE d.id = e.id
       AND d.source_ref = e.source_ref
       AND d.metadata = e.applied_metadata
     RETURNING
        d.id::text AS row_id,
        d.source_ref,
        d.commander_name,
        d.deck_name,
        d.metadata ? 'commander_identity_model' AS has_commander_identity_model,
        d.metadata ? 'partner_identity_backfill' AS has_partner_identity_backfill
)
...
BEGIN
    ...
    FROM pg001_partner_identity_rolled_back;

    IF rollback_count <> 10 THEN
        RAISE EXCEPTION 'PG-001 rollback expected 10 updated rows, got %', rollback_count;
    END IF;
END $$;

TABLE pg001_partner_identity_rolled_back ORDER BY source_ref;
