-- PG059 sync metadata restore precheck.
-- Scope: trusted rows already closed by PG052/PG054/PG057/PG058 whose
-- oracle_hash/oracle-runtime metadata can be erased by the reviewed JSON sync.
...
UNION ALL
SELECT 'target_card_id_missing_rows', count(*)::text
FROM target_state
WHERE target_card_id IS NULL
UNION ALL
SELECT 'backup_table_exists',
       CASE
         WHEN to_regclass('manaloom_deploy_audit.pg059_sync_metadata_restore_20260623_022328') IS NULL
           THEN '0'
         ELSE '1'
       END;
...
SELECT
  ...
  cbr.effect_json,
  left(coalesce(cbr.notes, ''), 160) AS notes
FROM target_rules tr
LEFT JOIN cards c
  ON lower(c.name) = lower(tr.card_name)
LEFT JOIN card_battle_rules cbr
