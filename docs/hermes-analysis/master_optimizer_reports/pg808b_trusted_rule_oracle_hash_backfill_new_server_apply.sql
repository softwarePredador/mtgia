\echo 'PG808B trusted rule oracle_hash backfill apply'

BEGIN;

CREATE SCHEMA IF NOT EXISTS manaloom_deploy_audit;

CREATE TABLE manaloom_deploy_audit.pg808b_trusted_rule_oracle_hash_backfill_new_server_20260712 AS
SELECT
  br.*,
  md5(c.oracle_text) AS pg808b_new_oracle_hash,
  NOW() AS pg808b_backed_up_at
...
updated AS (
  ...
  SET
    ...
    notes = CONCAT_WS(
      E'\n',
      NULLIF(br.notes, ''),
      'PG808B: backfilled oracle_hash for trusted executable rule from current cards.oracle_text md5 on 2026-07-12.'
    )
  FROM target
...
SELECT
  (SELECT COUNT(*) FROM manaloom_deploy_audit.pg808b_trusted_rule_oracle_hash_backfill_new_server_20260712) AS backup_rows,
