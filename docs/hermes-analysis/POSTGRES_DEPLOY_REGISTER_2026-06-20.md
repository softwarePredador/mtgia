...
## PG058 Deck 6 L3B Simple Red Ritual Batch - Applied 2026-06-23 02:11 UTC
...
- Backup row count: `5`.
- Updated trusted runtime rows: `2`.
- Disabled generated/legacy shadow rows: `3`.
...
- Runtime replay evidence: `treasure_created` now emits the same
  `rule_logical_key` and `rule_oracle_hash` for `Unexpected Windfall`; focused
  event artifact:
...
## PG199 - Taii Wakeen, Perfect Shot

Status: applied to PostgreSQL, synced into Hermes SQLite, validated in deck
audit, and accepted by the battle strategy gate.
...
- Exact XMage mapping:
  `TaiiWakeenPerfectShotTriggeredAbility` draws a card when a source the
  controller controls deals noncombat damage to a creature equal to that
  creature's toughness; `TaiiWakeenPerfectShotEffect` is an `{X}, {T}`
  replacement effect that adds X to noncombat damage from sources the
  controller controls until end of turn.
- ManaLoom battle model scope:
...
- XMage hint maps `TaiiWakeenPerfectShot` from the exact internal ability/effect
  classes plus `SimpleActivatedAbility` and `TapSourceCost`.
- Semantic classifier marks only the exact Taii scope as batch-safe.
- Battle runtime now handles turn-scoped noncombat damage modifiers, Taii's
  contextual `{X}, {T}` activation, and the exact-toughness draw trigger.
- Event contract classifier marks `noncombat_damage_modified` as a
  `strategy_signal`, preventing the static audit from treating the new runtime
  event as unclassified.
...
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg199_taii_wakeen_package_20260625_precheck.sql`.
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg199_taii_wakeen_package_20260625_apply.sql`.
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg199_taii_wakeen_package_20260625_postcheck.sql`.
- Rollback SQL:
...
- Affected deck audit:
  ...
  `pass/coherent_for_current_gate`.
- Tests:
  mapper tests ran `179` tests OK; classifier tests ran `165` tests OK;
  `battle_card_specific_tests.py`, `test_battle_analyst_v10_3.py`,
  `test_battle_event_contract_static_audit.py`, and
  `test_battle_forensic_audit_supported_effects.py` passed.
- Battle strategy gate:
...
- PG199 is applied, postchecked, synced, locally tested, deck-coherence
  validated for deck `612`, and strategy-audited.
- Do not reuse PG199.
- Continue next with the remaining Lorehold-touching
  `needs_rule_before_strategy` cards. Current top items after PG199 are
  `Trouble in Pairs`, `Deflecting Palm`,
  `Primal Amulet // Primal Wellspring`, `Redress Fate`, and
  `Starfield Shepherd`.
- Next package number is PG200.

## PG200 - Trouble in Pairs
...
- Decks touched by current Lorehold/opponent matrix: `614`, `619`.
- XMage source:
  `/Users/desenvolvimentomobile/Downloads/mage-master/Mage.Sets/src/mage/cards/t/TroubleInPairs.java`.
- Exact XMage mapping:
  `SkipExtraTurnsAbility(true)` skips opponent extra turns; the custom
  `TroubleInPairsTriggeredAbility` draws one card when an opponent attacks the
  controller with two or more creatures, draws their second card in a turn, or
  casts their second spell in a turn.
- ManaLoom battle model scope:
...
- XMage hint maps `TroubleInPairs` from the exact class, imported watcher
  classes, `SkipExtraTurnsAbility`, and `DrawCardSourceControllerEffect`.
- Semantic classifier marks only the exact Trouble scope as batch-safe.
- Battle runtime now handles all four runtime surfaces: opponent second draw,
  opponent second spell, two-or-more attackers at the controller, and skipped
  opponent extra turns.
- Event contract classifier marks `extra_turn_skipped` as a strategy signal.

Package files:

- Package:
  `docs/hermes-analysis/master_optimizer_reports/pg200_trouble_in_pairs_package_20260625_package.md`.
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg200_trouble_in_pairs_package_20260625_precheck.sql`.
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg200_trouble_in_pairs_package_20260625_apply.sql`.
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg200_trouble_in_pairs_package_20260625_postcheck.sql`.
- Rollback SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg200_trouble_in_pairs_package_20260625_rollback.sql`.

Evidence:

- Precheck:
  `target_card_rows=1`, canonical card id
  `56045b1c-6d1b-4e58-8393-4fd05d42e312`,
  `existing_rule_rows=2`, `expected_rule_rows_before=0`,
  `would_deprecate_shadow_rows=2`.
- Apply:
  backup rows `2`, `deprecated_shadow_rows=2`, `upserted_rows=1`, `COMMIT`.
- Postcheck:
  ...
  `promoted_oracle_hash_rows=1`, `backup_rows=2`.
- PG -> Hermes sync:
  `docs/hermes-analysis/master_optimizer_reports/battle_card_rules_sqlite_from_pg_pg200_trouble_in_pairs_20260625.json`;
  `selected_card_count=1`, `pg_rows_loaded=1`,
  `sqlite_inserted_or_updated=3`, `canonical_snapshot_rows_exported=3241`.
- Post-sync pipeline:
  `docs/hermes-analysis/master_optimizer_reports/xmage_current_replay_batch_pipeline_20260625_pg200_trouble_in_pairs_postsync_v1_manifest.json`;
  expanded scope moved to `high=397`, `medium=63`, `pass=502`.
- Post-sync matrix:
  `docs/hermes-analysis/master_optimizer_reports/lorehold_ideal_candidate_matrix_20260625_pg200_trouble_in_pairs_postsync_v1.json`;
  scoped rows `567`, `battle_ready=350`,
  `needs_rule_before_strategy=217`, `runtime_needed=16`,
  `mapper_manual=144`, `split_scope=53`.
- Strategy consistency:
  `docs/hermes-analysis/master_optimizer_reports/xmage_strategy_consistency_audit_20260625_pg200_trouble_in_pairs_postsync_v1.json`;
  `18/18` checks passed.
- Affected deck audits:
  `docs/hermes-analysis/master_optimizer_reports/deck614_battle_rule_coherence_pg200_trouble_in_pairs_postsync_v1.json`
  and
  `docs/hermes-analysis/master_optimizer_reports/deck619_battle_rule_coherence_pg200_trouble_in_pairs_postsync_v1.json`;
  both report `Trouble in Pairs` as `pass/coherent_for_current_gate`.
- Tests:
  mapper tests ran `180` tests OK; classifier tests ran `166` tests OK;
  `battle_card_specific_tests.py`, `test_battle_analyst_v10_3.py`,
...
- PG200 is applied, postchecked, synced, locally tested, deck-coherence
  validated for decks `614` and `619`, and strategy-audited.
- Do not reuse PG200.
- Continue next with the remaining Lorehold-touching
  `needs_rule_before_strategy` cards. Current top items after PG200 are
  `Deflecting Palm`, `Primal Amulet // Primal Wellspring`, `Redress Fate`,
  `Starfield Shepherd`, and `Bedlam Reveler`; the expanded opponent matrix also
  surfaces higher-scored opponent blockers such as `Psychic Frog`,
...
## PG206 - Damage Each Opponent Spell

Status: applied, postchecked, synced, tested, strategy-audited.

Scope:

- Promotes exactly `Boltwave`.
- XMage local source:
  `/Users/desenvolvimentomobile/Downloads/mage-master/Mage.Sets/src/mage/cards/b/Boltwave.java`.
- Semantic family:
  `opponent_damage_spell`.
- Runtime effect:
  `damage_each_opponent`.
- Battle scope:
  `spell_damage_each_opponent_v1`.

Package files:

- Package:
  `docs/hermes-analysis/master_optimizer_reports/pg206_damage_each_opponent_package.md`.
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg206_damage_each_opponent_precheck.sql`.
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg206_damage_each_opponent_apply.sql`.
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg206_damage_each_opponent_postcheck.sql`.
- Rollback SQL:
  `docs/hermes-analysis/master_optimizer_reports/pg206_damage_each_opponent_rollback.sql`.
...
- Postcheck:
  ...
  `promoted_oracle_hash_rows=1`, `backup_rows=2`.
- PG -> Hermes sync:
  `docs/hermes-analysis/master_optimizer_reports/battle_card_rules_sqlite_from_pg_pg206_damage_each_opponent_20260625_0633.json`;
  `selected_card_count=1`, `pg_rows_loaded=3`,
  `sqlite_inserted_or_updated=3`, `canonical_snapshot_rows_exported=3242`,
  `generated_rows=1`.
- Runtime cache spot-check:
  `Boltwave` resolves from local cache as `effect=damage_each_opponent`,
  `battle_model_scope=spell_damage_each_opponent_v1`, `amount=3`,
  `damage=3`, `target_controller=opponents`,
  `_rule_logical_key=battle_rule_v1:50c63854aa8fb56018a56312c07cdd69`,
  `_rule_oracle_hash=0c062fe415bc928cf3762258c2073736`, and
  `_rule_execution_status=auto`.
- Post-sync pipeline:
  `docs/hermes-analysis/master_optimizer_reports/xmage_current_replay_batch_pipeline_20260625_pg206_damage_each_opponent_postsync_v1_manifest.json`;
  expanded scope moved to `high=389`, `medium=63`, `pass=510`, with no
  remaining `opponent_damage_spell` package proposal because `Boltwave`
  became battle-ready.
- Post-sync matrix:
  `docs/hermes-analysis/master_optimizer_reports/lorehold_ideal_candidate_matrix_20260625_pg206_damage_each_opponent_postsync_v1.json`;
  scoped rows `580`, `battle_ready=371`,
  `needs_rule_before_strategy=209`.
- Lorehold deck block:
  decks `608` through `616` moved from `101` to `100` remaining
  `needs_rule_before_strategy` rows by structured `deck_ids`
  (`78` mapper manual, `16` split-scope, `6` runtime-needed).
- Effective queue:
  `docs/hermes-analysis/master_optimizer_reports/xmage_effective_queue_20260625_pg206_damage_each_opponent_postsync_v1.json`;
  remaining lane counts are `manual_mapper_backlog=338`,
  `split_scope_backlog=72`, `runtime_family_backlog=20`,
  `blocked_missing_xmage_source=4`, and no package-ready unprepared rows.
- Strategy consistency:
  `docs/hermes-analysis/master_optimizer_reports/xmage_strategy_consistency_audit_20260625_pg206_damage_each_opponent_postsync_v1.json`;
  `18/18` checks passed.
- Tests:
  focused mapper/classifier tests passed through `unittest`;
  event-contract static test passed; focused battle regression
  `test_pg206_boltwave_damages_each_opponent` passed through the
  `test_battle_analyst_v10_3.py` harness.
- Battle strategy gate:
  ...
  `event_contract_static_status=event_contract_static_ready`,
  `test_results_status_counts={"pass":18}`, `test_results_total=18`,
  and `deck_provenance_files=16`.

Register decision:

- PG206 is applied, postchecked, synced, locally tested, and
  strategy-audited.
- Do not reuse PG206.
...
## 2026-07-05 - PG499 Dies Token Java String Parser New Server
...
- Runtime family: `xmage_creature_dies_create_tokens_v1`.
- XMage source signature:
  `DiesSourceTriggeredAbility + CreateTokenEffect`.
- Promoted cards: `2`.
- Promoted card names: `Conclave Cavalier`, `Mausoleum Guard`.
- Dynamic/custom token descriptions and token classes with mana abilities remain
  blocked for separate runtime families.
...
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg499_dies_token_java_string_parser_new_server_precheck.sql`.
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg499_dies_token_java_string_parser_new_server_apply.sql`.
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg499_dies_token_java_string_parser_new_server_postcheck.sql`.
- Rollback SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg499_dies_token_java_string_parser_new_server_rollback.sql`.
- Manifest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg499_dies_token_java_string_parser_new_server_manifest.json`.
...
- PG -> Hermes sync:
  ...
  `pg_rows_loaded=8417`, `sqlite_inserted_or_updated=8181`,
...
- SQLite validation:
  ...
  `status=pass`, `validated_card_count=2`, `issue_count=0`.
- Contract audit:
  ...
  `51/51` checks passed.
- Post-sync queue:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_adaptation_queue_20260705_post_pg499_dies_token_java_string_parser_new_server_commander_legal.json`;
...
- PG499 is applied and should not be rebuilt.
- Exact fixed dies-token makers with parseable token descriptors are executable
  through `xmage_creature_dies_create_tokens_v1`.
- Dynamic/custom token makers must wait for separate exact runtime families.

## 2026-07-05 - PG500 ETB Destroy Static Keywords New Server
...
- Promoted cards: `7`.
- Promoted card names: `Acid Web Spider`, `Acidic Slime`,
  `Aven Cloudchaser`, `Cloudchaser Eagle`, `Manticore`,
  `Rooftop Assassin`, `Stingblade Assassin`.
- Only static self keyword auxiliary abilities are allowed in this batch.
  Kicker, evoke, conditional/reflexive ETB destroy, unsupported targets, and
  composite destroy effects remain blocked for separate runtime families.
...
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg500_etb_destroy_static_keywords_new_server_precheck.sql`.
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg500_etb_destroy_static_keywords_new_server_apply.sql`.
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg500_etb_destroy_static_keywords_new_server_postcheck.sql`.
- Rollback SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg500_etb_destroy_static_keywords_new_server_rollback.sql`.
- Manifest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg500_etb_destroy_static_keywords_new_server_manifest.json`.

Execution evidence:

- Precheck:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg500_etb_destroy_static_keywords_new_server_precheck.out`;
  all 7 target cards matched exactly one canonical row, and 2 old
  `Acidic Slime` rows were identified for deprecation.
- Apply:
...
- PG -> Hermes sync:
  ...
  `pg_rows_loaded=8424`, `sqlite_inserted_or_updated=8188`,
...
- SQLite validation:
  ...
  `status=pass`, `validated_card_count=7`, `issue_count=0`.
- Contract audit:
...
- PG500 is applied and should not be rebuilt.
- ETB destroy rows with only static self keyword auxiliaries are executable
...
## 2026-07-05 - PG504 activated damage target parser
...
- Promoted cards: `11`.
- Promoted card names: `Deadeye Duelist`, `Elite Headhunter`,
  `Femeref Archers`, `Pyroclastic Elemental`, `Razortip Whip`,
  `Shauku's Minion`, `Slingshot Goblin`, `Sorcerer of the Fang`,
  `Spinal Villain`, `Western Paladin`, and `Zealot of the God-Pharaoh`.
- Only exact activated permanent damage/destroy target-parser shapes are
  allowed in this batch. Dynamic damage, unsupported activation costs, modal
  abilities, random targets, multi-target damage, and non-matching
  Oracle/XMage target pairs remain blocked.

Package files:

- Package:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_package.md`.
- Manifest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_manifest.json`.
- Precheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_precheck.sql`.
- Apply SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_apply.sql`.
- Postcheck SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_postcheck.sql`.
- Rollback SQL:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_rollback.sql`.

Execution evidence:

- Apply evidence:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_apply_evidence.md`.
- Apply:
  `deprecated_shadow_rows=0`, `upserted_rows=11`, `COMMIT`.
- Postcheck:
  all 11 promoted rows have `promoted_rule_rows=1`,
  `promoted_verified_auto_rows=1`, and `promoted_oracle_hash_rows=1`.
- Field postcheck:
  all 11 promoted rows have expected activated scope, target, target
  constraints, and fixed damage amount where applicable.
- PG -> Hermes SQLite sync:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg504_activated_damage_target_parser_new_server_pg_to_sqlite_sync.json`;
  `pg_rows_loaded=8446`, `sqlite_inserted_or_updated=8210`,
  `canonical_snapshot_rows_exported=5972`.
- Runtime lookup:
  all 11 cards resolve from the curated runtime cache with expected activated
  damage/destroy scope.
- Battle suite:
  ...
  has `632` PASS lines and no `Traceback`, `FAILED`, `ERROR`, or `SkipTest`.
- Audits:
  PG/Hermes/SQLite `51/51` pass, XMage strategy `26/26` pass, operational
  surface `pass`, and legacy contamination `pass`.
- Post-sync queue:
  `target_identity_count=26045`, `xmage_authoritative_source_count=25731`,
...
## 2026-07-05 - PG514 exile/scry targets

- Deploy id: `xmage_pg514_exile_scry_targets_new_server`.
- Runtime family: `xmage_exile_target_scry_spell`.
- Promoted cards: `2`.
- Promoted card names: `Devout Decree` and `Ray of Ruin`.
- Scope boundary: only local-XMage-backed exile-target plus fixed scry patterns
  with exact target constraints are allowed in this package. Unsupported exile
  variants, extra clauses, modal effects, unsupported costs, and broad
  `xmage_*_review_v1` rows remain blocked.
...
- Validation:
  combined exact-scope/runtime suite `826` tests passed, battle package E2E
  validation `pass`, XMage strategy `26/26` pass, operational surface `39/39`
  pass, legacy contamination `32/32` pass, and PG/Hermes/SQLite `51/51` pass.
- Post-sync queue:
  `target_identity_count=25999`, `xmage_authoritative_source_count=25685`,
  `xmage_missing_source_exception_count=314`,
  `xmage_authoritative_parser_gap_count=0`, and
...
- Global readiness:
  ...
  `snapshot_has_any_rule=6021`, and
...
- PG514 is applied and should not be rebuilt.
- Exile+scry target constraints are now exact-scope supported for the two
  promoted cards.
