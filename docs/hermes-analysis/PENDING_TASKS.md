...
> Oitavo split de optimize concluído: diagnostics finais da rota foram movidos
> para `optimize_route_diagnostics_support.dart`, preservando
> `optimize_diagnostics` e merges sem sobrescrita.
...
> Décimo primeiro split de optimize concluído: validação pós-processamento da
...
> Décimo quarto split de optimize concluído: filtro de identidade de cor foi
...
> Décimo sexto split de optimize concluído: top-up determinístico de básicos em
> modo complete foi movido para `optimize_route_complete_top_up_support.dart`,
> mantendo a query de IDs de básicos na rota e cobrindo dedupe/round-robin em
> testes.
> Décimo sétimo split de optimize concluído: proteção de remoção de lands em
...
| 6 | Próximo split da rota optimize: orquestração de análise virtual/validação automática | 1-2 dias | Médio | `optimize_route_addition_data_support.dart` verde |
...
| `optimize_route_complete_top_up_support.dart` | Top-up determinístico de básicos no modo complete, separado da query de IDs | 91 |
