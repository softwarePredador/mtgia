...
- `COMMANDER_DECKBUILDING_CONTRACT_2026-06-29.md`
  ...
  - Pivot operacional de 2026-07-05: o foco do aprendizado passa a ser o core
    ...
    evidencia local atual:
    `master_optimizer_reports/global_commander_learning_priority_audit_20260706_engine_axis_exhaustion_current.md`.
    Essa fila agora consome o router de exaustao de fontes externas/nonpayoff
    e o router de protecao de `Biotransference`: quando o eixo engine fica sem
    corte viavel, o proximo passo global passa a ser
    `pivot_to_cross_commander_role_axis_learning_after_engine_axis_exhaustion`,
    nao mais expansao de fonte dentro do mesmo deck.
    ...
    A revisao de trace/replacement agora roda em
    `manaloom-knowledge/scripts/global_commander_engine_cut_trace_replacement_reviewer.py`,
    com evidencia:
    `master_optimizer_reports/global_commander_engine_cut_trace_replacement_reviewer_20260706_current.md`.
    Resultado: `Archaeomancer's Map` segue bloqueado porque apareceu como
    ...
    A fila global foi rerodada em
    `master_optimizer_reports/global_commander_learning_priority_audit_20260706_engine_axis_exhaustion_current.md`;
    resultado: Kaalia `619` segue como topo, agora com
    `pivot_to_cross_commander_role_axis_learning_after_engine_axis_exhaustion`.
    ...
    `manaloom-knowledge/scripts/global_commander_candidate_added_card_exposure_trace_generator.py`
    gerou
    `master_optimizer_reports/global_commander_candidate_added_card_exposure_trace_generator_20260706_lorehold_profile_repair_package.md`
    com status `candidate_added_card_forced_exposure_all_exercised_diagnostic_only`:
    as quatro cartas foram exercitadas sob forced access `opening_hand`, mas
    ...
    promocao.
    O feedback atual foi regenerado em
    `master_optimizer_reports/global_commander_battle_feedback_model_20260706_larger_gate_current.md`.
    Ele adiciona `package_feedback`, marca esse pacote como
    `package_blocked_by_protected_baseline_gate`, preserva o blocker de carta sem
    exercicio, e recomenda
    `block_package_until_new_source_lane_cut_or_strategy`. Rerodar o strategy
    matrix com esse feedback gerou
    `master_optimizer_reports/global_commander_candidate_package_strategy_matrix_20260706_lorehold_profile_repair_package_post_larger_feedback.md`
    com `package_strategy_blocks_battle`, `battle_feedback_blocker_count=3` e
    `replace_failed_package_source_lane_or_cut_set_before_battle`; o pacote nao
    pode reabrir battle sem source lane, cut set ou hipotese estrategica nova.
    O ciclo global rerodado com esse feedback atual levou o learning priority a
    `pivot_to_cross_commander_role_axis_learning_after_ramp_axis_exhaustion`, o
    pivot/policy ao eixo `land`, e o land-floor builder ao deck `612`. A copia
    candidata limpa esta documentada em
    `master_optimizer_reports/global_commander_candidate_copy_materializer_20260706_larger_gate_feedback_land_floor_deck612.md`;
    a cadeia passou, mas a matriz especifica
    ...
    consolidar o historico por assinatura add/cut e pacote exatos antes de
    reabrir a fila de aprendizado. Evidencia local anterior:
