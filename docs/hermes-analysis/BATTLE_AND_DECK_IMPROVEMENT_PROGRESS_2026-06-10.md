...
- `server/routes/ai/optimize/index.dart` — 2781 linhas após splits de
  resposta/diagnóstico, envelope async, parsing inicial, payload final e
  warnings/diagnostics finais da rota.
...
2. Continuar extraindo blocos da rota `routes/ai/optimize/index.dart`: os
   ...
   warnings finais e diagnostics finais já foram feitos; os próximos cortes
   seguros são blocos de fallback/retry e validação pós-processamento.
