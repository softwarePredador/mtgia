...
## PGC Namespace - 2026-06-28

- This agent now uses `PGC###` for card oracle/rule packages.
- Before creating a new `PGC###`, read recent `PG###` artifacts from the
  parallel agent and confirm the scope does not overlap.
- Legacy `PG247` and `PG248` artifacts below were generated before the namespace
  split; new packages from this agent must not use `PG###`.

## PGC001 Seething Song Oracle Hash Restore - 2026-06-28

Current cycle result:

- PGC001 restored only
  `oracle_hash=ccd492289c6f1c14c8fb7a248d7bbf32` for `Seething Song`.
- The active runtime stayed
  `battle_rule_v1:3eb15dc581c6b913158f9b63c023f3d7`,
  `effect=ramp_ritual`, `battle_model_scope=single_shot_red_ritual_v1`.
- PostgreSQL -> Hermes sync after PGC001 used `apply_pg=false`.
- Deck `6` post-audit is `pass=100`; deck `606` post-audit is `pass=81`.
...
## PG247 Deck 6 Seething Song Hash-Only Closure - 2026-06-28

Current cycle result:

- PostgreSQL remained the source of truth; Hermes SQLite was resynced after the
  durable PostgreSQL apply.
- Deck `6` was restored/synced from the official PostgreSQL deck before the
  card audit; no deck swap was applied.
- Current deck `6` pre-PG247 audit had one medium finding:
  `Seething Song` with `trusted_rule_without_oracle_hash`.
- The active rule was already correct:
  `battle_rule_v1:3eb15dc581c6b913158f9b63c023f3d7`,
  `effect=ramp_ritual`, `battle_model_scope=single_shot_red_ritual_v1`,
  `mana_produced=5`, `produces=R`, `execution_status=auto`.
- PG247 restored only
...
- Package:
  `docs/hermes-analysis/master_optimizer_reports/pg247_seething_song_oracle_hash_drift_package.md`.
- Precheck/apply/postcheck:
...
- Focused tests: `test_sync_battle_card_rules_pg_selection.py -v` passed `16`
  tests; selected `pg058_simple_red_ritual_family` battle tests passed `2`
  tests.
- Post-audit:
...
- Deck `607` is a protected generation baseline, and
  `candidate_607_squee_v1` is now a demoted recursion hypothesis in
  `lorehold_candidate_hypothesis_registry_20260626.json`.
- Current 2026-06-28 rebaseline gates using the PG247/PG248 local state did
  not prove Squee as an upgrade: seed42 g3 tied `4W/5L/0S` against deck `607`
  `4W/5L/0S`, and seed99 g3 lost `0W/9L/0S` against deck `607` `2W/7L/0S`.
- That does not supersede the card-rule workflow. The current deck `607`
  coherence audit is `medium=7`, `pass=87`, with remaining land/mana-base
  review rows; deck `6` remains the clean `pass=100` card-rule reference.
...
## PG248 Deck 606 Land Shadow Cleanup - 2026-06-28

Current cycle result:

- PostgreSQL remained the source of truth; Hermes SQLite/canonical snapshot was
  resynced after the durable PostgreSQL apply.
- PG248 closed the deck `606` L1 land/mana-base shadow queue for seven cards:
  `Boros Garrison`, `Boseiju, Who Shelters All`, `Command Beacon`,
  `Eiganjo, Seat of the Empire`, `Furycalm Snarl`, `Reliquary Tower`, and
  `Valakut, the Molten Pinnacle`.
- The package rejected/disabled generated `needs_review/review_only` land
  shadows where a trusted curated land-only runtime row already existed.
- The package intentionally did not implement utility-land clauses such as ETB
  tapped/bounce, uncounterable mana, channel damage, commander-zone movement,
  max hand size, or Valakut damage.
...
- Postcheck reported `target_cards=7`, `trusted_land_rows=7`,
...
## PG080 Deck 606 L3 Mana/Ramp Batch - 2026-06-23 08:20 UTC

Closed:

- Deck `606` L3 mana/ramp support cards:
  `Monologue Tax`, `Mox Opal`, and `Simian Spirit Guide`.
- The trusted PostgreSQL rows now have matching `oracle_hash`,
  oracle-specific `battle_model_scope`, stable `logical_rule_key`, and
  verified/auto runtime metadata.
- Three generated `needs_review`/`review_only` shadow rows were disabled after
  the trusted replacement rows passed postcheck.
- No deck swap, no `deck_cards` mutation, and no learned-deck mutation.

Runtime semantics:

- `Monologue Tax` is modeled as an opponent second-spell trigger that creates
  one Treasure.
- `Mox Opal` remains a mana rock but requires metalcraft: three controlled
  artifacts before it contributes mana.
- `Simian Spirit Guide` is handled as a hand-exile activated mana ability; it
  adds one mana and moves the card from hand to exile without resolving as a
  spell.
...
- Postcheck facts: `target_rule_rows=3`, `target_hash_match_rows=3`,
...
- PG -> SQLite/canonical sync:
  ...
  with `pg_rows_loaded=1824`, `sqlite_inserted_or_updated=1805`, and
  `canonical_snapshot_rows_exported=3201`.
- Focused events:
  `docs/hermes-analysis/master_optimizer_reports/deck606_pg080_l3_mana_ramp_focused_events_20260623_052022.jsonl`
  has three rows and proves all three PG080 `rule_logical_key` values.

Tests:

- `python3 docs/hermes-analysis/manaloom-knowledge/scripts/test_deck_card_battle_rule_coherence_audit.py -v`
- `python3 docs/hermes-analysis/manaloom-knowledge/scripts/test_battle_analyst_v10_3.py`
- `python3 -m py_compile` for the touched runtime/test/audit scripts during
  this cycle.

All passed for the PG080 L3 checkpoint.
...
- Use PG081 for the next PostgreSQL package.
...
## PG081-PG085 Runtime/Hash Closeout - 2026-06-23 08:38 UTC

Closed in this workflow cycle:

- PG081 closed three shared deck `607`/`608` high rules:
  `Artist's Talent`, `Pinnacle Monk // Mystic Peak`, and
  `Redirect Lightning`.
- PG082 restored trusted hash provenance for `Silence` and adopted the
  validated deck `6`/`606` hash-only package for `Library of Leng`,
  `Scroll Rack`, `Unexpected Windfall`,
  `Valakut Awakening // Valakut Stoneforge`, and `Wayfarer's Bauble`.
- PG083 restored trusted runtime hashes for `Fellwar Stone`, `Mana Vault`,
  `Mox Amber`, `Talisman of Conviction`, and `Seething Song`.
- PG084 restored `Seething Song` runtime metadata after the sync exposed
  metadata drift.
- PG085 is not a deploy package. It is the post-runtime checkpoint label after
  the PG081-PG084 packages and the concurrent PG082 hash-only package were
  synchronized.

Validation evidence:

- PostgreSQL postchecks exist for PG081, PG082, PG083, and PG084 under
...
- Deck `606` has only four medium hash-only rows:
  `Hexing Squelcher`, `Ragavan, Nimble Pilferer`, `Skyclave Apparition`, and
  `Underworld Breach`.
- Deck `607` still has 23 high rows. The battle-critical first lane is
  `Avatar's Wrath`, `Call Forth the Tempest`, `Creative Technique`,
  `Dawn's Truce`, `Everything Comes to Dust`, `Fated Clash`,
  `Furygale Flocking`, `Generous Gift`, `High Noon`, `Insurrection`,
  `Prismari Pianist`, `Promise of Loyalty`, `Starfall Invocation`,
  `Stroke of Midnight`, `Tempt with Bunnies`, and `Winds of Abandon`.
- Deck `608` still has 17 high rows. The battle-critical/support first lane is
  `Angel's Grace`, `Cool but Rude`, `Magmakin Artillerist`,
  `Pyromancer Ascension`, `Razorgrass Ambush // Razorgrass Field`,
  `Return the Favor`, `Twinflame Tyrant`, `Untimely Malfunction`,
  `Velomachus Lorehold`, `Ancient Copper Dragon`, `Goldspan Dragon`,
  `Locket of Yesterdays`, `Pyromancer's Goggles`, `Surly Badgersaur`, and
  `Tablet of Discovery`.

Rule for the next cycle:

- Use PG086 for the next PostgreSQL package.
- Do not claim a better Lorehold deck, a battle rebaseline, or a deck swap
...
- Hash-only provenance for `Library of Leng`, `Scroll Rack`,
  `Unexpected Windfall`, `Valakut Awakening // Valakut Stoneforge`, and
  `Wayfarer's Bauble`.
- Deck `6` returned to `pass=100` after a fresh PostgreSQL -> SQLite sync.
- Deck `606` returned to `medium=4`, `pass=77`.
- No runtime semantics, deck composition, or learned-deck state changed.

Why only these five:

- `Fellwar Stone`, `Mana Vault`, `Mox Amber`, `Seething Song`, and
  `Talisman of Conviction` already had hashes in PostgreSQL and only required
  a clean sync to SQLite.
- `Hexing Squelcher`, `Ragavan, Nimble Pilferer`, `Skyclave Apparition`, and
  `Underworld Breach` are not hash-only safe: their current trusted rows are
  generic or materially incomplete relative to Oracle text and must move
  through semantic review/executor gating.
...
- Postcheck facts: `target_rule_rows=5`, `target_hash_match_rows=5`,
...
- Focused events:
  ...
  has 10 rows and proves all five PG082 `rule_logical_key` and
  `rule_oracle_hash` values.
...
All passed for the PG082 checkpoint.
...
- Use PG083 for the next PostgreSQL package in this thread.
- Continue deck `606` semantic review for `Hexing Squelcher`,
  `Ragavan, Nimble Pilferer`, `Skyclave Apparition`, and
  `Underworld Breach`.

## PG087/PG088 Deck 606 Remaining Semantic Batch - 2026-06-23 09:03 UTC

Closed:

- Deck `606` is now closed for the card-rule coherence gate:
  ...
  reports `pass=81`.
- The closed cards are `Hexing Squelcher`,
  `Ragavan, Nimble Pilferer`, `Skyclave Apparition`, and
  `Underworld Breach`.
- No deck swap, no `deck_cards` mutation, and no learned-deck mutation.

Why this was not hash-only:

- All four cards had trusted executable rows lacking `oracle_hash`, but their
  prior trusted rows were too generic relative to Oracle text.
- PG087 replaced the generic rows with card-specific scopes and preserved
  unsupported clauses as explicit annotations instead of implying runtime
  execution. PG088 then corrected the four `oracle_hash` values to the raw
  `oracle_text` md5 convention.

Runtime/modeling notes:

- `Hexing Squelcher`: creature body plus self-uncounterable and
  controller-spell counter shield are executable through the counter target
  filter; ward-pay-life clauses remain annotation-only.
- `Ragavan, Nimble Pilferer`: creature body and haste are executable;
  combat-damage Treasure/exile/cast and dash remain annotation-only.
- `Skyclave Apparition`: creature body plus ETB exile now uses runtime target
  filters for nonland, nontoken, mana value <= 4; leave-battlefield Illusion
  token remains annotation-only.
- `Underworld Breach`: passive enchantment resolution is modeled; escape grant,
  escape additional cost, and end-step sacrifice remain annotation-only.

PostgreSQL package:

- Precheck/apply/postcheck:
  `docs/hermes-analysis/master_optimizer_reports/deck606_remaining_semantic_pg087_precheck_20260623_085349.out`,
...
- Postcheck facts: `target_rule_rows=4`, `target_hash_match_rows=4`,
  `target_missing_hash_rows=0`, `target_expected_effect_rows=4`,
  `target_expected_scope_rows=4`, `trusted_auto_rows=4`,
  `non_disabled_shadow_rows=0`, `disabled_shadow_rows=4`, and
  `backup_rows=8` under the original PG087 normalized-hash convention.
- PG088 hash-convention correction:
  `docs/hermes-analysis/master_optimizer_reports/deck606_pg087_hash_convention_fix_pg088_postcheck_20260623_090018.out`
  reports four raw-hash matches and `backup_rows=4`.
...
- Focused events:
  ...
  proves all four final raw-hash logical rule keys plus Hexing and Skyclave
  runtime behavior.
...
All passed for the PG088 checkpoint.
...
## PG089 Start Queue Snapshot - 2026-06-23 09:11 UTC
...
## PG094 Hash/Scope Restore For Canonical Rules - 2026-06-23 10:33 UTC

Status: `applied_validated`.

What changed:

- PG094 did not advance to a new high battle-critical card. The start sync
  exposed hash/scope/effect drift in canonical card-rule metadata, so the
  correct next action was a restore package.
- Restored cards:
  ...
  `Valakut Awakening // Valakut Stoneforge`, and `Wayfarer's Bauble`.
- This was a PostgreSQL source-of-truth correction plus PG -> SQLite/canonical
  sync, not a decklist change and not a new replay baseline.
...
- Precheck:
  ...
  reported 12 expected/resolved target rows, 12 raw hash matches, 1 row needing
  hash restore, 3 rows needing effect restore, and 1 row needing status restore.
- Apply:
  `docs/hermes-analysis/master_optimizer_reports/pg094_hash_scope_restore_apply_20260623_102141.out`
  reported `updated_rows=12`, `restored_rows=12`, and `COMMIT`.
- Postcheck:
  `docs/hermes-analysis/master_optimizer_reports/pg094_hash_scope_restore_postcheck_20260623_102141.out`
  reported 12/12 hash, effect, and status restored rows plus 12 backup rows.
...
- Focused events:
  ...
  prove 12 SQLite rows and 12 runtime-selected logical keys after the final
  sync.
...
- PG095 should return to deck `607` battle-critical high cards before
  support/passive rows.
