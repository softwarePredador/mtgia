...
> **Atualizado:** 2026-06-14 por ManaLoom Knowledge Synthesis (Cron #12)
> **Branch:** codex/hermes-analysis-docs
> **HEAD:** 5b9c361d
> **Metodo:** Cruzamento do conhecimento MTG (edh_bracket_policy.dart v11-categorias, optimization_functional_roles.dart, GAMECHANGER_RESEARCH_REPORT #15, MASTER_OPTIMIZER_PREFLIGHT 2026-06-14, SQLite knowledge.db schema analysis) com codigo Dart vivo
> **Base de conhecimento:** edh_bracket_policy.dart (11 categorias confirmadas, _knownInfiniteComboPieces=3, _knownValueEngineNames=5, officialGameChangerNamesForBracketPolicy=54), optimization_functional_roles.dart (_knownComboPieceNames=6 incl. Underworld Breach), oracle_cache_coverage (mana_cost_filled=2853/3217, 88.7%), SQLite (35/100 deck_cards CMC=0/NULL, Tergrid absent from oracle cache), gamechanger research exec #15 (22/54 GCs missing 40.7%)
> **Novas tasks nesta execucao:** 5 (3xP1, 2xP2) — Underworld Breach in _knownInfiniteComboPieces, _knownValueEngineNames heuristic expansion, SQLite CMC corruption execution, oracle cache mana_cost coverage, _knownInfiniteComboPieces alignment with _knownComboPieceNames
...
## ================================================================
## NOVAS TASKS - 2026-06-14 Knowledge Synthesis (Cron #12)
## ================================================================

> **Base de conhecimento:** edh_bracket_policy.dart (v11 categorias: _knownInfiniteComboPieces=3, _knownValueEngineNames=5, officialGC=54), optimization_functional_roles.dart (_knownComboPieceNames=6 incl. Underworld Breach), master_optimizer_preflight 2026-06-14 (oracle_cache_coverage: mana_cost_filled=2853/3217=88.7%, keywords_filled=1474/3217=45.8%), SQLite knowledge.db (35/100 deck_cards CMC=0/NULL, Tergrid not in oracle cache), gamechanger research exec #15 (22/54 GCs missing=40.7%)
> **Novas tasks nesta execucao:** 5 (3xP1, 2xP2)

---

### [P1] NOVA - Add Underworld Breach to _knownInfiniteComboPieces in bracket policy - Domain Skill Gap 28

**Conhecimento MTG:** Underworld Breach is an official Game Changer (54-card official list) and a premier infinite combo piece in cEDH (with Lion Eye Diamond + Grinding Station, or with Lotus Petal + other 0-cmc artifacts). The functional role classifier (optimization_functional_roles.dart:518) already includes underworld breach in _knownComboPieceNames. The domain skill explicitly documents this discrepancy as Gap 28.

**Evidencia no codigo:**
- server/lib/edh_bracket_policy.dart:347-351 - _knownInfiniteComboPieces has only 3 cards: thassa oracle, demonic consultation, tainted pact. Underworld Breach is NOT in this list.
- server/lib/edh_bracket_policy.dart:405 - Underworld Breach IS in officialGameChangerNamesForBracketPolicy (54-card list).
- server/lib/ai/optimization_functional_roles.dart:518 - Underworld Breach IS in _knownComboPieceNames (6-card list for functional role classification).

**Gap:** Underworld Breach gets tagged as gameChanger (via official list) but NOT as infiniteCombo (because not in _knownInfiniteComboPieces). The bracket policy infinite combo budget is blind to Underworld Breach.

**Impacto:** P1 - In bracket 3 (max 2 infinite combos), a deck with Underworld Breach + LED combo shows infinite combo budget as 0/2 consumed, while actually having 1 infinite combo piece. In bracket 2 (max 0 infinite combos), Underworld Breach is not detected, allowing an illegal combo piece to pass bracket validation.

**Risco:** P1 - Bracket policy incorrectly validates decks with Underworld Breach combos.

**Acao recomendada:**
1. Add underworld breach to _knownInfiniteComboPieces in server/lib/edh_bracket_policy.dart.
2. Consider aligning _knownInfiniteComboPieces (bracket policy) with _knownComboPieceNames (functional roles) to prevent future drift.
3. Add test confirming Underworld Breach returns {gameChanger, infiniteCombo} from tagCardForBracket().
...
### [P1] NOVA - _knownValueEngineNames lacks oracle text heuristic - only 5 hardcoded names, needs generic detection

**Conhecimento MTG:** Value engines generate recurring value each turn cycle. Examples include: Field of the Dead (creates 2/2 zombie each turn), Consecrated Sphinx (draws 2 cards/turn), Seedborn Muse (untaps each turn), Bolas Citadel (play from top of library). Many are also Game Changers (tagged via official list), but the valueEngine bracket category provides additional budget tracking. The _isKnownValueEngineName function only checks 5 hardcoded names with NO oracle text fallback.

**Evidencia no codigo:**
- server/lib/edh_bracket_policy.dart:420-423 - _isKnownValueEngineName() only checks _knownValueEngineNames.
- server/lib/edh_bracket_policy.dart:541-547 - _knownValueEngineNames has only 5 entries: seedborn muse, tergrid, bolas citadel, sensei divining top, aetherflux reservoir.
- No oracle text heuristic exists for value engine detection, unlike other categories (boardWipe, cardAdvantage, stax, protection all have _looksLike* heuristic functions).

**Gap:** Value engine bracket detection is purely name-based. Cards like Field of the Dead (tagged as gameChanger), Gaea Cradle (tagged as fastMana), and Consecrated Sphinx (tagged as gameChanger only) are NOT tagged as valueEngine. Non-GC value engines would be completely invisible to the value engine bracket budget.

**Impacto:** P1 - The value engine bracket budget (max 6 in bracket 3) is effectively non-functional except for 5 hardcoded cards. New value engine cards added to a deck won't consume value engine budget.

**Risco:** P1 - Bracket budget for valueEngine category shows artificially low counts for most decks. Future non-GC value engines go completely undetected.

**Acao recomendada:**
1. Add _looksLikeValueEngine() oracle text heuristic for generic value engine detection.
2. Heuristic patterns: beginning of your upkeep + beneficial effect; at the beginning of each end step + you may.
3. Add Field of the Dead, Consecrated Sphinx to _knownValueEngineNames.
4. Ensure test coverage for the new heuristic.
...
### [P1] NOVA - SQLite CMC data still corrupted - sync script needs execution against actual knowledge.db

**Conhecimento MTG:** The CMC correction tasks (P1 from Cron #10 and previous runs) are marked RESOLVIDO in code but the actual SQLite data remains corrupted. Current state: 35/100 cards in deck_cards have CMC=0 or NULL. The sync_pg_card_metadata_to_hermes.py backfill script exists and is tested, but has never been executed against the production knowledge.db.

**Evidencia no codigo:**
- SQLite query confirms: SELECT COUNT(*) FROM deck_cards WHERE cmc IS NULL OR cmc = 0 returns 35 rows out of 100.
- server/lib/ai/cmc_safety.dart - Code fallback exists but is exercised only when data is already corrupted.
- docs/hermes-analysis/manaloom-knowledge/scripts/sync_pg_card_metadata_to_hermes.py - Script exists, tested in dry-run, but never executed against the live knowledge.db.

**Gap:** The CMC correction code changes are complete, but the actual DATA CORRECTION has never been applied. Every downstream consumer operates on data where 35% of cards have corrupted CMC=0.

**Impacto:** P1 - Despite all code fixes being marked resolved, the production data remains corrupted. All analysis tools produce subtly wrong numbers.

**Risco:** P1 - Highest-impact data integrity issue. Code fix is complete but worthless without execution.

**Acao recomendada:**
1. Run the backfill sync script against the live SQLite:
   python3 sync_pg_card_metadata_to_hermes.py --sqlite-db knowledge.db
2. Verify correction - non-land cards with CMC=0 should drop to only true CMC=0 cards.
3. Add a health check in the cron governor to alert when suspicious_nonland_zero_cmc > 0.
...
import sqlite3
conn = sqlite3.connect('docs/hermes-analysis/manaloom-knowledge/scripts/knowledge.db')
total = conn.execute('SELECT COUNT(*) FROM deck_cards').fetchone()[0]
bad = conn.execute("SELECT COUNT(*) FROM deck_cards WHERE cmc IS NULL OR (cmc = 0 AND type_line NOT LIKE '%land%')").fetchone()[0]
print(f'Total cards: {total}, suspicious CMC=0 non-lands: {bad}')
conn.close()
"
```
...
### [P2] NOVA - Oracle cache sync coverage gap - empty mana_cost (364/3217), keywords (1442/3217), power/toughness (2126/3217)

**Conhecimento MTG:** The card_oracle_cache table in SQLite is the local cache for card data. Empty fields mean certain analyses have incomplete data. Keywords (54.2% empty) affects battle analyst simulation; power/toughness (66.1% empty) affects combat math.

**Evidencia no codigo:**
- master_optimizer_preflight_20260614_134147.md:24 - Coverage report shows only oracle_text (99.9%) and mana_cost (88.7%) well populated.
- The preflight check reports this as oracle_cache_coverage: ok but keywords (45.8%) and power/toughness (33.9%) are critically low.

**Gap:** The preflight check validates existence but not completeness. Keyword coverage at 45.8% means the battle analyst has incomplete data for 54%+ of creature cards.

**Impacto:** P2 - Battle analyst simulations run with incomplete keyword data for the majority of creatures. Flying, trample, deathtouch tracking is missing for 54% of cards.

**Risco:** P2 - The preflight check masks this gap. Battle simulations may be inaccurate without user awareness.

**Acao recomendada:**
1. Add granular coverage thresholds to the preflight check.
2. Fix the sync script to populate keywords, power, toughness from PostgreSQL.
3. Update preflight report format to show field-level coverage status.
...
import sqlite3
conn = sqlite3.connect('docs/hermes-analysis/manaloom-knowledge/scripts/knowledge.db')
total = conn.execute('SELECT COUNT(*) FROM card_oracle_cache').fetchone()[0]
for f in ['keywords_json', 'mana_cost', 'oracle_text', 'power', 'toughness']:
    filled = conn.execute(f'SELECT COUNT(*) FROM card_oracle_cache WHERE {f} IS NOT NULL AND {f} != ""').fetchone()[0]
    print(f'{f}: {filled}/{total} = {filled/total*100:.1f}%')
conn.close()
"
```
...
### [P2] NOVA - _knownInfiniteComboPieces needs expansion beyond Underworld Breach - align with _knownComboPieceNames

**Conhecimento MTG:** The bracket policy _knownInfiniteComboPieces (3 cards) is drastically smaller than the functional role classifier _knownComboPieceNames (6 cards). Both lists serve different purposes, but the overlap should be larger. Cards like Dramatic Reversal + Isochron Scepter (infinite mana) and Basalt Monolith (infinite mana with Rings of Brighthearth or Mesmeric Orb for mill) are well-known infinite combo pieces that consume zero bracket budget.

**Evidencia no codigo:**
- server/lib/edh_bracket_policy.dart:347-351 - _knownInfiniteComboPieces (3 cards): thassa oracle, demonic consultation, tainted pact.
- server/lib/ai/optimization_functional_roles.dart:515-522 - _knownComboPieceNames (6 cards): basalt monolith, dramatic reversal, underworld breach, grand architect, sensei divining top, power artifact.

**Gap:** Dramatic Reversal + Isochron Scepter (well-known infinite combo) passes bracket 2 validation (max 0 infinite combos) because neither card is tagged as infiniteCombo. The bracket policy only covers 3 specific combo pieces.

**Impacto:** P2 - Bracket compliance for infinite combos covers only ~15% of known pieces. cEDH-adjacent decks can pack infinite combos without affecting their bracket.

**Risco:** P2 - Bracket compliance is structurally incomplete for infinite combos beyond the Thoracle/Consultation/TPact suite.

**Acao recomendada:**
1. Expand to include cards from _knownComboPieceNames that are also infinite combo pieces.
2. Add kiki-jiki, helm of the host, walking ballista where they overlap with game changer list.
3. Add cross-reference test ensuring _knownComboPieceNames infinite combo pieces are reflected in _knownInfiniteComboPieces.
4. Long-term: consider oracle-text-based infinite combo detection.
...
| # | Prioridade | Task | Origem |
|:-:|:----------|:-----|:-------|
| 1 | P1 | Add Underworld Breach to _knownInfiniteComboPieces | Domain Skill Gap 28, edh_bracket_policy.dart:347-351 vs :405, optimization_functional_roles.dart:518 |
| 2 | P1 | _knownValueEngineNames - add oracle text heuristic, only 5 hardcoded names | edh_bracket_policy.dart:541-547, no _looksLike* function for value engines |
| 3 | P1 | SQLite CMC backfill execution - 35/100 deck_cards with CMC=0/NULL | SQLite query (code fix resolved but data never corrected) |
| 4 | P2 | Oracle cache sync coverage - keywords 45.8%, mana_cost 88.7%, power/toughness 33.9% | MASTER_OPTIMIZER_PREFLIGHT 2026-06-14 (coverage report) |
| 5 | P2 | _knownInfiniteComboPieces expansion - align with _knownComboPieceNames (6 vs 3 cards) | edh_bracket_policy.dart:347-351 vs optimization_functional_roles.dart:515-522 |

> **Nota:** Tasks #1 e #5 sao complementares - #1 e o fix imediato para Underworld Breach, #5 e a expansao estrutural para alinhar as duas listas.
> **Nota:** Task #2 aborda o gap de deteccao generica de value engines - atualmente so 5 cartas hardcoded sao detectadas, sem fallback de oracle text.
