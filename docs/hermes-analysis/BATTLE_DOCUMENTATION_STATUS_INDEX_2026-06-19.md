...
| `BATTLE_DECISION_TRACE_TAXONOMY.md` | current | Taxonomia atual de tipos de decision trace, campos obrigatorios e dono/auditor por tipo; atualizada para o latest `20260620_014808` e marcada com `BV-085` para separar `170` linhas field-contract-only de linhas strategy/research-specific. |
| `BATTLE_REPLAY_GATE_MATRIX.md` | current | Matriz atual de gates obrigatorios e status final agregado de replay; atualizada para o latest recorrente `20260620_014808`, que esta `review_required` com `mandatory_gate_divergences=["forensic_audit=review_required"]`, 16/16 seeds completas, action critic limpo, strategy sem blockers, mas forensic com `Machine God's Effigy` via `functional_tags_json`/heuristic e lineage incompleta. A matriz mantem `replay.txt` como projecao humana porque `replay_decision_auditor` nao avalia completude humana; tambem explicita que `effect_coverage_unknowns=0` e source-unknown bloqueante, enquanto `effect_coverage_effect_totals_unknown=41` segue como denominador visivel de familias unknown. `BV-081` segue aberto porque o summary ainda nao publica `run_profile`/`run_scope`/`invocation_kind` explicito; `BV-082` segue aberto porque source-coherence learned-deck ainda nao junta 1:1 entre artifacts por chave estavel (`0/11` matches por `source_url`/`row_id`, e `deck_provenance.json` por seed ainda omite `source_url`); `BV-083` segue aberto porque `action_event_types_total=534` e seed-sum, enquanto o global distinto observado e `50`; `BV-084` segue aberto porque `research_review` marca mulligan bloqueado sem publicar samples por seed/decision na propria categoria; `BV-085` segue aberto porque `170` decision traces observadas sao `accepted_field_contract_waiver` sem learning-grade/contador explicito no summary; `BV-086` segue aberto porque o latest recorrente nao esta trusted por forensic review-required; `BV-087` segue aberto porque o backlog de unknown templates esta source-unknown-only enquanto `effect_unknown_cards=34` permanece sem contrato por carta no artifact; `BV-088` segue aberto porque lineage forensic unaccepted ainda nao e condicao direta do gate final quando nao ha findings forensic; `BV-089` segue aberto porque o `replay.txt` humano do latest ainda contem `67` `kind=?` e `1` `cause=?`. |
