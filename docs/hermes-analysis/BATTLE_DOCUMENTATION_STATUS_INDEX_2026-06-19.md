...
| `BATTLE_DECISION_TRACE_TAXONOMY.md` | current | Taxonomia atual de tipos de decision trace, campos obrigatorios e dono/auditor por tipo; atualizada para o latest `20260620_025107`, com `2306` rows e `168` linhas field-contract-only (`BV-085`). |
| `BATTLE_REPLAY_GATE_MATRIX.md` | current | Matriz atual de gates obrigatorios e status final agregado de replay; atualizada para o latest recorrente `20260620_025107`, que esta `trusted_for_strategy_learning` com `mandatory_gate_divergences=[]`, 16/16 seeds completas, action critic limpo, strategy sem blockers, forensic limpo e `11` seeds globalmente elegiveis. A matriz mantem `replay.txt` como projecao humana porque `replay_decision_auditor` nao avalia completude humana; o latest ainda tem `100` `kind=?` no replay humano (`BV-089`). `BV-081` segue aberto porque o summary ainda nao publica `run_profile`/`run_scope`/`invocation_kind` explicito; `BV-082` segue aberto porque source-coherence learned-deck ainda nao junta 1:1 entre artifacts por chave estavel (`0/12` matches por `source_url`/`row_id`, e `deck_provenance.json` por seed ainda omite `source_url`); `BV-083` segue aberto porque `action_event_types_total=520` e seed-sum, enquanto o global distinto observado e `55`; `BV-084` segue aberto porque `research_review` marca mulligan bloqueado sem publicar samples por seed/decision na propria categoria; `BV-085` segue aberto porque `168` decision traces observadas sao `accepted_field_contract_waiver` sem learning-grade/contador explicito no summary; `BV-086` segue aberto como regression coverage/observability para `functional_tags_json`, embora o latest nao reproduza o blocker; `BV-087` segue aberto porque o backlog de unknown templates esta source-unknown-only enquanto `effect_unknown_cards=34` permanece sem contrato por carta no artifact; `BV-088` segue aberto porque lineage forensic unaccepted ainda nao e condicao direta do gate final quando nao ha findings forensic. |
