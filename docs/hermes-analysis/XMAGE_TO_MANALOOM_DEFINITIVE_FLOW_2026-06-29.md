...
## 2026-07-05 PG502 Token Sacrifice Colorless Mana Checkpoint

PG502 closed the exact XMage token subpattern for fixed colorless Eldrazi Spawn
creature tokens whose token class has
`SimpleManaAbility(Zone.BATTLEFIELD, Mana.ColorlessMana(1),
new SacrificeSourceCost())` and matching text
...
- `xmage_authoritative_exact_scope_split.py` now accepts only the exact
  `SimpleManaAbility + Mana.ColorlessMana(1) + SacrificeSourceCost` token
  pattern and emits structured token mana metadata.
- Unsupported token text, unsupported token abilities, infect, prowess, toxic,
  banding, triggered token text, dynamic token counts, additional token fanout,
  and custom token text remain blocked.
- `battle_analyst_v9.py` carries token self-sacrifice mana metadata through
  fixed spell token creation, ETB token creation, dies-token creation, and the
  shared token factory.
- Created tokens reuse the existing
  `xmage_self_sacrifice_mana_source_permanent_v1` contextual activation path
  and vanish instead of entering graveyard when sacrificed.
...
- Split candidate:
  ...
  with `proposal_count=5` and `safe_for_batch_pg_package_count=5`.
- PostgreSQL package:
...
- Precheck:
  ...
  validated one canonical Oracle-hash-matched row for every promoted card and
  found two stale `Skittering Invasion` rows to deprecate.
- Apply:
...
- Postcheck:
  ...
  confirmed all promoted rows as verified/auto with Oracle hashes.
- Field postcheck:
  ...
  `produced_symbols=["C"]` for all five rules.
- PG -> Hermes/SQLite sync:
...
- Runtime cache validation:
  ...
  confirmed all five cards resolve from curated runtime cache with the
  expected scopes and token mana metadata.
- Battle tests:
...
- PG/Hermes/SQLite contract:
  ...
  passed `51/51`.
- Post-sync Commander-legal queue:
  `target_identity_count=26061`, `xmage_authoritative_source_count=25747`,
  `xmage_missing_source_exception_count=314`,
  `xmage_authoritative_parser_gap_count=0`, and
  `xmage_authoritative_adapter_required_count=25747`.
- Post-sync global readiness:
  `battle_and_oracle_ready=4889`, `battle_family_mapper_required=28984`,
  `generic_runtime_or_no_card_rule=360`, `oracle_data_sync=4`,
  `commander_legality_sync=3`, and `oracle_identity_rule_link_or_copy=2`.
- Post-sync exact splitter recheck:
...
Residual boundary: PG502 does not close arbitrary token sacrifice text or all
Eldrazi-like token makers. Only the exact fixed colorless self-sacrifice mana
token pattern is executable. Scion/Sliver variants, non-creature sacrifice
...
## PG515 Removal Compensation Token Checkpoint

As of 2026-07-05, PG515 is applied and synced against the new server target.
It promotes the exact local-XMage pattern where a targeted destroy/exile spell
removes one target and that target's controller creates a fixed simple creature
token.
...
- final exact-scope recheck:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_post_pg515_removal_compensation_tokens_new_server.md`
- global readiness:
  `docs/hermes-analysis/master_optimizer_reports/global_card_oracle_battle_readiness_20260705_post_pg515_removal_compensation_tokens_new_server.md`
- authoritative queue:
...
Residual boundary: PG515 does not authorize non-creature compensation tokens,
changeling tokens, unsupported auxiliary abilities, broader removal/exile
...
## PG519 Counter Gain Life Checkpoint

As of 2026-07-05, PG519 is applied and synced against the new server target.
It promotes exact local-XMage instant spells where the spell counters target
spell and the controller gains a fixed amount of life when the counter resolves.
...
- exact split report:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_pg519_counter_gain_life_candidate.md`
- apply evidence:
  `docs/hermes-analysis/master_optimizer_reports/pg519_xmage_pg519_counter_gain_life_new_server_apply_evidence.md`
- PostgreSQL -> SQLite sync:
...
- runtime smoke:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg519_counter_gain_life_new_server_runtime_get_card_effect.out`
- final exact-scope recheck:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_post_pg519_counter_gain_life_new_server.md`
- global readiness:
  `docs/hermes-analysis/master_optimizer_reports/global_card_oracle_battle_readiness_20260705_post_pg519_counter_gain_life_new_server.md`
- authoritative queue:
...
Residual boundary: PG519 does not authorize counterspells with extra targets,
counter-unless clauses, mode/conditional/additional cost variants, dynamic life
gain, non-spell stack objects, activated or triggered counter abilities, or
broad `xmage_*_review_v1` promotions. The next wave must start from the
rebuilt post-PG519 queue.

## PG520 Counter Unless Pays Checkpoint

As of 2026-07-05, PG520 is applied and synced against the new server target.
It promotes exact local-XMage instant/sorcery one-shot counterspells where the
spell counters target spell or nonartifact spell unless the target controller
pays a fixed generic mana tax.
...
- exact split report:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_pg520_counter_unless_pays_candidate.md`
- apply evidence:
  `docs/hermes-analysis/master_optimizer_reports/pg520_xmage_pg520_counter_unless_pays_new_server_apply_evidence.md`
- PostgreSQL -> SQLite sync:
  `docs/hermes-analysis/master_optimizer_reports/battle_card_rules_sqlite_from_pg_pg520_counter_unless_pays_new_server.json`
- battle package E2E:
  `docs/hermes-analysis/master_optimizer_reports/battle_package_end_to_end_validation_20260705_pg520_counter_unless_pays_new_server.md`
- runtime smoke:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg520_counter_unless_pays_new_server_runtime_get_card_effect.out`
- focused unittest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg520_counter_unless_pays_new_server_unittest.out`
- final exact-scope recheck:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_post_pg520_counter_unless_pays_new_server.md`
- global readiness:
  `docs/hermes-analysis/master_optimizer_reports/global_card_oracle_battle_readiness_20260705_post_pg520_counter_unless_pays_new_server.md`
- authoritative queue:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_adaptation_queue_20260705_post_pg520_counter_unless_pays_new_server_commander_legal.md`

Post-sync queue evidence:

- `battle_and_oracle_ready=4979`
- `battle_family_mapper_required=28894`
...
Residual boundary: PG520 does not authorize dynamic or non-fixed counter-unless
taxes, X taxes, domain/count/devotion formulas, modal counterspells,
exile-if-countered variants, extra targets, non-spell stack objects, activated
or triggered counter abilities, or broad `xmage_*_review_v1` promotions. The
next wave must start from the rebuilt post-PG520 queue.

## PG521 X Create Tokens Checkpoint

As of 2026-07-05, PG521 is applied and synced against the new server target.
It promotes exact local-XMage one-shot token spells whose source creates
non-land creature tokens equal to the paid X value.
...
- `xmage_x_create_creature_tokens_spell_v1`

Evidence:

- exact split report:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_pg521_x_token_candidate.md`
- apply evidence:
  `docs/hermes-analysis/master_optimizer_reports/pg521_xmage_pg521_x_create_tokens_new_server_apply_evidence.md`
- PostgreSQL -> SQLite sync:
  `docs/hermes-analysis/master_optimizer_reports/battle_card_rules_sqlite_from_pg_pg521_x_create_tokens_new_server.json`
- battle package E2E:
  `docs/hermes-analysis/master_optimizer_reports/battle_package_end_to_end_validation_20260705_pg521_x_create_tokens_new_server.md`
- runtime smoke:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg521_x_create_tokens_new_server_runtime_get_card_effect.out`
- focused unittest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg521_x_create_tokens_new_server_unittest.out`
- final exact-scope recheck:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_post_pg521_x_create_tokens_new_server.md`
- global readiness:
  `docs/hermes-analysis/master_optimizer_reports/global_card_oracle_battle_readiness_20260705_post_pg521_x_create_tokens_new_server.md`
- authoritative queue:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_adaptation_queue_20260705_post_pg521_x_create_tokens_new_server_commander_legal.md`
...
Residual boundary: PG521 does not authorize land creature tokens, contextual
dynamic token counts, token classes with unsupported keyword/runtime behavior,
multiple token effects, `.withAdditionalTokens`, or broad
`xmage_*_review_v1` promotions. `Awaken the Woods` remains intentionally
...
## PG524 Tap Plus Self-Sacrifice Mana Checkpoint

As of 2026-07-05, PG524 is applied and synced against the new server target.
It promotes exact local-XMage artifact mana sources with one normal fixed-color
tap mana ability and one separate fixed-color self-sacrifice mana ability.
...
- exact split report:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_pg524_tap_and_self_sacrifice_mana_candidate.md`
- package:
  `docs/hermes-analysis/master_optimizer_reports/pg524_tap_and_self_sacrifice_mana_new_server_package_package.md`
- apply evidence:
  `docs/hermes-analysis/master_optimizer_reports/pg524_tap_and_self_sacrifice_mana_new_server_apply_evidence.md`
- PostgreSQL -> SQLite sync:
  `docs/hermes-analysis/master_optimizer_reports/battle_card_rules_sqlite_from_pg_pg524_tap_and_self_sacrifice_mana_new_server.json`
- battle package E2E:
  `docs/hermes-analysis/master_optimizer_reports/battle_package_end_to_end_validation_20260705_pg524_tap_and_self_sacrifice_mana_new_server.md`
- runtime smoke:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg524_tap_and_self_sacrifice_mana_new_server_runtime_smoke.out`
- focused unittest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg524_tap_and_self_sacrifice_mana_new_server_unittest.out`
- authoritative queue:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_adaptation_queue_20260705_post_pg524_tap_and_self_sacrifice_mana_new_server_commander_legal.md`
- global readiness:
  `docs/hermes-analysis/master_optimizer_reports/global_card_oracle_battle_readiness_20260705_post_pg524_tap_and_self_sacrifice_mana_new_server.md`
- final exact-scope recheck:
...
- `battle_and_oracle_ready=4992`
- `battle_family_mapper_required=28881`
- `target_identity_count=25958`
- `xmage_authoritative_source_count=25644`
- `xmage_missing_source_exception_count=314`
- `xmage_authoritative_parser_gap_count=0`
- `xmage_authoritative_adapter_required_count=25644`
- final exact-scope recheck `proposal_count=0`
- final exact-scope recheck `safe_for_batch_pg_package_count=0`

Residual boundary: PG524 only authorizes one fixed-color tap ability plus one
fixed-color self-sacrifice mana ability with no sacrifice activation mana cost.
It does not authorize `Kaleidostone`, which combines ETB draw with `{5}, {T},
Sacrifice` for five fixed colors, nor broader artifact mana-source review rows,
pay-life/discard/target sacrifice costs, conditional mana, or broad
`xmage_*_review_v1` promotions.

## PG525 Dies Fixed Mana Checkpoint

As of 2026-07-05, PG525 is applied and synced against the new server target.
It promotes exact local-XMage permanents with one non-optional
`DiesSourceTriggeredAbility` carrying one fixed `BasicManaEffect`.
...
- `xmage_permanent_dies_add_fixed_mana_v1`

Evidence:

- exact split report:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_exact_scope_split_20260705_pg525_dies_fixed_mana_new_server_candidate.md`
- package:
  `docs/hermes-analysis/master_optimizer_reports/pg525_dies_fixed_mana_new_server_package_package.md`
- apply evidence:
  `docs/hermes-analysis/master_optimizer_reports/pg525_dies_fixed_mana_new_server_apply_evidence.md`
- PostgreSQL -> SQLite sync:
  `docs/hermes-analysis/master_optimizer_reports/battle_card_rules_sqlite_from_pg_pg525_dies_fixed_mana_new_server.json`
- battle package E2E:
  `docs/hermes-analysis/master_optimizer_reports/battle_package_end_to_end_validation_20260705_pg525_dies_fixed_mana_new_server.md`
- runtime smoke:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg525_dies_fixed_mana_new_server_runtime_smoke.out`
- focused unittest:
  `docs/hermes-analysis/master_optimizer_reports/xmage_pg525_dies_fixed_mana_new_server_unittest.out`
- authoritative queue:
  `docs/hermes-analysis/master_optimizer_reports/xmage_authoritative_adaptation_queue_20260705_post_pg525_dies_fixed_mana_new_server_commander_legal.md`
- global readiness:
  `docs/hermes-analysis/master_optimizer_reports/global_card_oracle_battle_readiness_20260705_post_pg525_dies_fixed_mana_new_server.md`
- final exact-scope recheck:
...
- `battle_and_oracle_ready=4995`
- `battle_family_mapper_required=28878`
...
Residual boundary: PG525 only authorizes fixed, non-optional dies-trigger mana
where Oracle and XMage agree on exact symbols and count. It does not authorize
conditional dies triggers, optional mana triggers, dynamic mana, ETB conditioned
...
## PG555 Independent Mana Auxiliary New Server Evidence
...
- focused code checks passed: `py_compile`, `54` package/E2E pytest tests, and
  `621` exact-scope splitter unittest tests;
- exact split selected `5` cards for
  `xmage_simple_mana_source_with_unmodeled_auxiliary` under
  `xmage_simple_tap_mana_source_permanent_v1`;
- package precheck found `5/5` target card rows and no existing matching rows;
- package apply upserted `5` rows and deprecated `0` shadow rows;
- PostgreSQL postcheck confirmed `5/5` promoted rows, `5/5`
  verified/auto, and `5/5` oracle hash rows;
- PG -> SQLite sync loaded `8,953` PostgreSQL rows, updated `8,717` SQLite
  rows, and exported `6,455` canonical snapshot rows;
- package E2E: `status=pass`, `scenario_count=5`, `event_count=5`, and
...
- pre-cycle `target_identity_count=25543`
- post-cycle `target_identity_count=25538`
- post-cycle `xmage_authoritative_source_count=25224`
- post-cycle `xmage_missing_source_exception_count=314`
- post-cycle `xmage_authoritative_parser_gap_count=0`
- post-cycle `xmage_authoritative_adapter_required_count=25224`
- post-cycle `adapter_work_unit_count=11354`

Runtime semantics:

- the splitter now accepts a direct independent safe mana ability on a
  permanent even when a separate auxiliary activated ability has
  `SacrificeSourceCost`;
- only the mana ability is executable; auxiliary draw, destroy, damage, or
  recursion effects remain explicitly unmodeled metadata;
- the guard still blocks cases where sacrifice, delayed draw, or another
  auxiliary effect is part of the mana ability itself.

Residual boundary: PG555 does not authorize Astrolabe/Sextant-style mana
abilities where sacrifice/draw is coupled to mana production, conditional mana
