...
| S8-03 | `IN_PROGRESS` | S2, S4 | `/root` | `CachedCardImage`; cache/eviction; `app/integration_test/image_memory_runtime_test.dart`; probe CDP/process/RSS/rede; evidência Sprint 8 | memory/image profile | evidência Sprint 8 — prova física anterior de 180 imagens Samsung permanece válida; probe Web específico implementado e integrado aos contratos 15/15, mas o profile Chrome 150 falhou fechado: seis fallbacks/`EncodingError` e redownload de 15.677.244 bytes no percurso repetido, apesar de RSS/heap dentro dos orçamentos; faltam correção Web + reexecução verde, Android final e perfil completo |
