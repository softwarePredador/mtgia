...
| S3-05 | `PASS` | S3-01 | `/root` | `app/test/ui/fixtures/ui_keyboard_focus_matrix.json`; `app/test/ui/ui_keyboard_focus_matrix_test.dart`; `app/lib/features/profile/profile_screen.dart`; `app/lib/features/auth/screens/login_screen.dart`; testes de teclado/foco Web; `app/doc/UI_TEST_SURFACE_MAP.md`; `docs/qa/ui-live/current/battle-coach-web-keyboard`; `docs/qa/MANALOOM_P0_UI_LIVE_EVIDENCE_2026-07-27.md` | keyboard/focus Web | build Web release real: Battle Coach aprovado em 9/9 checkpoints com Tab/Shift+Tab/Enter/Space/Escape, foco visível, digitação, filtro, seleção/preflight e restauração após modal; log sanitizado, console sem entrada proibida e manifesto vinculado ao digest corrente |
...
| S3-07 | `PASS` | S3-02–S3-06 | `/root` | `docs/MANALOOM_UI_LIVE_EVIDENCE_CONTRACT.md`; `scripts/manaloom_ui_live_evidence_gate.sh`; `app/tool/ui_runtime_evidence.dart`; harness visual autenticado isolado; fixture determinística; matriz/guard; evidência corrente com digest | visual live-evidence gate | matriz P0 corrente aprovada: 54 Web mobile + 53 desktop + 53 wide + 54 Android emulator, acrescida de 12 estados Core Product Android, 7 estados Battle Coach Android e 9 checkpoints de teclado Web; `docs/qa/ui-live/latest.json` fecha `PASS_AUTOMATED` + `PASS_RUNTIME` + `PASS_VISUAL_REVIEWED` sobre 242/242 PNGs no mesmo digest, sem atribuir crédito físico ao emulador |
...
| S5-10 | `PASS` | S5-01 | `/root` | evidências Sprint 5 e auditoria XMage/Forge; outputs candidatos em `/tmp` | `quality_gate.sh engine-delta` + capability gate | 95/95 checks; fonte local explícita/limpa/pinada; auditor reexecutado com 316 cartas/328 fixtures (XMage 120/131, Forge 196/197), ambos +113 commits; `docs/qa/MANALOOM_ENGINE_DELTA_REVIEW_2026-07-23.md` registra `retain_current_pins` por delta XMage client-only e Forge regressivo/não qualificado, sem promoção/deploy |
...
| S8-02 | `IN_PROGRESS` | S4, S6 | `/root` | `PerformanceService`; harness core Web/device; `quality_gate.sh performance`; budgets p50/p95; evidência Sprint 8 | p50/p95 benchmark | evidência Sprint 8 — startup Web Chrome 150 revalidado em 7 amostras (cold p50/p95 622/660 ms; warm 222/238 ms), harness 4/4 + Flutter 32/32; contrato do harness integrado ao `full`; faltam startup Android e as oito superfícies autenticadas Web+device |
| S8-03 | `IN_PROGRESS` | S2, S4 | `/root` | `CachedCardImage`; cache/eviction; `app/integration_test/image_memory_runtime_test.dart`; probe CDP/process/RSS/rede; evidência Sprint 8 | memory/image profile | evidência Sprint 8 — prova física anterior de 180 imagens Samsung permanece válida; profile Chrome 150 passou com 180 imagens, zero fallback/timeout, cache/RSS/heap nos orçamentos e 0 bytes transferidos na repetição após corrigir e cobrir os cabeçalhos reais da fixture; faltam Android final e incorporação dos dois alvos ao perfil completo |
| S8-04 | `IN_PROGRESS` | S4-04, S6 | `/root` | `server/lib/ai_generate_provider_abort.dart`; `server/routes/ai/generate/index.dart`; lifecycle de jobs; testes de timeout/abort; contrato API; evidência Sprint 8 | AI failure injection | evidência Sprint 8 — timeout/cancelamento abortam fisicamente o request OpenAI; `DELETE` propaga pelo estado durável PostgreSQL; retry terminal limpa job/request key; analyzer, 45/45 focados, `ai-bridge` 114/114 e `full` verdes em `b84302e1e`; falta somente a matriz E2E externa completa na SHA final |
| S8-05 | `BLOCKED` | S4, S5, S6 | `/root` | observabilidade app/server; request ID; redaction; health/readiness; evidência Sprint 8 | Sentry/request-id proof | SHA de implementação limpa existe; bloqueado até existir ambiente publicado/Sentry para provar evento e correlação app→API→erro da mesma revisão |
...
| S8-08 | `BLOCKED` | S1, S8-05 | `/root` | dependency audit; secret scan; SBOM/OSV; auth/runtime policy; evidência Sprint 8 | security/SBOM/OSV | evidência Sprint 8 — `deps`, secret scan e source-lock SBOM/OSV determinístico verdes (Android 539 / Web combinado 954 componentes, 0 vulnerabilidades de release); segue bloqueado por S8-05, ausência de APK/AAB final, identidade `HEAD != origin/master`, SBOM do artefato e rejeição de token antigo pós-deploy |
...
| ID | Estado | Depende de | Owner | Arquivos pretendidos | Gate mínimo | Evidência |
|---|---|---|---|---|---|---|
| S9-01 | `IMPLEMENTED_UNPROVEN` | S8 | `/root` | manifestos/índices de retenção; 23 conteúdos canônicos; auditor e testes | consumer/retention inventory | evidência Sprint 9 + `docs/qa/MANALOOM_CLEAN_SHA_PUBLICATION_2026-07-23.md` — 942 fontes classificadas, 0 não governadas, recuperação integral e commit atômico `2139ec9f6`; S8 continua aberta |
| S9-02 | `IMPLEMENTED_UNPROVEN` | S9-01 | `/root` | `docs/CONTEXTO_PRODUTO_ATUAL.md`; plano/tracker; referências históricas/correntes | status/link consistency | evidência Sprint 9 — contexto/project logic sincronizados e 7.378 referências validadas sem link inválido; permanece dependente de S9-01 |
| S9-03 | `IMPLEMENTED_UNPROVEN` | S9-01 | `/root` | contratos/índices XMage; auditor de retenção; conteúdo deduplicado | canonical-auditor regression | evidência Sprint 9 — três entrypoints gigantes reduzidos de 26.263 para 496 linhas, 1.584.706 bytes históricos preservados e selados, regressões canônicas verdes e retenção 17/17; aceite local concluído, estado depende de S9-01 |
| S9-04 | `IMPLEMENTED_UNPROVEN` | S9-01 | `/root` | rotas/imports sem consumidor; compatibility routes; source reachability; gates | route/import consumers + gates | evidência Sprint 9 — `deps`, `server-target`, `full` e E2E determinístico sem falhas; gate completo repetido em `2139ec9f6`; permanece dependente de S9-01 |
| S9-05 | `IMPLEMENTED_UNPROVEN` | S9-01 | `/root` | duplicatas/efêmeros; manifestos de recuperação; gate de retenção | `quality_gate.sh report-retention` | evidência Sprint 9 — 12/12 checks, 17/17 testes, 0 duplicatas/resíduos; índices+canônicos+deleções rastreados atomicamente em `2139ec9f6` |
| S9-06 | `IMPLEMENTED_UNPROVEN` | S9-02–S9-05 | `/root` | documentação operacional; links; secret scan; project logic | docs/link/secret audit | evidência Sprint 9 — secret scan, project logic, 7.378 links, diff congelado e gate completo verdes; S9-03 fechada localmente, resta a cadeia de dependências |

## Sprint 10

| ID | Estado | Depende de | Owner | Arquivos pretendidos | Gate mínimo | Evidência |
|---|---|---|---|---|---|---|
| S10-01 | `IMPLEMENTED_UNPROVEN` | S0–S9 | `/root` | plano/tracker; flags/rotas/manifesto release; matriz de acessibilidade | clean SHA/scope manifest | `docs/qa/MANALOOM_CLEAN_SHA_PUBLICATION_2026-07-23.md` — escopo Web+Android, scanner inacessível e iOS/VoiceOver `DEFERRED_BY_SCOPE`; S9-03 fechada localmente, mas S8 e a cadeia S9 ainda impedem o fechamento |
| S10-02 | `IMPLEMENTED_UNPROVEN` | S10-01 | `/root` | scripts/gates locais; testes release/readiness; summary final | deterministic gates + summary | `full` PASS em `b84302e1e` (backend 1742, Flutter 1159 + 1 skip, Web, Patrol e schema 51); `ai-eval`, `ai-bridge`, Battle 2× e `web` PASS; E2E determinístico `PARTIAL` com 10 PASS, 9 skips guardados e 0 falha/bloqueio; `docs/qa/MANALOOM_ENGINE_DELTA_REVIEW_2026-07-23.md` fecha a revisão com `retain_current_pins` (XMage incremental client-only; Forge não qualificado/regressivo e compare truncado); resta S10-01 |
