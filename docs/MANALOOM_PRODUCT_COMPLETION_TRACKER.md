...
| S3-05 | `IN_PROGRESS` | S3-01 | `/root` | `app/test/ui/fixtures/ui_keyboard_focus_matrix.json`; `app/test/ui/ui_keyboard_focus_matrix_test.dart`; `app/lib/features/profile/profile_screen.dart`; `app/lib/features/auth/screens/login_screen.dart`; testes de teclado/foco Web; `app/doc/UI_TEST_SURFACE_MAP.md`; `docs/qa/MANALOOM_SPRINT3_UX_EVIDENCE_2026-07-21.md` | keyboard/focus Web | a evidência histórica permanece válida para Home/Decks/Collection/Community/Profile/Battle-Replays; a nova superfície Battle Coach tem automação e Android físico aprovados, mas ainda exige roteiro Web real de Tab/Shift+Tab/Enter/Space/Escape, foco visível e restauração antes de recuperar `PASS` |
...
| S3-07 | `IN_PROGRESS` | S3-02–S3-06 | `/root` | `docs/MANALOOM_UI_LIVE_EVIDENCE_CONTRACT.md`; `scripts/manaloom_ui_live_evidence_gate.sh`; `app/tool/ui_runtime_evidence.dart`; harness visual autenticado isolado; fixture determinística; matriz/guard; evidência corrente com digest | visual live-evidence gate | os 80 PNGs de 2026-07-21 são baseline histórica e não provam o source atual; Battle Coach fecha o primeiro recorte corrente com `PASS_AUTOMATED` + `PASS_RUNTIME` no Samsung SM-A135M + `PASS_VISUAL_REVIEWED` em 7/7 hashes (`docs/qa/ui-live/latest.json`); faltam recaptura e revisão da matriz P0 ampla em Web real e Android para recuperar `PASS` global |
