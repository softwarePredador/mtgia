...
| S8-03 | `IN_PROGRESS` | S2, S4 | `/root` | `CachedCardImage`; cache/eviction; `app/integration_test/image_memory_runtime_test.dart`; evidência Sprint 8 | memory/image profile | evidência Sprint 8 — prova física anterior de 180 imagens Samsung permanece válida; revalidação Chrome falhou fechado porque `HtmlImage` deixa `PaintingBinding.imageCache` em 0, invalidando essa métrica Web; faltam probe Web apropriado, repetição Android final e perfil completo |
