...
| `DCK-P0-01` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | Revisão otimista de deck + ledger imutável de mudanças + undo universal. | `BT-DB-001` | Todo mutator usa expected revision/If-Match; uma de duas concorrentes vence; outra 409; retry não duplica; receipt atômico evita segunda validação; undo recusa HEAD novo. Decisão do dono em 2026-09-22: D-29, `If-Match` com transição: primeiro só avisa; passa a ser exigido quando a versão mínima do app mandar o cabeçalho. Parte do servidor feita em 2026-09-24 na branch `deck/rodada1-2026-09-24`, ainda fora do ar, com a migration 067 proposta e não aplicada: `decks.revision` e o ledger `deck_change_events`, que só recebe INSERT (um gatilho recusa UPDATE e DELETE direto; as linhas saem só na cascata do deck ou da conta). Todo mutator de deck existente (`PUT` e `PATCH /decks/:id`, `cards`, `cards/set`, `cards/replace`, `cards/remove`, `cards/bulk`, o rollback do Optimize e `POST /import/to-deck`) trava o deck, confere o `If-Match` antes de escrever (409 `deck_revision_conflict` com a revisão atual), sobe a revisão e grava no ledger só o que mudou, na mesma transação da mudança, e devolve o recibo (`revision`, `change_event_id`) com o `ETag`; o `DELETE` confere o `If-Match` e não grava evento. Transição da D-29: sem o cabeçalho a mudança segue com `revision_warning: if_match_missing`; `MANALOOM_DECK_IF_MATCH_REQUIRED=1` passa a exigir (428 `deck_revision_required`). Retry: a mesma `Idempotency-Key` devolve o recibo gravado sem aplicar de novo, e a mesma chave com outro pedido responde 422. Desfazer universal: `POST /decks/:id/changes/:eventId/undo` devolve o estado de antes como mudança nova, recusa HEAD novo (409 `deck_undo_conflict`), nunca publica e passa pelas regras de hoje (409 `deck_undo_invalid`); `GET /decks/:id/changes` lista o histórico com o que mudou. O artefato do Optimize passou a ligar a revisão (`DCK-P0-02`), e o `PUT` com prévia velha do Optimize responde 409, como o bulk. Evidência: `server/test/deck_revision_support_test.dart`, `server/test/deck_revision_ledger_db_live_test.dart` (PostgreSQL descartável: todo mutator, duas mudanças concorrentes com um vencedor e um 409, retry, transição, desfazer, histórico e ledger só de acréscimo), `server/test/deck_review_artifact_db_live_test.dart` e os testes de privacidade com o ledger na exportação e na exclusão; 25 mutações derrubadas. A validação estrita dentro de cada mudança fica com o `DCK-P1-04`, como a medição de 2026-09-22 apontou. Falta: o app mandar `If-Match` e a chave, ler o recibo e mostrar o histórico e o desfazer (raia do app); aplicar a 067 na produção pede a palavra do dono. |
| `DCK-P0-02` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | `DeckReviewArtifact v1` único para preview/commit, com HMAC, revisão, hash, constraints e expiração. | `DCK-P0-01` | Qualquer mudança semântica invalida; outro usuário/tamper/stale/expired falha. Decisão do dono em 2026-09-22: D-29, artifact de preview com TTL de 24 h, reutilizável pelo mesmo usuário. Feito em 2026-09-24 na branch `deck/rodada1-2026-09-24`, ainda fora do ar: `server/lib/decks/deck_review_artifact.dart` é o `DeckReviewArtifact v1` (token com prefixo `drv1`, payload e assinatura em base64url; HMAC-SHA256 sobre prefixo e payload; 24 h, reutilizável pelo mesmo usuário enquanto vale, pela D-29). Ele liga dono, deck, revisão e assinatura do conteúdo do deck no preview, `input_hash` e `constraints_hash` canônicos (SHA-256 com as chaves em ordem) e o tipo do fluxo; revisão e assinatura do commit têm de ser as do preview, inclusive quando ausentes. A autorização de apply do Optimize passou a ser esse artefato, do tipo `optimize_apply`, emitido só para o usuário autenticado e verificado no `PUT /decks/:id` e no `POST /decks/:id/cards/bulk` antes de qualquer escrita, com o motivo em `authorization_error`; o token `v2` antigo é recusado, e o app segue devolvendo o objeto sem mexer. Evidência: `server/test/deck_review_artifact_test.dart` (cada recusa com seu código), `server/test/optimize_apply_authorization_test.dart` (dono e estado velho) e `server/test/deck_review_artifact_db_live_test.dart` (PostgreSQL descartável: controle positivo e, relendo cartas, validação e histórico de otimização, nenhuma escrita com artefato adulterado, expirado, de outro usuário, de outro deck ou de estado velho), com 14 mutações derrubadas. Com o `DCK-P0-01`, o artefato liga também a revisão do deck: mudança só de nome ou formato depois da prévia dá `stale_deck_revision`. O mesmo artefato no import sobre deck existente, no import novo, no Generate e no Rebuild fica com o `DCK-P0-03`, o `DCK-P1-02`, o `DCK-P0-04` e o `DCK-P1-08`, como a medição de 2026-09-22 separou. |
| `DCK-P0-03` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | Importar em deck existente em duas fases, diff completo e undo. | `DCK-P0-01`, `DCK-P0-02` | Zero delete/insert antes de confirmar; falha preserva original; stale retorna 409. Decisão do dono em 2026-09-22: D-29, o preview de import em deck existente, que não grava nada, fica sob `decks_private`; o commit continua sob `deck_replace_all`; na beta, o import entra por deck novo (`/import`). Parte do servidor feita em 2026-09-24 na branch `deck/rodada2-2026-09-24`, ainda fora do ar, sem migration: `POST /import/to-deck/preview` (sob `decks_private`) lê e resolve a lista, calcula a lista final (soma ou troca, com o comandante mantido quando a troca não traz um), a diferença completa (o que entra, sai e muda, com nome, quantidade, papel e condição, e os totais) e a validação da lista final numa transação só de leitura, e emite o `DeckReviewArtifact v1` do tipo `import_to_deck`, que carrega a lista final e liga dono, deck, revisão e conteúdo. `POST /import/to-deck` (sob `deck_replace_all`) só aplica com esse artefato: sem ele responde 428 `import_review_required`; deck mudado depois da prévia dá 409 `import_preview_stale`; artefato adulterado, expirado ou de outro deck dá 409 `import_review_invalid`; regra que passou a falhar depois da prévia dá 400; em todos, nada é escrito. Confirmado, grava só as linhas que mudam e o evento `import_to_deck` do ledger, que o desfazer universal volta. Evidência: `server/test/import_to_deck_two_phase_db_live_test.dart` (PostgreSQL descartável, 9 casos: prévia sem escrita, commit exato, comandante mantido, stale, adulterado, expirado, de outro deck, sem artefato, regra que falha depois, desfazer exato, deck de outra pessoa e lista sem mudança), `server/test/import_to_deck_merge_support_test.dart` e 11 mutações derrubadas. Falta: o app pedir a prévia, mostrar a diferença, confirmar com o artefato e oferecer o desfazer (raia do app; o app de hoje manda a lista direto e recebe 428). |
...
| `DCK-P1-04` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | Uma única verdade de validação/readiness estrita por revisão. | `DCK-P0-01` | UI nunca diz “pronto” sem strict atual; ausência de legalidade não vira legal. Medição de 2026-09-22 (`docs/flows/_p0/deck-core.md`): metade pode andar hoje (H4/H7); tornar legalidade ausente bloqueante pode rebaixar decks validados e é decisão do dono. Produção em 2026-09-22 (`docs/qa/execution/2026-09-22/prod-readonly-estrutura-e-contagens.md`): 0 de 15 decks validados seriam rebaixados se legalidade ausente passasse a bloquear. Decisão do dono em 2026-09-22: D-28, tornar bloqueante agora; medido na produção, 0 de 15 decks validados seriam rebaixados. Parte do servidor feita em 2026-09-24 na branch `deck/rodada2-2026-09-24`, ainda fora do ar, com a migration 068 proposta e não aplicada. D-28: na validação estrita, carta sem linha de legalidade no formato recusa com `legality_unknown` (o `/validate` persiste o motivo; criar deck e importar deck novo gravam `draft` com ele); montar o deck segue aceitando a carta. Strict atual: a 068 põe gatilhos por comando em `card_legalities` (inserir, mudar e apagar), e o deck `validated` do formato que tem a carta volta a `draft` com `card_legality_changed` na mesma transação, qualquer que seja o escritor; regravar o mesmo status não rebaixa. Mesma resposta: todo recibo de mudança de deck traz `readiness` lido na transação da mudança; troca 1 por 1 de edição e mudança de cartas ou formato voltam `draft` pelos gatilhos, e mudar só o nome mantém `validated`. A análise (`GET /decks/:id/analysis`) não diz válido sem o estado estrito (`awaiting_strict_validation`, `strict_validated`) e trata legalidade ausente e `not_legal` como erro. Os seletores de candidatos do Optimize (12 filtros em 3 arquivos e o filtro por nome) deixaram de admitir carta sem linha legal ou restrita, como a D-28 pediu. Evidência: `server/test/deck_strict_readiness_db_live_test.dart` (PostgreSQL descartável, 7 casos), `server/test/deck_readiness_contract_test.dart`, `server/test/deck_validation_route_support_test.dart` e `server/test/optimize_legality_selectors_contract_test.dart`, com as 8 mutações do grupo `dck104` e 5 mutações dos gatilhos da 068 (banco refeito de uma cópia mutada) derrubadas. Falta: o app não dizer pronto sem `deck_state=validated` (painel de diagnóstico, H3), rotular a lista pelo estado do servidor (H5), usar o `readiness` do recibo e traduzir os motivos novos (raia do app); o receipt do live histórico (`deck_validation_state_live_test.dart`) com política isolada fica para a sessão do gate; aplicar a 068 na produção pede a palavra do dono. |
