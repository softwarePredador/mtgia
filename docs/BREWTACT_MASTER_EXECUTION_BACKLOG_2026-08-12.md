...
| `BT-DB-004` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | Proibir DDL runtime/fora de migrations e tombstonar resets destrutivos. | `BT-DB-001` | `update_schema.dart` já falha fechado; sync/backfill/Commander CLIs não executam CREATE/ALTER/DROP; `sync_state` tem um único DDL; schema só muda por migration+gate; auditor cobre todos os entrypoints. Evidência: `server/bin/update_schema.dart` (tombstone fail-closed). Estado corrigido em 2026-09-22 de `IN_PROGRESS_CONTAINED` para `TODO`: a contenção não vale para a maior parte da superfície — `update_schema.dart` falha fechado, mas 11 CLIs, `setup_database.dart` e 47 pacotes SQL versionados ainda alteram schema fora de migration; ver `docs/flows/_p0/banco.md`. Decisão do dono em 2026-09-22: D-48, `setup_database.dart` vira tombstone fail-closed; `extract_meta_insights --full` passa a exigir aprovação; `database_indexes.sql` é aposentado (os índices que a produção tiver viram migration); os 47 pacotes SQL vão para o arquivo e param de criar tabelas em `manaloom_deploy_audit`; `sync_state` com um único DDL. Feito em 2026-09-24, na branch `banco/rodada1-2026-09-24`, com as migrations 063 a 065 propostas e não aplicadas (aplicar na produção pede a palavra do dono): `setup_database.dart` virou tombstone (sai com 2 sem abrir conexão); `extract_meta_insights --full` para antes de conectar sem `MANALOOM_CONFIRM_POSTGRES_WRITES`; os 11 CLIs de sync, backfill, Commander, candidate quality e nomes localizados, o `audit_data_model_links.dart` e o Hermes `sync_battle_card_rules_pg.py` não executam DDL no PostgreSQL: conferem o catálogo só lendo e param com a lista do que falta (`server/lib/schema_requirements.dart` e as guardas Python); o DDL de schema saiu de `server/lib`; `sync_state` tem um único DDL, o do `database_setup.sql`, que é o da produção; `database_indexes.sql` foi aposentado; os 47 pacotes `*_apply.sql` ficam no arquivo de evidência, e um pacote novo com DDL falha no auditor. D-65: as 8 migrations que interpolavam constantes de `server/lib` (022, 023, 024, 025, 028, 029, 032 e 045) foram congeladas com o texto que o runner via (up e down das 60 migrations iguais byte a byte antes e depois), um teste exige SQL literal em toda migration, e a 063 recria `commander_learning_snapshot` por extenso, com os filtros que faltam na produção. D-67: a 064 adota 5 dos 6 `UNIQUE` só da produção, com os mesmos nomes; o `uq_binder_user_card_cond_foil_list` ficou de fora, pendente de decisão, porque não tem `language` e contradiz a identidade física da 049 (ADR 0008): na produção, a mesma carta em outro idioma dá 23505 fora do `ON CONFLICT` de `POST /binder`, que responde 500. A contagem de nulos das 26 colunas, só leitura, está em `server/sql/readonly/d67_not_null_null_counts.sql` para a coordenação rodar na produção. D-48: a 065 adota os 21 índices do `database_indexes.sql` que a produção tem, com a definição de lá; os outros 15 já estão no banco novo ou não existem em nenhum dos dois. Evidência: `server/test/schema_ddl_containment_test.dart` (auditor de `server/bin`, `server/lib`, `server/routes`, `server/sql`, `scripts` e dos scripts Hermes que falam com PostgreSQL, com lista fechada de 17 exceções com motivo e contagem; 5 testes), `server/test/migration_sql_literal_test.dart` (9), `server/test/schema_adoption_migrations_test.dart` (12, contra a saída da auditoria `BT-DB-001`), `server/test/schema_reset_tombstone_test.dart` (5, executa os binários), `server/test/card_cli_schema_guard_test.py` (4) e o teste do Hermes (35), agora ligados em `scripts/manaloom_release_ops_contract_test.sh`, e `server/test/schema_adoption_db_live_test.dart` (8, com `RUN_SCHEMA_DB_TESTS=1` em PostgreSQL descartável migrado do zero: índices com a definição da produção, a view da produção mais a 063 igual à do banco novo, o 23505 do índice legado do fichário e as requisições de cada CLI); 26 mutações falham como esperado. Segue aberto: a decisão sobre o `UNIQUE` do fichário; a leitura D-67 na produção e o aperto das colunas; aplicar a 063 a 065 na produção; o destino dos 4 builders de pacote; os outros 21 índices só da produção (proposta de 066, sem decisão); 7 índices adotados duplicam, com outro nome, um índice do banco novo, como já acontece na produção; o gerador de project logic ainda não classifica DDL como mutação. |
...
| `BT-CAP-001` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | Medir host e criar política de capacidade atual versionada. | `BT-GOV-001` | Memória/CPU/swap/DB/resources por serviço; sem reutilizar números de worktree temporário. Decisão do dono em 2026-09-22: D-14, leitura SSH somente leitura do host autorizada (CPU, memória e disco por container); o repositório ainda não tem ferramenta para isso. Primeira leitura somente leitura em 2026-09-23 (4 vCPU, 7.941 MB de RAM, disco em 70%, consumo por serviço e retrato antes e depois do reinício do host) em `docs/qa/execution/2026-09-23/host-xmage-e-reinicio.md`; faltam a política versionada e a ferramenta no repositório. Feito em 2026-09-24, na branch `banco/rodada1-2026-09-24`, sem tocar a produção: `server/config/capacity_policy.json` é a política versionada, com a leitura de 2026-09-23 como linha de base e a procedência de cada número (o validador exige que o valor apareça no trecho citado do receipt, e o receipt tem de estar em `docs/qa/execution/`, nunca em `/tmp` ou worktree), os limites do preflight com a justificativa de cada um (proposta desta tarefa, pendente de confirmação do dono) e o que a leitura não cobriu (swap, load, PostgreSQL e resources além do XMage). `scripts/manaloom_capacity_snapshot.sh` é a ferramenta de leitura: sem `--execute` só descreve; com `--execute` usa a âncora de host do deploy (`MANALOOM_EXPECTED_SSH_HOST_KEY_SHA256`), roda no host só leituras (`/proc`, `df`, `docker stats`, `docker service ls` e `inspect`) e no PostgreSQL `server/sql/readonly/capacity_postgres.sql` pelo wrapper em modo leitura, e grava um snapshot com procedência (alvo SSH, fingerprint do host e SHA da ferramenta). `scripts/manaloom_capacity_policy.py preflight` devolve PASS ou BLOCKED (memória, disco, swap, load, conexões e idade do snapshot) e é o que o `BT-CAP-002` e o `BT-GATE-006` vão chamar. `server/doc/CAPACITY_PLAN_10K_MAU.md` ficou marcado como histórico. Evidência: `server/test/capacity_policy_test.py` (21 testes, com shims de `ssh`, `git` e do wrapper: nenhuma conexão sai da máquina), ligado em `scripts/manaloom_release_ops_contract_test.sh`, e o inventário de chamadores do wrapper atualizado; 14 mutações falham como esperado. Segue aberto: a coordenação rodar a ferramenta na produção (D-14) e fixar na política a fingerprint do host; o dono confirmar os limites. |
...
| `BT-OBS-001` | P0 CORE | IMPLEMENTED_LOCAL_PENDING_FULL_GATE | SLOs e alertas de API, PG, jobs, cache, catálogo e releases. | `BT-KPI-001` | Receiver humano, thresholds, alert test e runbook; PII excluída. Medição de 2026-09-22 (`docs/flows/_p0/README.md`): ciclo — `BT-REL-001` depende deste ID, mas o alerta de release daqui precisa da identidade de `BT-REL-002`, que depende de `BT-REL-001`; é preciso partir este ID (decisão do dono). Decisão do dono em 2026-09-22: D-51, dividido: esta tarefa cobre SLOs e alertas de API, banco, jobs e catálogo, e o alerta de release vai para o `BT-OBS-003`; D-47, SLO de 99,5% de disponibilidade e p95 abaixo de 800 ms em leitura, alerta para o dono por e-mail ou Telegram, sem TSDB (PostgreSQL e `/health/metrics`). Feito em 2026-09-24, na branch `banco/rodada1-2026-09-24`, sem tocar a produção: `server/config/slo_alert_policy.json` versiona os dois SLOs da D-47 (disponibilidade de 99,5% em 30 dias, medida por janela de 5 minutos, e p95 de leitura abaixo de 800 ms), 12 regras de alerta de API, PostgreSQL, jobs do daemon, cache e catálogo, com limites justificados (proposta pendente de confirmação do dono), e o receptor humano (o dono, por e-mail pelo Resend ou Telegram, com destino e credenciais só no ambiente do ops). `RequestMetricsService` passou a guardar janelas de 5 e 60 minutos (um apagão depois de dias no ar agora dispara), a chave de métrica é o template da rota (nenhum UUID em chave, código de alerta ou mensagem) e o número de rotas tem teto; `/health/metrics` devolve as janelas e o tamanho do cache; o painel usa a janela de 5 minutos, e os limites dele ficaram presos à versão 3. `server/bin/manaloom_slo_alerts.py` é o avaliador, job `manaloom_slo_alerts` do daemon de ops a cada 5 minutos (roda até com política inválida): só lê (`/health/live`, `/health/ready`, `/health/metrics` com a chave de ops, PostgreSQL em READ ONLY e o manifesto de jobs), grava a observação de 5 minutos e o estado no volume do ops, manda uma mensagem por avaliação (novos, abertos há mais de 60 minutos e resolvidos) e, sem receptor configurado, grava a observação e termina com erro; `test-alert` é o teste de alerta e `report` calcula o SLO dos últimos 30 dias. Runbook: `docs/runbooks/SLO_E_ALERTAS.md`, uma seção por alerta. Evidência: `server/test/request_metrics_service_test.dart` (9), `operational_alerts_test.dart` (2 testes novos), `server/test/slo_alerts_test.py` (22, com HTTP, PostgreSQL e canal falsos), `server/test/slo_alerts_db_live_test.py` (1, `RUN_SCHEMA_DB_TESTS=1`) e o teste do daemon (26); 23 mutações falham como esperado. Segue aberto: configurar o serviço de ops (URL interna da API, chave de ops e receptor) e o teste de alerta reconhecido pelo dono, que são da coordenação e do dono; a queda vista de fora é o `BT-OBS-002`. |
