...
- O commit com a correcao do harness em `master` ficou
...
## 2026-07-07 — PG651 damage wipe combat/damaged scope no servidor novo

- Alvo operacional: PostgreSQL novo via `server/bin/with_new_server_pg.sh`
  (`127.0.0.1:15432/halder`).
- Subpadrões XMage -> ManaLoom fechados:
  - `Inflame`: `damage_scope=each_creature_dealt_damage_this_turn`, exigindo
    `WasDealtDamageThisTurnPredicate.instance` no Java XMage;
  - `Warpath`: `damage_scope=each_blocking_or_blocked_creature`, exigindo
    `BlockingPredicate.instance` e `BlockedPredicate.instance` no Java XMage.
- Pacote PostgreSQL:
  - `pg651_damage_combat_or_damaged_scope_new_server_package_precheck.sql`;
  - `pg651_damage_combat_or_damaged_scope_new_server_package_apply.sql`;
  - `pg651_damage_combat_or_damaged_scope_new_server_package_postcheck.sql`;
  - `pg651_damage_combat_or_damaged_scope_new_server_package_rollback.sql`.
- Resultado do apply:
  - precheck: `existing_rule_rows=0` para as duas cartas;
  - apply: `upserted_rows=2`;
  - postcheck: cada carta com `promoted_rule_rows=1`,
    `promoted_verified_auto_rows=1` e `promoted_oracle_hash_rows=1`.
- Sync PG -> Hermes/SQLite:
  - `pg_rows_loaded=5919`;
  - `sqlite_inserted_or_updated=5905`;
  - `canonical_snapshot_rows_exported=5882`.
- Estado global pós-PG651:
  - `battle_and_oracle_ready=5979`;
  - `battle_family_mapper_required=27897`;
  - `snapshot_has_verified_rule=6007`;
  - `snapshot_has_any_rule=7214`;
  - fila Commander-legal: `target_identity_count=24974`,
