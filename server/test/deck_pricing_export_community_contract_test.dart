...
void main() {
  group('pricing/export/community source contracts', () {
    test('pricing route is write-capable and returns current response fields',
        () {
      final source =
      ...
      expect(responseSource, contains("'currency': 'USD'"));
    ...
    test('community copy returns deck wrapper and copies basic card fields',
      ...
      final responseEnd =
          source.indexOf('  } on Exception catch', responseStart);
      expect(responseStart, isNonNegative);
      expect(responseEnd, greaterThan(responseStart));
      final responseSource = source.substring(responseStart, responseEnd);
