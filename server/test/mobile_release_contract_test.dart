...
void main() {
  test('mobile release requires clean source, signing and checksum validation', () {
    final build =
        File('../scripts/manaloom_build_android_release.sh').readAsStringSync();
    final publish =
        File(
    ...
    expect(build, contains('interactive_battle_enabled'));
    ...
    try {
      ...
      final blocked = Process.runSync(
        '/bin/bash',
        [File('../scripts/manaloom_publish_android_release.sh').absolute.path],
        environment: {
