...
void main() {
  group('saved deck fetch/hydration source contracts', () {
    test('GET /decks returns raw list rows with count and presentation colors',
        () {
      ...
      expect(
          source, contains('COALESCE(SUM(dc.quantity), 0)::int as card_count'));
      expect(
        source,
        contains(
    ...
    test('GET /decks/:id returns root detail fields and UI aggregates only',
        () {
      ...
      expect(source, contains("'color_identity': deckColorIdentity.toList()"));
      expect(source, contains("'stats': {"));
      expect(
        source,
        contains("'total_cards': cardsList.fold<int>"),
      );
    ...
    test(
        ...
        () {
      final source = File('routes/decks/[id]/index.dart').readAsStringSync();
      final cardsSelectStart =
          source.indexOf('final cardsResult = await conn.execute');
      final cardsSelectEnd =
          source.indexOf('// 3. Organizar', cardsSelectStart);
