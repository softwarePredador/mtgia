  ...
  group('AI generate learning boundary guards', () {
    test(
      ...
      () {
        ...
        expect(generateRoute, contains('loadUsableCommanderReferenceProfile('));
        expect(
          generateRoute,
        ...
        expect(
          RegExp(
            r'activeCommanderLearnedDeckCardNames\s*\(\s*activeLearnedDeck\s*,?\s*\)',
          ).hasMatch(generateRoute),
          isTrue,
        );
        ...
        expect(generateRoute, contains('activeLearnedDeck: activeLearnedDeck'));
        expect(
          generateRoute,
        ...
        expect(generateRoute, isNot(contains('promoted_learned_deck_pg')));
        expect(generateRoute, isNot(contains('GET /ai/commander-learning')));
      },
    );
    ...
    test(
      'coherent generate response exposes deterministic source diagnostics',
      () {
        final generateRoute =
            File('routes/ai/generate/index.dart').readAsStringSync();
        ...
        final fallbackHelperStart = generateRoute.indexOf(
    ...
    test(
      'generate fallback preserves original validation quality evidence',
      () {
        final generateRoute =
            File('routes/ai/generate/index.dart').readAsStringSync();
        ...
        expect(generateRoute, contains('validation.qualityEvidenceSummary()'));
      },
    );

    test('generate always exposes the deckbuilding contract diagnostics', () {
      final generateRoute =
          File('routes/ai/generate/index.dart').readAsStringSync();
      ...
      expect(
        ...
        ).allMatches(generateRoute),
        hasLength(3),
      );
      expect(
        generateRoute,
        isNot(contains('if (deckbuildingContractDiagnostics != null)')),
      );
      expect(
        generateRoute,
        isNot(
    ...
    test(
      ...
      () {
        ...
        expect(
          learningRoute,
          contains("'source': 'pg_commander_learned_decks'"),
        );
        expect(
          learningRoute,
