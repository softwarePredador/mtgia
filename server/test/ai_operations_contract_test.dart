...
void main() {
  ...
  test('backend deploy proves migrations 038-068 before remote mutation', () {
    final source =
        File('../scripts/manaloom_deploy_backend_image.sh').readAsStringSync();
    final guard =
    ...
    final migration041To058Definition = source.indexOf(
      'require_migrations_041_068_contract() {',
    );
    ...
    expect(
      source,
      contains('.checks.release_schema.required_range == "038-068"'),
    );
    expect(
      source,
      contains('.checks.release_schema.latest_migration == "068"'),
    );
