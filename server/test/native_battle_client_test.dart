...
void main() {
  ...
  test('rejects a zero-turn completed native payload', () async {
    final client = NativeBattleClient(
      baseUrl: 'http://native:8080',
      client: MockClient(
  ...
  test('rejects an error-bearing completed native payload', () async {
    final client = NativeBattleClient(
      baseUrl: 'http://native:8080',
      client: MockClient(
        (_) async => http.Response(
  ...
  test('maps service failures without falling back silently', () async {
    final client = NativeBattleClient(
      ...
      client: MockClient(
        (request) async =>
            http.Response(jsonEncode({'error': 'native_runtime_failed'}), 500),
      ),
    );
  ...
  test(
    'rejects a successful response with an untrusted engine contract',
    () async {
      final client = NativeBattleClient(
