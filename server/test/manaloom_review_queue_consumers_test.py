...
def _write_counterspell_gate_fixture(tmp: Path) -> Path:
    fixture = {
        ...
        "cards": [
            ...
            {
                "card_id": "card-clean-draw",
                "oracle_id": "oracle-clean-draw",
                "name": "Clean Draw",
                "mana_cost": "{R}",
                "type_line": "Sorcery",
...
def _write_exile_removal_fixture(tmp: Path) -> Path:
    fixture = {
        "commanders": [
            {
        ...
        "cards": [
            {
                "card_id": "card-clean-exile",
                "oracle_id": "oracle-clean-exile",
                "name": "Clean Exile",
                "mana_cost": "{1}{W}",
                "type_line": "Instant",
    ...
    path = tmp / "exile_removal_fixture.json"
    path.write_text(json.dumps(fixture), encoding="utf-8")
    return path


def _write_exile_nonland_permanent_fixture(tmp: Path) -> Path:
    fixture = {
        "commanders": [
            {
                "name": "Brago, King Eternal",
        ...
        "cards": [
            {
                "card_id": "card-clean-exile-nonland",
                "oracle_id": "oracle-clean-exile-nonland",
                "name": "Clean Exile Nonland",
                "mana_cost": "{2}{W}",
                "type_line": "Sorcery",
    ...
    path = tmp / "exile_nonland_fixture.json"
    path.write_text(json.dumps(fixture), encoding="utf-8")
    return path
...
def _write_exile_artifact_or_enchantment_fixture(tmp: Path) -> Path:
    fixture = {
        "commanders": [
            {
                "name": "Brago, King Eternal",
        ...
        "cards": [
            {
                "card_id": "card-clean-exile-artifact-or-enchantment",
                "oracle_id": "oracle-clean-exile-artifact-or-enchantment",
                "name": "Clean Exile Artifact Or Enchantment",
                "mana_cost": "{1}{W}",
                "type_line": "Instant",
    ...
    path = tmp / "exile_artifact_or_enchantment_fixture.json"
    path.write_text(json.dumps(fixture), encoding="utf-8")
    return path


def _write_artifact_recursion_fixture(tmp: Path) -> Path:
    fixture = {
        "commanders": [
            {
                "name": "Lorehold, the Historian",
        ...
        "cards": [
            {
                "card_id": "card-clean-artifact-recursion",
                "oracle_id": "oracle-clean-artifact-recursion",
                "name": "Clean Artifact Recursion",
                "mana_cost": "{1}{W}",
                "type_line": "Sorcery",
    ...
    path = tmp / "artifact_recursion_fixture.json"
    path.write_text(json.dumps(fixture), encoding="utf-8")
    return path


def _write_enchantment_recursion_fixture(tmp: Path) -> Path:
    fixture = {
        "commanders": [
            {
                "name": "Lorehold, the Historian",
        ...
        "cards": [
            {
                "card_id": "card-clean-enchantment-recursion",
                "oracle_id": "oracle-clean-enchantment-recursion",
                "name": "Clean Enchantment Recursion",
                "mana_cost": "{1}{W}",
                "type_line": "Sorcery",
                "oracle_text": "Return target enchantment card from your graveyard to your hand.",
                "color_identity": ["W"],
                "cmc": 2,
                "set_code": "mar",
                "legalities": {"commander": "legal"},
            ...
            {
                "card_id": "card-clean-artifact-enchantment-recursion",
                "oracle_id": "oracle-clean-artifact-enchantment-recursion",
                "name": "Clean Artifact Enchantment Recursion",
                "mana_cost": "{1}{W}",
                "type_line": "Sorcery",
                "oracle_text": (
    ...
    path = tmp / "enchantment_recursion_fixture.json"
    path.write_text(json.dumps(fixture), encoding="utf-8")
    return path
...
class ManaloomReviewQueueConsumersTest(unittest.TestCase):
    ...
    def test_exile_removal_uses_dedicated_exile_template_before_promotion(self) -> None:
        ...
        battle_queue = _load_module(
            "manaloom_battle_rule_review_queue_exile_removal",
            "bin/manaloom_battle_rule_review_queue.py",
        )
        focused_evidence = _load_module(
            "manaloom_battle_rule_focused_evidence_exile_removal",
            "bin/manaloom_battle_rule_focused_evidence.py",
        )
        promotion_gate = _load_module(
            "manaloom_battle_rule_promotion_gate_exile_removal",
            "bin/manaloom_battle_rule_promotion_gate.py",
        )

        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            fixture = _write_exile_removal_fixture(tmp)
            knowledge_db = tmp / "knowledge.db"
    ...
    def test_exile_nonland_permanent_uses_dedicated_template_before_promotion(self) -> None:
        candidate = _load_module(
            "manaloom_new_card_candidate_review_exile_nonland",
            "bin/manaloom_new_card_candidate_review.py",
        )
        battle_queue = _load_module(
            "manaloom_battle_rule_review_queue_exile_nonland",
            "bin/manaloom_battle_rule_review_queue.py",
        )
        focused_evidence = _load_module(
            "manaloom_battle_rule_focused_evidence_exile_nonland",
            "bin/manaloom_battle_rule_focused_evidence.py",
        )
        promotion_gate = _load_module(
            "manaloom_battle_rule_promotion_gate_exile_nonland",
            "bin/manaloom_battle_rule_promotion_gate.py",
        )

        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            fixture = _write_exile_nonland_permanent_fixture(tmp)
            knowledge_db = tmp / "knowledge.db"
            ...
            self.assertEqual(evidence_summary["evidence_count"], 1)
            self.assertEqual(
                evidence_summary["reasons"].get("exile_target_nonland_permanent_supported"),
                1,
            )
    ...
    def test_exile_artifact_or_enchantment_uses_combined_template_before_promotion(self) -> None:
        candidate = _load_module(
            "manaloom_new_card_candidate_review_exile_artifact_or_enchantment",
            "bin/manaloom_new_card_candidate_review.py",
        )
        battle_queue = _load_module(
            "manaloom_battle_rule_review_queue_exile_artifact_or_enchantment",
            "bin/manaloom_battle_rule_review_queue.py",
        )
        focused_evidence = _load_module(
            "manaloom_battle_rule_focused_evidence_exile_artifact_or_enchantment",
            "bin/manaloom_battle_rule_focused_evidence.py",
        )
        promotion_gate = _load_module(
            "manaloom_battle_rule_promotion_gate_exile_artifact_or_enchantment",
            "bin/manaloom_battle_rule_promotion_gate.py",
        )

        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            fixture = _write_exile_artifact_or_enchantment_fixture(tmp)
            knowledge_db = tmp / "knowledge.db"
            ...
            self.assertEqual(evidence_summary["evidence_count"], 1)
            self.assertEqual(
                evidence_summary["reasons"].get("exile_target_artifact_or_enchantment_supported"),
                1,
            )
    ...
    def test_artifact_recursion_uses_target_type_before_promotion(self) -> None:
        candidate = _load_module(
            "manaloom_new_card_candidate_review_artifact_recursion",
            "bin/manaloom_new_card_candidate_review.py",
        )
        battle_queue = _load_module(
            "manaloom_battle_rule_review_queue_artifact_recursion",
            "bin/manaloom_battle_rule_review_queue.py",
        )
        focused_evidence = _load_module(
            "manaloom_battle_rule_focused_evidence_artifact_recursion",
            "bin/manaloom_battle_rule_focused_evidence.py",
        )
        promotion_gate = _load_module(
            "manaloom_battle_rule_promotion_gate_artifact_recursion",
            "bin/manaloom_battle_rule_promotion_gate.py",
        )

        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            fixture = _write_artifact_recursion_fixture(tmp)
            knowledge_db = tmp / "knowledge.db"
            ...
            self.assertIn("graveyard_or_zone_recursion", drafts[0]["effect_families"])
            ...
            self.assertEqual(evidence_summary["evidence_count"], 1)
            self.assertEqual(
                evidence_summary["reasons"].get(
                    "return_target_artifact_card_from_graveyard_to_hand_supported"
                ),
                1,
            )
    ...
    def test_enchantment_recursion_uses_target_type_before_promotion(self) -> None:
        candidate = _load_module(
            "manaloom_new_card_candidate_review_enchantment_recursion",
            "bin/manaloom_new_card_candidate_review.py",
        )
        battle_queue = _load_module(
            "manaloom_battle_rule_review_queue_enchantment_recursion",
            "bin/manaloom_battle_rule_review_queue.py",
        )
        focused_evidence = _load_module(
            "manaloom_battle_rule_focused_evidence_enchantment_recursion",
            "bin/manaloom_battle_rule_focused_evidence.py",
        )
        promotion_gate = _load_module(
            "manaloom_battle_rule_promotion_gate_enchantment_recursion",
            "bin/manaloom_battle_rule_promotion_gate.py",
        )

        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            fixture = _write_enchantment_recursion_fixture(tmp)
            knowledge_db = tmp / "knowledge.db"
            ...
            self.assertEqual(
                evidence_summary["reasons"].get(
                    "return_target_enchantment_card_from_graveyard_to_hand_supported"
                ),
                1,
            )
            self.assertEqual(
                evidence_summary["reasons"].get(
                    "return_target_artifact_or_enchantment_card_from_graveyard_to_hand_supported"
                ),
                1,
            )
