...
void main() {
  group('experimental deck/AI authorization source guards', () {
    ...
    test('weakness-analysis recommendations use DB lookup over fixed staples',
      ...
      expect(
          weakness, contains('commanderColorIdentity.addAll(colorIdentity)'));
      expect(
        weakness,
        contains("'commander_color_identity'"),
      );
      expect(
          weakness, contains("'color_identity_source': colorIdentitySource"));
      expect(
          weakness, contains('Commander decks geralmente precisam de 33-38'));
    ...
    test('weakness-analysis response shape is covered without live DB writes',
        () {
      final weakness = File(
        'routes/ai/weakness-analysis/index.dart',
      ).readAsStringSync();
    ...
    test('AI simulation routes clamp runs and expose non-live response shapes',
        () {
      final simulate = File(
        'routes/ai/simulate/index.dart',
      ).readAsStringSync();
      ...
      expect(simulate, contains('_normalizedSimulationCount('));
      expect(simulate, contains('parsed.clamp(min, max)'));
      ...
      expect(matchup, contains("'color_identity_source': colorIdentitySource"));
    ...
    test(
      ...
      expect(advisorySupport, contains('buildOpenAiRecommendationsErrorBody'));
      ...
      expect(fallbackSupport, contains('buildHeuristicRecommendationsBody'));
      expect(fallbackSupport, contains('buildHeuristicRecommendationsForDeck'));
      ...
      expect(
          routeSupport, isNot(contains("body: {'error': 'OpenAI API Error")));
