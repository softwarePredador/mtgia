...
Handler buildTrades$idHandler(String id,) {
  final pipeline = const Pipeline().addMiddleware(trades_middleware.middleware);
  final router = Router()
    ..all('/messages', (context) => trades_$id_messages.onRequest(context,id,))..all('/respond', (context) => trades_$id_respond.onRequest(context,id,))..all('/status', (context) => trades_$id_status.onRequest(context,id,))..all('/', (context) => trades_$id_index.onRequest(context,id,));
  return pipeline.addHandler(router);
}
...
Handler buildNotificationsHandler() {
  ...
  final router = Router()
    ..all('/count', (context) => notifications_count.onRequest(context,))..all('/read-all', (context) => notifications_read_all.onRequest(context,))..all('/', (context) => notifications_index.onRequest(context,));
  return pipeline.addHandler(router);
}
...
Handler buildConversations$idHandler(String id,) {
  final pipeline = const Pipeline().addMiddleware(conversations_middleware.middleware);
  final router = Router()
    ..all('/messages', (context) => conversations_$id_messages.onRequest(context,id,))..all('/read', (context) => conversations_$id_read.onRequest(context,id,));
  return pipeline.addHandler(router);
}
...
Handler buildCommunityUsersHandler() {
  ...
  final router = Router()
    ..all('/<id>', (context,id,) => community_users_$id.onRequest(context,id,))..all('/', (context) => community_users_index.onRequest(context,));
  return pipeline.addHandler(router);
}
...
Handler buildCommunityDecksHandler() {
  final pipeline = const Pipeline().addMiddleware(community_middleware.middleware);
  final router = Router()
    ..all('/<id>', (context,id,) => community_decks_$id.onRequest(context,id,))..all('/', (context) => community_decks_index.onRequest(context,));
  return pipeline.addHandler(router);
}
...
Handler buildAuthHandler() {
  ...
  final router = Router()
    ..all('/login', (context) => auth_login.onRequest(context,))..all('/me', (context) => auth_me.onRequest(context,))..all('/register', (context) => auth_register.onRequest(context,));
  return pipeline.addHandler(router);
}
