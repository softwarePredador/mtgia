...
def supports_exile_target_nonland_permanent_template(draft: DraftRecord) -> bool:
    text = str(draft.draft.get("oracle_text_excerpt") or "").strip().lower()
    return (
        "targeted_interaction" in draft.effect_families
        and text == "exile target nonland permanent."
        and draft.proposed_status == "needs_review"
    )


def supports_exile_target_artifact_template(draft: DraftRecord) -> bool:
    text = str(draft.draft.get("oracle_text_excerpt") or "").strip().lower()
    return (
        "targeted_interaction" in draft.effect_families
        and text == "exile target artifact."
        and draft.proposed_status == "needs_review"
    )


def supports_exile_target_enchantment_template(draft: DraftRecord) -> bool:
    text = str(draft.draft.get("oracle_text_excerpt") or "").strip().lower()
    return (
        "targeted_interaction" in draft.effect_families
        and text == "exile target enchantment."
        and draft.proposed_status == "needs_review"
    )


def supports_exile_target_artifact_or_enchantment_template(draft: DraftRecord) -> bool:
    text = str(draft.draft.get("oracle_text_excerpt") or "").strip().lower()
    return (
        "targeted_interaction" in draft.effect_families
        and text == "exile target artifact or enchantment."
        and draft.proposed_status == "needs_review"
    )
...
def supports_simple_treasure_template(draft: DraftRecord) -> bool:
    text = str(draft.draft.get("oracle_text_excerpt") or "").strip().lower()
    return (
        "treasure_resource_generation" in draft.effect_families
        and text == "create a treasure token."
        and draft.proposed_status == "needs_review"
    )
...
def supports_return_target_creature_from_graveyard_template(draft: DraftRecord) -> bool:
    text = str(draft.draft.get("oracle_text_excerpt") or "").strip().lower()
    return (
        "graveyard_or_zone_recursion" in draft.effect_families
        and text == "return target creature card from your graveyard to your hand."
        and draft.proposed_status == "needs_review"
    )
...
def build_exile_target_nonland_permanent_evidence(draft: DraftRecord, output_dir: Path) -> EvidenceResult:
    battle = load_module(
        "battle_analyst_exile_target_nonland_permanent_evidence",
        HERMES_SCRIPTS_DIR / "battle_analyst_v9.py",
    )
    replay_auditor = load_module(
        "replay_decision_auditor_exile_target_nonland_permanent_evidence",
        HERMES_SCRIPTS_DIR / "replay_decision_auditor.py",
    )
    if battle.battle_rule_registry is None:
        return EvidenceResult(
    ...
    try:
        ...
        with closing(sqlite3.connect(runtime_db)) as conn:
            battle.battle_rule_registry.upsert_battle_card_rule(
                ...
                review_status="verified",
                oracle_hash=f"focused:{draft.oracle_id or draft.card_name}",
                notes="Temporary focused-evidence rule for exile target nonland permanent.",
            )
            conn.commit()
        battle.DB = str(runtime_db)
        battle.battle_rule_registry._RULE_CACHE.clear()
        ...
        battle.apply_effect_immediate(active, [opponent], spell, turn=4, rng=random.Random(14))
    finally:
        battle.DB = old_db
        if battle.battle_rule_registry is not None:
            battle.battle_rule_registry._RULE_CACHE.clear()
    ...
    focused_passed = bool(
        ...
        and any(card.get("name") == "Problem Artifact" for card in opponent.exile)
        and not any(card.get("name") == "Problem Artifact" for card in opponent.graveyard)
        and any(card.get("name") == "Small Creature" for card in opponent.battlefield)
    ...
    focused_payload = {
        ...
        "checks": {
            ...
            "artifact_in_exile": any(card.get("name") == "Problem Artifact" for card in opponent.exile),
            "artifact_not_in_graveyard": not any(card.get("name") == "Problem Artifact" for card in opponent.graveyard),
        ...
        "scope": "exile_target_nonland_permanent",
    }
    focused_path.write_text(json.dumps(focused_payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    evidence = {
        ...
        "official_sources": [
            f"Scryfall oracle text for oracle_id:{draft.oracle_id}",
            "Oracle text template: Exile target nonland permanent.",
        ],
        "focused_test_passed": focused_passed,
        "focused_test_refs": [str(focused_path)],
        "replay_audit_passed": counts.get("critical", 0) == 0 and counts.get("high", 0) == 0,
        ...
        "critical_findings": counts.get("critical", 0),
        "high_findings": counts.get("high", 0),
        "evidence_scope": "hard_behavior_exile_target_nonland_permanent_v1",
        "generated_by": "manaloom_battle_rule_focused_evidence",
    }
    return EvidenceResult(
        draft=draft,
        status="evidence_ready" if focused_passed else "evidence_failed",
        reason="exile_target_nonland_permanent_supported",
        evidence=evidence,
        artifacts=[str(focused_path), str(audit_path), str(events_path), str(decisions_path)],
    )


def build_exile_target_permanent_type_evidence(
    draft: DraftRecord,
    output_dir: Path,
    *,
    target_type: str,
    ...
    battle = load_module(
        f"battle_analyst_exile_target_{target_type}_evidence",
        HERMES_SCRIPTS_DIR / "battle_analyst_v9.py",
    )
    replay_auditor = load_module(
        f"replay_decision_auditor_exile_target_{target_type}_evidence",
        HERMES_SCRIPTS_DIR / "replay_decision_auditor.py",
    )
    if battle.battle_rule_registry is None:
        return EvidenceResult(
    ...
    try:
        ...
        with closing(sqlite3.connect(runtime_db)) as conn:
            battle.battle_rule_registry.upsert_battle_card_rule(
                ...
                review_status="verified",
                oracle_hash=f"focused:{draft.oracle_id or draft.card_name}",
                notes=f"Temporary focused-evidence rule for exile target {target_type}.",
            )
            conn.commit()
        battle.DB = str(runtime_db)
        battle.battle_rule_registry._RULE_CACHE.clear()
        ...
        battle.apply_effect_immediate(active, [opponent], spell, turn=4, rng=random.Random(15))
    finally:
        battle.DB = old_db
        if battle.battle_rule_registry is not None:
            battle.battle_rule_registry._RULE_CACHE.clear()
    ...
    focused_passed = bool(
        ...
        and any(card.get("name") == target_name for card in opponent.exile)
        and not any(card.get("name") == target_name for card in opponent.graveyard)
    ...
    focused_payload = {
        ...
        "checks": {
            ...
            "target_in_exile": any(card.get("name") == target_name for card in opponent.exile),
            "target_not_in_graveyard": not any(card.get("name") == target_name for card in opponent.graveyard),
            "decoy_preserved": any(card.get("name") == decoy_name for card in opponent.battlefield),
            "land_preserved": any(card.get("name") == "Basic Land" for card in opponent.battlefield),
            "critical_findings": counts.get("critical", 0),
            "high_findings": counts.get("high", 0),
        },
        "scope": f"exile_target_{target_type}",
    }
    focused_path.write_text(json.dumps(focused_payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    evidence = {
        ...
        "critical_findings": counts.get("critical", 0),
        "high_findings": counts.get("high", 0),
        "evidence_scope": f"hard_behavior_exile_target_{target_type}_v1",
        "generated_by": "manaloom_battle_rule_focused_evidence",
    }
    return EvidenceResult(
        draft=draft,
        status="evidence_ready" if focused_passed else "evidence_failed",
        reason=f"exile_target_{target_type}_supported",
        evidence=evidence,
        artifacts=[str(focused_path), str(audit_path), str(events_path), str(decisions_path)],
    )


def build_exile_target_artifact_evidence(draft: DraftRecord, output_dir: Path) -> EvidenceResult:
    return build_exile_target_permanent_type_evidence(
        draft,
        output_dir,
        ...
        template_text="Exile target artifact.",
    )


def build_exile_target_enchantment_evidence(draft: DraftRecord, output_dir: Path) -> EvidenceResult:
    return build_exile_target_permanent_type_evidence(
        draft,
        output_dir,
        ...
        template_text="Exile target enchantment.",
    )


def build_exile_target_artifact_or_enchantment_evidence(
    draft: DraftRecord,
    output_dir: Path,
) -> EvidenceResult:
    battle = load_module(
        "battle_analyst_exile_target_artifact_or_enchantment_evidence",
        HERMES_SCRIPTS_DIR / "battle_analyst_v9.py",
    )
    replay_auditor = load_module(
        "replay_decision_auditor_exile_target_artifact_or_enchantment_evidence",
        HERMES_SCRIPTS_DIR / "replay_decision_auditor.py",
    )
    if battle.battle_rule_registry is None:
        return EvidenceResult(
    ...
    try:
        ...
        with closing(sqlite3.connect(runtime_db)) as conn:
            battle.battle_rule_registry.upsert_battle_card_rule(
                ...
                review_status="verified",
                oracle_hash=f"focused:{draft.oracle_id or draft.card_name}",
                notes="Temporary focused-evidence rule for exile target artifact or enchantment.",
            )
            conn.commit()
        battle.DB = str(runtime_db)
        battle.battle_rule_registry._RULE_CACHE.clear()
        ...
        lower_value_enchantment = {
            "name": "Small Enchantment",
            "cmc": 1,
        ...
        opponent.battlefield = [land, creature, lower_value_enchantment, target]
        spell = {
            "name": draft.card_name,
            "cmc": 2,
            "type_line": "Instant",
        ...
        battle.apply_effect_immediate(active, [opponent], spell, turn=4, rng=random.Random(16))
    finally:
        battle.DB = old_db
        if battle.battle_rule_registry is not None:
            battle.battle_rule_registry._RULE_CACHE.clear()
    ...
    removal_event = any(
        ...
        and row.get("target") == "Problem Artifact"
        and row.get("target_legal") is True
        and row.get("target_type") == "artifact_or_enchantment"
        and row.get("target_is_creature") is False
        and row.get("destination") == "exile"
        for row in event_rows
    )
    focused_passed = bool(
        ...
        and any(card.get("name") == "Problem Artifact" for card in opponent.exile)
        and not any(card.get("name") == "Problem Artifact" for card in opponent.graveyard)
        and any(card.get("name") == "Small Enchantment" for card in opponent.battlefield)
        and any(card.get("name") == "Small Creature" for card in opponent.battlefield)
        and any(card.get("name") == "Basic Land" for card in opponent.battlefield)
        and counts.get("critical", 0) == 0
        and counts.get("high", 0) == 0
    ...
    focused_payload = {
        ...
        "checks": {
            ...
            "artifact_in_exile": any(card.get("name") == "Problem Artifact" for card in opponent.exile),
            "artifact_not_in_graveyard": not any(card.get("name") == "Problem Artifact" for card in opponent.graveyard),
            "lower_value_enchantment_preserved": any(card.get("name") == "Small Enchantment" for card in opponent.battlefield),
            "creature_preserved": any(card.get("name") == "Small Creature" for card in opponent.battlefield),
            "land_preserved": any(card.get("name") == "Basic Land" for card in opponent.battlefield),
            "critical_findings": counts.get("critical", 0),
            "high_findings": counts.get("high", 0),
        },
        "scope": "exile_target_artifact_or_enchantment",
    }
    focused_path.write_text(json.dumps(focused_payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    evidence = {
        ...
        "official_sources": [
            f"Scryfall oracle text for oracle_id:{draft.oracle_id}",
            "Oracle text template: Exile target artifact or enchantment.",
        ],
        "focused_test_passed": focused_passed,
        "focused_test_refs": [str(focused_path)],
        "replay_audit_passed": counts.get("critical", 0) == 0 and counts.get("high", 0) == 0,
        ...
        "critical_findings": counts.get("critical", 0),
        "high_findings": counts.get("high", 0),
        "evidence_scope": "hard_behavior_exile_target_artifact_or_enchantment_v1",
        "generated_by": "manaloom_battle_rule_focused_evidence",
    }
    return EvidenceResult(
        draft=draft,
        status="evidence_ready" if focused_passed else "evidence_failed",
        reason="exile_target_artifact_or_enchantment_supported",
        evidence=evidence,
        artifacts=[str(focused_path), str(audit_path), str(events_path), str(decisions_path)],
    )
...
def build_simple_treasure_evidence(draft: DraftRecord, output_dir: Path) -> EvidenceResult:
    battle = load_module(
        "battle_analyst_treasure_evidence",
        HERMES_SCRIPTS_DIR / "battle_analyst_v9.py",
    )
    replay_auditor = load_module(
        "replay_decision_auditor_treasure_evidence",
        HERMES_SCRIPTS_DIR / "replay_decision_auditor.py",
    )
    if battle.battle_rule_registry is None:
        return EvidenceResult(
    ...
    try:
        ...
        with closing(sqlite3.connect(runtime_db)) as conn:
            battle.battle_rule_registry.upsert_battle_card_rule(
                ...
                review_status="verified",
                oracle_hash=f"focused:{draft.oracle_id or draft.card_name}",
                notes="Temporary focused-evidence rule for simple Treasure token creation.",
            )
            conn.commit()
        battle.DB = str(runtime_db)
        battle.battle_rule_registry._RULE_CACHE.clear()
        ...
        battle.apply_effect_immediate(active, [opponent], spell, turn=3, rng=random.Random(22))
    finally:
        battle.DB = old_db
        if battle.battle_rule_registry is not None:
            battle.battle_rule_registry._RULE_CACHE.clear()
    ...
    resolved_event = any(
        row.get("event") == "spell_resolved"
        and row.get("card") == draft.card_name
        and row.get("effect") == "treasure_maker"
        for row in event_rows
    )
    treasure_event = any(
        row.get("event") == "treasure_created"
        and row.get("card") == draft.card_name
        and row.get("treasures_created") == 1
        and row.get("treasures") == 1
        for row in event_rows
    )
    ...
    focused_payload = {
        ...
        "passed": focused_passed,
        "checks": {
            "spell_resolved_as_treasure_maker": resolved_event,
            "treasure_created_event": treasure_event,
            "treasure_count_after_resolution": active.treasures,
            "spell_finished_in_graveyard": spell_finished,
            "critical_findings": counts.get("critical", 0),
            "high_findings": counts.get("high", 0),
        },
        "scope": "simple_treasure_resource_generation",
    }
    focused_path.write_text(json.dumps(focused_payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    evidence = {
        ...
        "official_sources": [
            f"Scryfall oracle text for oracle_id:{draft.oracle_id}",
            "Oracle text template: Create a Treasure token.",
        ],
        "focused_test_passed": focused_passed,
        "focused_test_refs": [str(focused_path)],
        "replay_audit_passed": counts.get("critical", 0) == 0 and counts.get("high", 0) == 0,
        ...
        "critical_findings": counts.get("critical", 0),
        "high_findings": counts.get("high", 0),
        "evidence_scope": "hard_behavior_simple_treasure_generation_v1",
        "generated_by": "manaloom_battle_rule_focused_evidence",
    }
    return EvidenceResult(
        draft=draft,
        status="evidence_ready" if focused_passed else "evidence_failed",
        reason="simple_treasure_generation_supported",
        evidence=evidence,
        artifacts=[str(focused_path), str(audit_path), str(events_path), str(decisions_path)],
    )


def build_return_target_creature_from_graveyard_evidence(
    draft: DraftRecord,
    output_dir: Path,
) -> EvidenceResult:
    battle = load_module(
        "battle_analyst_creature_graveyard_recursion_evidence",
        HERMES_SCRIPTS_DIR / "battle_analyst_v9.py",
    )
    replay_auditor = load_module(
        "replay_decision_auditor_creature_graveyard_recursion_evidence",
        HERMES_SCRIPTS_DIR / "replay_decision_auditor.py",
    )
    if battle.battle_rule_registry is None:
        return EvidenceResult(
    ...
    try:
        ...
        with closing(sqlite3.connect(runtime_db)) as conn:
            battle.battle_rule_registry.upsert_battle_card_rule(
                ...
                review_status="verified",
                oracle_hash=f"focused:{draft.oracle_id or draft.card_name}",
                notes="Temporary focused-evidence rule for creature-card recursion to hand.",
            )
            conn.commit()
        battle.DB = str(runtime_db)
        battle.battle_rule_registry._RULE_CACHE.clear()
        ...
        active.graveyard = [noncreature_decoy, creature_target]
        spell = {
            "name": draft.card_name,
            "cmc": 2,
            "type_line": "Sorcery",
        ...
        battle.apply_effect_immediate(active, [opponent], spell, turn=4, rng=random.Random(23))
    finally:
        battle.DB = old_db
        if battle.battle_rule_registry is not None:
            battle.battle_rule_registry._RULE_CACHE.clear()
    ...
    recursion_event = any(
        ...
        and row.get("recovered") == ["Graveyard Creature"]
        for row in event_rows
    )
    creature_recovered = any(card.get("name") == "Graveyard Creature" for card in active.hand)
    decoy_preserved = any(card.get("name") == "Graveyard Sorcery" for card in active.graveyard)
    spell_finished = any(card.get("name") == draft.card_name for card in active.graveyard)
    focused_passed = bool(
        recursion_event
        and creature_recovered
        and decoy_preserved
        and spell_finished
        and counts.get("critical", 0) == 0
        and counts.get("high", 0) == 0
    ...
    focused_payload = {
        ...
        "checks": {
            ...
            "creature_recovered_to_hand": creature_recovered,
            "noncreature_decoy_preserved": decoy_preserved,
            "spell_finished_in_graveyard": spell_finished,
            "critical_findings": counts.get("critical", 0),
            "high_findings": counts.get("high", 0),
        },
        "scope": "return_target_creature_card_from_graveyard_to_hand",
    }
    focused_path.write_text(json.dumps(focused_payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    evidence = {
        ...
        "official_sources": [
            f"Scryfall oracle text for oracle_id:{draft.oracle_id}",
            "Oracle text template: Return target creature card from your graveyard to your hand.",
        ],
        "focused_test_passed": focused_passed,
        "focused_test_refs": [str(focused_path)],
        "replay_audit_passed": counts.get("critical", 0) == 0 and counts.get("high", 0) == 0,
        ...
        "critical_findings": counts.get("critical", 0),
        "high_findings": counts.get("high", 0),
        "evidence_scope": "hard_behavior_return_target_creature_card_from_graveyard_to_hand_v1",
        "generated_by": "manaloom_battle_rule_focused_evidence",
    }
    return EvidenceResult(
        draft=draft,
        status="evidence_ready" if focused_passed else "evidence_failed",
        reason="return_target_creature_card_from_graveyard_to_hand_supported",
        evidence=evidence,
        artifacts=[str(focused_path), str(audit_path), str(events_path), str(decisions_path)],
    )
...
def build_simple_draw_card_evidence(draft: DraftRecord, output_dir: Path) -> EvidenceResult:
    ...
    replay_auditor = load_module(
        "replay_decision_auditor_simple_draw_card_evidence",
        HERMES_SCRIPTS_DIR / "replay_decision_auditor.py",
    )
    if battle.battle_rule_registry is None:
        return EvidenceResult(
...
def evaluate_draft(draft: DraftRecord, output_dir: Path) -> EvidenceResult:
    ...
    if supports_exile_target_nonland_permanent_template(draft):
        return build_exile_target_nonland_permanent_evidence(draft, output_dir)
    if supports_exile_target_artifact_template(draft):
        return build_exile_target_artifact_evidence(draft, output_dir)
    if supports_exile_target_enchantment_template(draft):
        return build_exile_target_enchantment_evidence(draft, output_dir)
    if supports_exile_target_artifact_or_enchantment_template(draft):
        return build_exile_target_artifact_or_enchantment_evidence(draft, output_dir)
    ...
    if supports_simple_treasure_template(draft):
        return build_simple_treasure_evidence(draft, output_dir)
    if supports_simple_draw_card_template(draft):
