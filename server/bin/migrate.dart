...
final migrations = <Migration>[
  ...
  Migration(
    version: '052',
    name: 'version_battle_simulation_attempts',
    up: '''
      ...
      -- Historical replays stay readable through LEFT JOIN consumers. They are
      -- deliberately not promoted to versioned attempts because the legacy
      -- payload cannot prove a canonical terminal outcome, deck revision, or
      -- complete learning contract retroactively.
    ''',
    down: '''
      DROP TABLE IF EXISTS battle_simulation_attempts CASCADE;
    ''',
  ),
  Migration(
    version: '053',
    name: 'create_battle_replay_annotations',
    up: '''
            ...
            COALESCE(
              CASE kind
                ...
                WHEN 'note' THEN
                  ...
                  AND char_length(trim(payload->>'text')) BETWEEN 1 AND 2000
    ...
    ''',
    down: '''
      DROP TABLE IF EXISTS battle_replay_annotations CASCADE;
      DROP INDEX IF EXISTS uq_battle_attempt_id_replay;
    ''',
  ),
  Migration(
    version: '054',
    name: 'create_battle_jobs',
    up: '''
      ALTER TABLE battle_simulation_attempts
      ADD COLUMN IF NOT EXISTS job_request_schema_version TEXT;
      ALTER TABLE battle_simulation_attempts
      ADD COLUMN IF NOT EXISTS job_request_hash TEXT;
      ALTER TABLE battle_simulation_attempts
      ADD COLUMN IF NOT EXISTS engine_request_correlation_source TEXT;

      ALTER TABLE battle_simulation_attempts
      DROP CONSTRAINT IF EXISTS chk_battle_attempt_job_request_hash;
      ALTER TABLE battle_simulation_attempts
      ADD CONSTRAINT chk_battle_attempt_job_request_hash
        CHECK (
      ...
      ALTER TABLE battle_simulation_attempts
      DROP CONSTRAINT IF EXISTS chk_battle_attempt_request_correlation;
      ALTER TABLE battle_simulation_attempts
      ADD CONSTRAINT chk_battle_attempt_request_correlation
        CHECK (
        ...
        CONSTRAINT chk_battle_job_engine_request
          CHECK (
            ...
            OR (
              ...
              AND char_length(engine_request_schema_version) BETWEEN 1 AND 64
              AND engine_request_correlation_source IN (
                'sidecar_echo_validated',
                'server_dispatch_recorded'
              )
    ...
    ''',
    down: '''
      DROP TABLE IF EXISTS battle_jobs CASCADE;
      DROP INDEX IF EXISTS idx_battle_attempt_job_request_hash;
      ALTER TABLE battle_simulation_attempts
      DROP CONSTRAINT IF EXISTS chk_battle_attempt_request_correlation;
      ALTER TABLE battle_simulation_attempts
      DROP CONSTRAINT IF EXISTS chk_battle_attempt_job_request_hash;
      ALTER TABLE battle_simulation_attempts
      DROP COLUMN IF EXISTS engine_request_correlation_source;
      ALTER TABLE battle_simulation_attempts
      DROP COLUMN IF EXISTS job_request_hash;
      ALTER TABLE battle_simulation_attempts
      DROP COLUMN IF EXISTS job_request_schema_version;
    ''',
  ),
  Migration(
    version: '055',
    name: 'create_battle_job_live_records',
    up: '''
  ...
  Migration(
    version: '058',
    name: 'snapshot_trade_item_identity',
    up: '''
      ALTER TABLE trade_items
      ...
      DROP TRIGGER IF EXISTS manaloom_trade_item_snapshot_immutable
      ON trade_items;
      ...
      ADD CONSTRAINT chk_trade_items_snapshot_status CHECK (
        snapshot_status IN (
          'captured', 'legacy_recovered', 'legacy_unavailable'
        )
      );
      ALTER TABLE trade_items
    ...
    down: '''
      DROP TRIGGER IF EXISTS manaloom_trade_item_snapshot_immutable
      ON trade_items;
      DROP FUNCTION IF EXISTS manaloom_trade_item_snapshot_immutable();
      ALTER TABLE trade_items
      DROP CONSTRAINT IF EXISTS chk_trade_items_snapshot_lifecycle;
      ALTER TABLE trade_items
      DROP CONSTRAINT IF EXISTS chk_trade_items_snapshot_payload;
      ...
      ALTER TABLE trade_items
      DROP CONSTRAINT IF EXISTS chk_trade_items_snapshot_schema;
      ALTER TABLE trade_items DROP COLUMN IF EXISTS snapshot_captured_at;
      ALTER TABLE trade_items DROP COLUMN IF EXISTS item_snapshot;
      ALTER TABLE trade_items DROP COLUMN IF EXISTS snapshot_status;
...
MigrationRollbackPolicy migrationRollbackPolicy(String version) =>
    switch (version) {
      ...
      '033' || '035' || '060' || '067' => MigrationRollbackPolicy.emptyOnly,
