...
def validate(conn: sqlite3.Connection, artifacts_dir: Path) -> list[DeckValidation]:
    if not all(_table_exists(conn, name) for name in ("decks", "commanders", "deck_cards")):
        raise RuntimeError("knowledge.db missing required decks/commanders/deck_cards tables")
